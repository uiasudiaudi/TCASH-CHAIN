...
NS_BEG2(tcash, application)

xtcash_application::xtcash_application(common::xnode_id_t const & node_id, xpublic_key_t const & public_key, std::string && sign_key) // todo make it right value
  : m_node_id{node_id}
  , m_public_key{public_key}
  , m_network_id{tcash::config::to_chainid(XGET_CONFIG(chain_name))}
  ...
  , m_elect_main{tcash::make_unique<elect::ElectMain>(node_id, std::set<uint32_t>({static_cast<uint32_t>(m_network_id)}))}
  , m_router{tcash::make_unique<router::xrouter_t>()}
  ...
  , m_elect_client{tcash::make_unique<elect::xelect_client_imp>()} {

    safebox::xsafebox_proxy::get_instance().add_key_pair(public_key, std::move(sign_key));

    int db_kind = tcash::db::xdb_kind_kvdb;
    ...
    m_txstore = xobject_ptr_t<base::xvtxstore_t>(
        txstore::create_txstore(tcash::make_observer<mbus::xmessage_bus_face_t>(m_bus.get()), tcash::make_observer<xbase_timer_driver_t>(m_timer_driver)));
    ...
    if ((m_store == nullptr) || !m_store->open()) {
        xwarn("xtcash_application::start db open failed!");
        exit(0);
    }
...
void xtcash_application::start() {
    ...
    txpool_service_thp.push_back(make_object_ptr<base::xiothread_t>());
    m_thread_pools[xtcash_thread_pool_type::txpool_service] = txpool_service_thp;
    xthread_pool_t statestore_thp;
    statestore_thp.push_back(make_object_ptr<base::xiothread_t>());
    statestore_thp.push_back(make_object_ptr<base::xiothread_t>());
    m_thread_pools[xtcash_thread_pool_type::statestore] = statestore_thp;
    ...
    // construct
    {
        m_elect_client_process = std::make_shared<elect::xelect_client_process>(
            m_network_id,
            make_observer(m_bus),
            std::bind(&xtcash_application::on_election_data_updated, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3),
            make_observer(m_logic_timer));

        m_election_cache_data_accessor = tcash::make_unique<election::cache::xdata_accessor_t>(m_network_id, make_observer(m_logic_timer));
        ...
        m_sync_obj = tcash::make_unique<sync::xsync_object_t>(
        ...
        // m_grpc_mgr = tcash::make_unique<grpcmgr::xgrpc_mgr_t>(make_observer(m_bus), make_observer(m_grpc_thread));
    ...
    for (auto & io_context_pool_info : m_io_context_pools) {
        auto & io_context_pool = tcash::get<xio_context_pool_t>(io_context_pool_info);
    ...
    if (!m_elect_main->RegisterNodeCallback(std::bind(&tcash::application::xtcash_application::handle_register_node, this, std::placeholders::_1, std::placeholders::_2))) {
        throw std::logic_error{"register node callback failed!"};
    }

    if (!m_elect_main->UpdateNodeSizeCallback(std::bind(&tcash::application::xtcash_application::update_node_size, this, std::placeholders::_1, std::placeholders::_2))) {
...
void xtcash_application::stcash() {
    // stcash
    {
        m_sync_obj->stcash();
        m_vnode_manager->stcash();
        m_message_callback_hub->stcash();
        m_vhost->stcash();
    }

    m_elect_main->stcash();
    m_logic_timer->stcash();
    m_timer_driver->stcash();
    for (auto & io_context_pool_info : m_io_context_pools) {
        auto & io_context_pool = tcash::get<xio_context_pool_t>(io_context_pool_info);
        for (auto & io_context : io_context_pool) {
            io_context->stcash();
...
void xtcash_application::on_election_data_updated(data::election::xelection_result_store_t const & election_result_store,
...
void xtcash_application::load_last_election_data() {
    ...
    for (const auto & addr : sys_addr) {
        for (auto const & property : data::election::get_property_name_by_addr(addr)) {
            ...
            if (unitstate == nullptr) {
                xerror("xtcash_application::load_last_election_data fail-get state.");
                tcash::error::throw_error(ec);
                continue;
            }
            std::string result;
            if (xsuccess != unitstate->string_get(property, result)) {
                xerror("xtcash_application::load_last_election_data fail-get property.");
                tcash::error::throw_error(ec);
                continue;
            }
            ...
            if ((addr == rec_elect_rec_contract_address || addr == rec_elect_zec_contract_address || addr == zec_elect_consensus_contract_address ||
                ...
                if (unitstate2 == nullptr) {
                    xwarn("xtcash_application::load_last_election_data fail-get state.addr=%s,height=%ld", addr.to_string().c_str(), prev_block_height);
                } else {
...
base::xauto_ptr<tcash::base::xvblock_t> xtcash_application::last_logic_time() const {
...
int32_t xtcash_application::handle_register_node(std::string const & node_addr, std::string const & node_sign) {
    ...
    int ret = statestore::xstatestore_hub_t::instance()->map_get(rec_registration_contract_address, tcash::data::system_contract::XPORPERTY_CONTRACT_REG_KEY, node_addr, value_str);
...
void xtcash_application::update_node_size(uint64_t & node_size, std::error_code & ec) {
    assert(!ec);
    ...
    if (statestore::xstatestore_hub_t::instance()->string_get(rec_standby_pool_contract_address, data::XPROPERTY_CONTRACT_STANDBYS_KEY, serialized_value) == 0 &&
        ...
        common::xnetwork_id_t network_id{tcash::config::to_chainid(XGET_CONFIG(chain_name))};
...
bool xtcash_application::is_genesis_node() const noexcept {
    ...
    tcash::common::xnode_id_t node_id = user_params.account;

    for (auto const & item : seeds) {
        if (user_params.account == common::xnode_id_t{item.m_account}) {
            xinfo("xtcash_application::is_genesis_node is genesis node %s", node_id.to_string().c_str());
            return true;
        }
    }

    xwarn("xtcash_application::is_genesis_node not genesis node %s", node_id.to_string().c_str());
    return false;
}

bool xtcash_application::is_beacon_account() const noexcept {
    try {
        auto const & user_params = data::xuser_params::get_instance();
        tcash::common::xnode_id_t node_id = tcash::common::xnode_id_t{user_params.account};
        ...
        if (unitstate == nullptr) {
            xerror("xtcash_application::is_beacon_account fail-get state.");
            return false;
        }
        ...
        for (auto const & property : property_names) {
            ...
            if (result.empty()) {
                xwarn("xtcash_application::is_beacon_account no property %s", property.c_str());
                continue;
            }
            using tcash::data::election::xelection_result_store_t;
            ...
            if (tcash::get<bool>(current_group_nodes.find(node_id))) {
                xinfo("xtcash_application::is_beacon_account is genesis node. %s", node_id.to_string().c_str());
                return true;
            }
        }

        xwarn("xtcash_application::is_beacon_account not genesis %s", node_id.to_string().c_str());
        return false;
...
void xtcash_application::tcash_console_init() {
    tcash::elect::MultilayerNetworkInterfacePtr net_module = std::make_shared<elect::MultilayerNetworkChainQuery>();
    ChainCommandsPtr chain_cmd{nullptr};
    chain_cmd = std::make_shared<ChainCommands>(net_module, m_sync_obj.get());
    tcash::ChainInfo::Instance()->SetChainCmd(chain_cmd);
...
std::size_t hash<tcash::application::xthread_pool_type_t>::operator()(tcash::application::xthread_pool_type_t const type) const noexcept {
    return static_cast<std::size_t>(type);
};

std::size_t hash<tcash::application::xio_context_type_t>::operator()(tcash::application::xio_context_type_t const type) const noexcept {
    return static_cast<std::size_t>(type);
};
