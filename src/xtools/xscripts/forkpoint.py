...
def utc_to_tcash_clock(utc_timestamp):
    return (utc_timestamp - 1573189200) // 10

def tcash_clock_to_utc(tcash_clock):
    return tcash_clock * 10 + 1573189200

def beijing_to_clock(beijing_time):
    utc_timestamp = beijing_to_utc_timestamp(beijing_time)
    print(utc_to_tcash_clock(utc_timestamp))

def clock_to_beijing(clock):
    utc_timestamp = tcash_clock_to_utc(clock)
...
def parse_time(input_str):
    ...
    raise ValueError('Unable to parse input as either tcash Clock like 10608840 or Beijing time like "2023-03-20 10:00:00"')
