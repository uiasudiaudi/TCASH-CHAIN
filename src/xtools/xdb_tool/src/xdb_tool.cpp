...
void xdb_tool::init_xdb_tool(std::string const& db_path) {
    ...
    #endif
    store_ = tcash::store::xstore_factory::create_store_with_kvdb(db_path_);
    tcash::base::xvchain_t::instance().set_xdbstore(store_.get());
    blockstore_.attach(tcash::store::get_vblockstore());
}
...
void xdb_tool::get_voteinfo_from_block(std::string const& tableblock_addr, uint64_t start, uint64_t end) {
    ...
    for (uint64_t i = start; i <= end; ++i) {
        ...
        if (!is_xip2_empty(auditor_xip2)) {
            ...
            tcash::auth::xmutisigdata_t aggregated_sig_obj;
...
std::string xdb_tool::cons_electinfo_by_height(uint64_t height, bool print) const {
    ...
    auto vblock = blockstore_->load_block_object((std::string)tcash::sys_contract_zec_elect_consensus_addr, height, 0, false);
    if (vblock == nullptr) {
    ...
    xunit_bstate_ptr_t unitstate = tcash::statestore::xstatestore_hub_t::instance()->get_unit_state_by_unit_block(vblock.get());
    ...
    for (auto i = 0; i < CONS_AUDITOR_COUNT; ++i) {
        uint8_t auditor_group_id = tcash::common::xauditor_group_id_value_begin + i;
        outstr << " auditor_" << std::dec << (uint16_t)auditor_group_id << ":";

        std::string property_name = std::string(tcash::data::XPROPERTY_CONTRACT_ELECTION_RESULT_KEY) + "_" + std::to_string(auditor_group_id);
        ...
        auto const & election_result_store = tcash::codec::msgpack_decode<xelection_result_store_t>({std::begin(result), std::end(result)});

        // auditor
        auto & auditor_current_group_nodes = election_result_store.result_of(tcash::common::xnetwork_id_t{NETWORK_ID})
                                         .result_of(tcash::common::xnode_type_t::consensus_auditor)
                                         .result_of(tcash::common::xdefault_cluster_id)
                                         .result_of(tcash::common::xgroup_id_t{auditor_group_id});

        for (auto const& node_by_slotid: auditor_current_group_nodes.results()) {
            outstr << " " << node_by_slotid.second.node_id().value();
            tcash::common::xip2_t xip{
                tcash::common::xnetwork_id_t{NETWORK_ID},
                tcash::common::xconsensus_zone_id,
                tcash::common::xdefault_cluster_id,
                tcash::common::xgroup_id_t{auditor_group_id},
        ...
        for (auto count = 0; count < CONS_VALIDATOR_COUNT; ++count) {
            ...
            if (auditor_group_id == 1) {
                validator_group_id = tcash::common::xvalidator_group_id_value_begin;
            } else if (auditor_group_id == 2) {
                validator_group_id = tcash::common::xvalidator_group_id_value_begin + 2;
            }

            validator_group_id += count;

            outstr << " validator_" << std::dec << (uint16_t)validator_group_id << ":";
            auto & validator_current_group_nodes = election_result_store.result_of(tcash::common::xnetwork_id_t{NETWORK_ID})
                                                .result_of(tcash::common::xnode_type_t::consensus_validator)
                                                .result_of(tcash::common::xdefault_cluster_id)
                                                .result_of(tcash::common::xgroup_id_t{validator_group_id});
            ...
            for (auto const& node_by_slotid: validator_current_group_nodes.results()) {
                ...
                tcash::common::xip2_t xip{
                    ...
                    tcash::common::xconsensus_zone_id,
                    tcash::common::xdefault_cluster_id,
                    tcash::common::xgroup_id_t{validator_group_id},
                    node_by_slotid.first,
...
void xdb_tool::all_cons_electinfo() const {
    ...
    std::fstream file_out(filename, std::ios_base::out | std::ios_base::trunc );
    auto block_height = get_blockheight(tcash::sys_contract_zec_elect_consensus_addr);
...
std::string xdb_tool::get_multisig_votestr(tcash::auth::xmutisigdata_t const& aggregated_sig_obj) const {
    std::string res;
    tcash::auth::xnodebitset& nodebits = aggregated_sig_obj.get_nodebitset();
...
void xdb_tool::credit_data(uint64_t table_id) {
    if (table_id != 0) { // specific table by table id
        std::string specific_tableblock_addr =  tcash::sys_contract_sharding_table_block_addr;
...
void xdb_tool::specific_clockheight(uint64_t start_gmttime, uint64_t end_gmttime) {
    ...
    for (auto i = 0; i < enum_vbucket_has_tables_count; ++i) {
        ...
        std::fstream file_out(filename, std::ios_base::out | std::ios_base::trunc );
        for (uint64_t i = 1; i <= block_height; ++i) {
            auto vblock = blockstore_->load_block_object((std::string)tcash::sys_contract_zec_elect_consensus_addr, i, 0, false);
            xblock_t* block = dynamic_cast<xblock_t*>(vblock.get());
            std::stringstream outstr;
...
tcash::data::xstatistics_data_t xdb_tool::get_fulltable_statistic(std::string const& tableblock_addr, uint64_t height) {
        ...
        tcash::base::xstream_t base_stream{tcash::base::xcontext_t::instance()};
