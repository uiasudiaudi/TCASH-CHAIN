...
public:
    xtcash_hash_t()
      : tcash::base::xhashplugin_t(-1)  //-1 = support every hash types
    {
    }
...
public:
    ...
    virtual const std::string hash(const std::string & input, enum_xhash_type type) override {
        ...
        auto hash = tcash::utl::xsha2_256_t::digest(input);
...
int main(int argc, char** argv) {
    ...
    tcash::config::config_register.get_instance().set(tcash::config::xchain_name_configuration_t::name, std::string{tcash::config::chain_name_mainnet});
    tcash::data::xrootblock_para_t para;
    tcash::data::xrootblock_t::init(para);
