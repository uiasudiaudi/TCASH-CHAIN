...
using namespace tcash::db_export;

#define XDB_EXPORT_LOG

class xtcash_hash_t : public tcash::base::xhashplugin_t {
public:
    xtcash_hash_t()
...
private:
    xtcash_hash_t(const xtcash_hash_t &) = delete;
    xtcash_hash_t & operator=(const xtcash_hash_t &) = delete;

public:
    virtual ~xtcash_hash_t(){};
...
int main(int argc, char ** argv) {
    /*auto hash_plugin = */new xtcash_hash_t();
    tcash::config::config_register.get_instance().set(config::xmin_free_gas_asset_onchain_goverance_parameter_t::name, std::to_string(ASSET_tcash(100)));
    tcash::config::config_register.get_instance().set(config::xfree_gas_onchain_goverance_parameter_t::name, std::to_string(25000));
    tcash::config::config_register.get_instance().set(config::xmax_validator_stake_onchain_goverance_parameter_t::name, std::to_string(5000));
    tcash::config::config_register.get_instance().set(config::xchain_name_configuration_t::name, std::string{tcash::config::chain_name_testnet});
    tcash::config::config_register.get_instance().set(config::xroot_hash_configuration_t::name, std::string{});
    data::xrootblock_para_t para;
    data::xrootblock_t::init(para);
    tcash::utl::xkeyaddress_t pubkey1("");  // just init xsecp256k1_t
    ...
    if (function_name == "check_fast_sync" || function_name == "check_state_data") {
        ...
        tcash::base::xstring_utl::split_string(table_query, ',', table_info);
        std::map<common::xtable_address_t, uint64_t> table_query_criteria;
        for (auto const & t : table_info) {
            std::vector<std::string> pair;
            tcash::base::xstring_utl::split_string(t, ':', pair);
        ...
        for (auto const & genesis_account_datum : genesis_data) {
            genesis_accounts_data.emplace(common::xaccount_address_t::build_from(tcash::get<std::string const>(genesis_account_datum)), tcash::get<uint64_t>(genesis_account_datum));
        }
...
#pragma omp parallel for
        for (uint i = 0; i < table_query_criteria.size(); ++i) {
            ...
            auto const & table_address = tcash::get<common::xtable_address_t const>(tbl);
            auto const table_height = tcash::get<uint64_t>(tbl);
            ...
            auto const result = tools.get_account_data(units,
                                                       ...
                                                       {common::xtoken_id_t::tcash, common::xtoken_id_t::eth, common::xtoken_id_t::usdt, common::xtoken_id_t::usdc},
        ...
        for (auto const & genesis_account_datum : genesis_data) {
            genesis_accounts.insert(common::xaccount_address_t::build_from(tcash::get<std::string const>(genesis_account_datum)));
        }
