...
xdb_read_tools_t::xdb_read_tools_t(std::string const & db_path) {
    ...
    dst_db_kind |= tcash::db::xdb_kind_readonly;
...
{
    tcash::sync::xcommon_span_t span(height);
...
void xdb_read_tools_t::db_read_txindex(std::string const & hex_txhash, base::enum_txindex_type txindex_type) {
    std::string txhash = tcash::to_string(tcash::from_hex(hex_txhash));
...
void xdb_read_tools_t::db_data_parse() {
    tcash::store::xstore * pxstore = dynamic_cast< tcash::store::xstore*>(m_xvdb_ptr);
