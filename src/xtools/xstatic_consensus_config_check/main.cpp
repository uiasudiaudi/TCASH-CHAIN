...
void check_str(std::string const & name, std::string const & str, int group_num = -1) {
    ...
    tcash::base::xstring_utl::split_string(str, ',', group_nodes_infos_str);
    std::cout << name;
    if (group_num != -1) {
        std::cout << " " << group_num;
    }
    ...
    for (auto _node_info : group_nodes_infos_str) {
        ...
        tcash::base::xstring_utl::split_string(_node_info, '.', one_node_info);
...
void check_auditor_validator_str(std::string const & expect_name, std::string const & str, int group_num) {
    ...
    tcash::base::xstring_utl::split_string(str, ':', group_type_and_nodes_infos);
...
void check_consensus_str(std::string const & str) {
    ...
    tcash::base::xstring_utl::split_string(str, '|', group_string_value);
...
void check_standby_str(std::string const & str){
        ...
        tcash::base::xstring_utl::split_string(str, ',', node_standby_infos);
...
int main(int argc, char * argv[]) {
    ...
    auto & config_center = tcash::config::xconfig_register_t::get_instance();
