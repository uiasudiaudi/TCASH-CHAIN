...
class xrpc_eth_parser_t {
 public:
    ...
    static  void log_to_json_for_tcash_rpc(uint32_t log_idx, evm_common::xevm_log_t const& log, Json::Value & js_v);
    static  void receipt_to_json_for_tcash_rpc(const std::string & tx_hash, xtxindex_detail_ptr_t const& sendindex, Json::Value & js_v, std::error_code & ec);
    static  void receipt_to_json_for_tcash_rpc(std::string const & blockhash, uint32_t tx_idx,  data::xeth_transaction_t const& ethtx,
                                             data::xeth_store_receipt_t const &evm_tx_receipt,Json::Value & js_v, std::error_code & ec);
