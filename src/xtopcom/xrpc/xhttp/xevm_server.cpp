...
using namespace tcash::xChainRPC;
NS_BEG2(tcash, xrpc)
...
xevm_server::xevm_server(shared_ptr<xrpc_edge_vhost> edge_vhost,
    ...
    if (m_rpc_service == nullptr) {
        m_rpc_service = tcash::make_unique<xevm_rpc_service<xedge_evm_http_method>>(edge_vhost, xip2, archive_flag, block_store, txstore, elect_main, election_cache_data_accessor);
    } else {
...
void xevm_server::start(uint16_t nPort, uint32_t nThreadNum) {
    ...
    if (m_enable_ratelimit) {
        ...
        m_ratelimit = tcash::make_unique<RatelimitServer>(m_config);
        auto self = shared_from_this();
