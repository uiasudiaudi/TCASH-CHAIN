...
xhttp_server::xhttp_server(shared_ptr<xrpc_edge_vhost> edge_vhost,
    ...
    if (m_rpc_service == nullptr) {
        m_rpc_service = tcash::make_unique<xrpc_service<xedge_http_method>>(edge_vhost, xip2, archive_flag, block_store, txstore, elect_main, election_cache_data_accessor);
    } else {
