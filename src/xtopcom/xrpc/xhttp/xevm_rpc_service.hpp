...
xevm_rpc_service<T>::xevm_rpc_service(shared_ptr<xrpc_edge_vhost> edge_vhost,
    ...
    m_edge_method_mgr_ptr = tcash::make_unique<T>(edge_vhost, xip2, m_io_service, archive_flag, block_store, txstore, elect_main, election_cache_data_accessor);
    m_pre_request_handler_mgr_ptr = tcash::make_unique<xpre_request_handler_mgr>();
...
void xevm_rpc_service<T>::execute(shared_ptr<conn_type> & conn, const std::string & content, const std::string & ip) {
    ...
    try {
            ...
            Json::Reader reader;
            tcash::xrpc::xjson_proc_t json_proc;
            // reader.
