...
#define REGISTER_QUERY_METHOD(func_name)                                                                                                                                           \
    m_query_method_map.emplace(std::pair<std::string, tcash::xrpc::query_method_handler>{std::string{#func_name},                                                                    \
...
public:
    xtx_exec_json_key(const std::string & rpc_version) {
        if (rpc_version == tcash::data::RPC_VERSION_V2) {
...
public:
    ...
    //void query_account_property_base(Json::Value & jph, const std::string & owner, const std::string & prop_name, tcash::data::xunitstate_ptr_t unitstate, bool compatible_mode);
    ...
    Json::Value parse_tx(tcash::data::xtransaction_t * tx_ptr, const std::string & version);
    ...
    Json::Value parse_action(const tcash::data::xaction_t & action, data::xtransaction_t const * tx);
...
private:
    ...
    std::unordered_map<std::string, tcash::xrpc::query_method_handler> m_query_method_map;
    std::shared_ptr<tcash::vnetwork::xvnetwork_driver_face_t> m_shard_host;
