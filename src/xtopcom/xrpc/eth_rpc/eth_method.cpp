...
void EthMethod::web3_sha3(const Json::Value & js_req, Json::Value & js_rsp) {
    ...
    input = tcash::HexDecode(input.substr(2));
    const tcash::uint256_t hash_value = tcash::utl::xkeccak256_t::digest(input.data(), input.size());
    std::string hash_str = std::string((char *)hash_value.data(), (std::string::size_type)hash_value.size());
    js_rsp["result"] = std::string("0x") + tcash::HexEncode(hash_str);
...
void EthMethod::eth_gasPrice(const Json::Value& js_req, Json::Value& js_rsp)
{
    ...
    js_rsp["result"] = tcash::to_hex_prefixed_shrink_0((tcash::evm_common::h256)base_price);
