...
protected:
    ...
    tcash::observer_ptr<base::xvtxstore_t> m_txstore;
...
xedge_evm_method_base<T>::xedge_evm_method_base(shared_ptr<xrpc_edge_vhost> edge_vhost,
                                        ...
                                        observer_ptr<elect::ElectMain> elect_main,
                                        observer_ptr<tcash::election::cache::xdata_accessor_face_t> const & election_cache_data_accessor)
  : m_edge_local_method_ptr(tcash::make_unique<xedge_local_method<T>>(elect_main, xip2))
...
{
    m_edge_handler_ptr = tcash::make_unique<T>(edge_vhost, ioc, election_cache_data_accessor);
