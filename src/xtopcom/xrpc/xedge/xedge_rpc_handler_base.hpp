...
protected:
    ...
    observer_ptr<tcash::election::cache::xdata_accessor_face_t>    m_election_cache_data_accessor;
};
...
void xedge_handler_base<T>::edge_send_msg(const std::vector<std::shared_ptr<xrpc_msg_request_t>>& edge_msg_list, const std::string &tx_hash, const std::string &account, const common::xenum_message_id& msg_type)
{
    for (auto msg_ptr : edge_msg_list) {
        try {
            ...
            xwarn("[xrpc_edge_vhost] xtcash_error_t exception caught: cateogry:%s; msg:%s; error code:%d; error msg:%s", eh.code().category().name(), eh.what(), eh.code().value(), eh.code().message().c_str());
