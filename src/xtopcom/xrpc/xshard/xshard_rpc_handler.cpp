...
void xshard_rpc_handler::process_msg(const xrpc_msg_request_t & edge_msg) {
    ...
    // xtcash_scope_executer on_exit([&edge_msg, &json_proc] {
    ...
    case enum_xrpc_tx_type::enum_xrpc_tx_type: {
        ...
        /*consensus_service::xunit_service_face* unit_src =
            static_cast<consensus_service::xunit_service_face*>(xcontext_t::instance().get_global_object(enum_xtcash_global_object_unit_service));
...
void xshard_rpc_handler::shard_process_request(const xrpc_msg_request_t & edge_msg, const xvnode_address_t & edge_sender, const uint64_t msghash) {
    ...
    // xtcash_scope_executer on_exit([response_msg_ptr, &msghash, &edge_sender, &send_to_edge, this] {
    //     if (send_to_edge) {
...
void xshard_rpc_handler::stcash() {
