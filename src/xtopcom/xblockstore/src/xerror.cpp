...
class xtcash_blockstore_category : public std::error_category {
public:
...
using xblockstore_category_t = xtcash_blockstore_category;
...
size_t hash<tcash::store::error::xerrc_t>::operator()(tcash::store::error::xerrc_t errc) const noexcept {
    return static_cast<size_t>(static_cast<std::underlying_type<tcash::store::error::xerrc_t>::type>(errc));
}
