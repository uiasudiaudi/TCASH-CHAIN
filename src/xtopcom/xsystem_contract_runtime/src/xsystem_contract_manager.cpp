...
xtcash_contract_deployment_data::xtcash_contract_deployment_data(common::xnode_type_t node_type,
...
//void xtcash_system_contract_manager::deploy(observer_ptr<base::xvblockstore_t> const & blockstore) {
//    // rec registration
...
bool xtcash_system_contract_manager::contains(common::xaccount_address_t const & address) const noexcept {
    return m_system_contract_deployment_data.find(address) != std::end(m_system_contract_deployment_data);
}

std::unique_ptr<contract_common::xbasic_contract_t> xtcash_system_contract_manager::system_contract(common::xaccount_address_t const & address, std::error_code & ec) const noexcept {
    assert(!ec);
    ...
    if (it != std::end(m_system_contract_creators)) {
        auto & creator = tcash::get<std::unique_ptr<xcontract_object_creator_t>>(*it);
...
std::unique_ptr<contract_common::xbasic_contract_t> xtcash_system_contract_manager::system_contract(common::xaccount_address_t const & address) const {
    std::error_code ec;
...
std::unordered_map<common::xaccount_address_t, xcontract_deployment_data_t> const & xtcash_system_contract_manager::deployment_data() const noexcept {
    return m_system_contract_deployment_data;
}

void xtcash_system_contract_manager::init_system_contract(common::xaccount_address_t const & contract_address, observer_ptr<base::xvblockstore_t> const & blockstore) {
    if (blockstore->exist_unit(contract_address.vaccount())) {
        xwarn("xtcash_system_contract_manager::init_contract_chain contract account %s genesis block exist", contract_address.to_string().c_str());
        return;
    }
    xwarn("xtcash_system_contract_manager::init_contract_chain contract account %s genesis block not exist", contract_address.to_string().c_str());
    ...
    state_accessor::xstate_accessor_t sa{tcash::make_observer(bstate.get()), state_accessor::xstate_access_control_data_t{}};
    auto contract_state =
        tcash::make_unique<contract_common::xcontract_state_t>(contract_address, tcash::make_observer(std::addressof(sa)), contract_common::xcontract_execution_param_t{});
    ...
    auto contract_ctx= tcash::make_unique<contract_common::xcontract_execution_context_t>(std::move(action), make_observer(contract_state.get())); // action will be moved into xcontract_execution_context_t.
    ...
    xtransaction_execution_result_t result = contract_obj->execute(tcash::make_observer(contract_ctx));
    ...
    if (!ret) {
        xerror("xtcash_system_contract_manager::init_contract_chain %s genesis block fail", contract_address.to_string().c_str());
        return;
    }
    xwarn("xtcash_system_contract_manager::init_contract_chain contract_adress: %s, %s", contract_address.to_string().c_str(), ret ? "SUCC" : "FAIL");
}
