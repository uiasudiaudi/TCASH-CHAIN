...
auto do_call(Callable && func, Obj && obj, Tuple && tuple, tcash::index_sequence<I...>) -> decltype(func(std::forward<Obj>(obj), std::get<I>(std::forward<Tuple>(tuple))...)) {
...
auto call(Callable && func, T && obj, Tuple && tuple) -> decltype(do_call(std::forward<Callable>(func),
                                                                          ...
                                                                          tcash::make_index_sequence<std::tuple_size<typename std::remove_reference<Tuple>::type>::value>{})) {
...
void call_contract_api(ContractT * obj, tcash::base::xstream_t & stream, Callable && func, void (ContractT::*)(Args...)) {
    ...
    do {                                                                                                                                                                           \
        ...
        try {                                                                                                                                                                      \
            if (data::xconsensus_action_stage_t::recv != exe_ctx->consensus_action_stage()) {                                                                                      \
                result.status.ec = tcash::contract_runtime::error::xerrc_t::enum_vm_not_correct_action_stage_error;                                                                  \
                assert(false);                                                                                                                                                     \
                return result;                                                                                                                                                     \
            }                                                                                                                                                                      \
            ...
            tcash::contract_runtime::system::call_contract_api(this, stream, std::mem_fn(&CONTRACT_API), &CONTRACT_API);                                                             \
            ...
            result.status.ec = tcash::contract_runtime::error::xerrc_t::unknown_error;                                                                                               \
            result.status.extra_msg = eh.what();                                                                                                                                   \
        } catch (...) {                                                                                                                                                            \
            result.status.ec = tcash::contract_runtime::error::xerrc_t::unknown_error;                                                                                               \
        }                                                                                                                                                                          \
...
#define BEGIN_CONTRACT_API()                                                                                                                                                        \
    tcash::contract_common::xcontract_execution_result_t execute(observer_ptr<tcash::contract_common::xcontract_execution_context_t> exe_ctx) override {                                \
        this->reset_execution_context(exe_ctx);                                                                                                                                     \
        tcash::contract_common::xcontract_execution_result_t result;                                                                                                                  \
        ...
        if (action_data.empty()) {                                                                                                                                                  \
            stream_ptr = tcash::make_unique<base::xstream_t>(base::xcontext_t::instance());                                                                                           \
        } else {                                                                                                                                                                    \
            stream_ptr = tcash::make_unique<base::xstream_t>(base::xcontext_t::instance(), const_cast<uint8_t *>(action_data.data()), static_cast<uint32_t>(action_data.size()));     \
        }                                                                                                                                                                           \
...
#define END_CONTRACT_API                                                                                                                                                            \
        assert(false);                                                                                                                                                              \
        tcash::error::throw_error(tcash::contract_runtime::error::xerrc_t::contract_api_not_found);                                                                                     \
        return {};                                                                                                                                                                  \
    }
...
#define XCONTRACT_ENSURE(condition, msg)                                                                                                                                            \
    do {                                                                                                                                                                           \
        if (!(condition)) {                                                                                                                                                        \
            ...
            tcash::error::throw_error(ec, msg);                                                                                                                                      \
        }                                                                                                                                                                          \
    } while (false)
