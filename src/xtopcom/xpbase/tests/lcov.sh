...
do
    ...
    lcov_capture_path="${parent_path}/cbuild/src/xtcashcom/${xmodule}/CMakeFiles/${xmodule}.dir/src/"
    
    lcov -d $lcov_capture_path -o ${binname}.info1  -c
    
    lcov --remove ${binname}.info1 \
        '/usr/*' \
        $parent_path'/src/xtcashcom/xdepends/*' \
        $parent_path'/src/xtcashcom/xbasic/*' \
        $parent_path'/src/xtcashcom/xcommon/*' \
        $parent_path'/src/xtcashcom/xcrypto/*' \
        $parent_path'/src/xtcashcom/xdata/*' \
        $parent_path'/src/xtcashcom/xnetwork/*' \
        $parent_path'/src/xtcashcom/xkad/proto'$distrib'/*' \
        -o ${binname}.info
