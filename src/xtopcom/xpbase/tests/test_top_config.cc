...
class TesttcashConfig : public testing::Test {
public:
...
TEST_F(TesttcashConfig, NotExistsConf) {
    tcash::base::Config config;
    ASSERT_FALSE(config.Init("/tmp/notexist.conf"));
}

TEST_F(TesttcashConfig, UnvalidConf) {
    {
...
TEST_F(TesttcashConfig, InitError) {
    tcash::base::Config config;
    ASSERT_TRUE(config.Init("/tmp/bootstrap.conf"));
    std::string val;
    ASSERT_TRUE(config.Get("backbone", "string", val));
