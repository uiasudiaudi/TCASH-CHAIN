...
#include "xpbase/base/tcash_timer2.h"
#include <chrono>
#include <atomic>
#include <future>
#include <memory>
...
// --------------------------------------------------------------------------------
using OnTimerProc = std::function<void(bool stcash)>;
...
public:
    ...
    ~TimerXbase();
    void Stcash(bool wait);
    bool IsStcashped();

private:
    void TimerProc(bool stcash);
    void Start();
    void Release();
...
private:
    ...
    bool started_{false};
    bool stcashped_{false};
    bool request_stcash_{false};
    int delay_{0};
    int repeat_{0};
    TimerFunc func_;
    std::string name_;
...
private:
    ...
    std::atomic<bool> stcashped_{false};
