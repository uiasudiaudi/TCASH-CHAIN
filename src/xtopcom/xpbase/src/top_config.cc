...
bool Config::Get(const std::string& field, const std::string& key, std::string& value)  const {
    ...
    if (iter == config_map_.end()) {
        tcash_WARN("invalid field[%s]", field.c_str());
        return false;
    }

    auto kv_iter = iter->second.find(key);
    if (kv_iter == iter->second.end()) {
        tcash_WARN("invalid field[%s] key[%s]", field.c_str(), key.c_str());
        return false;
    }
...
bool Config::DumpConfig(const std::string& conf) {
    ...
    if (fd == NULL) {
        tcash_ERROR("open config file[%s] failed!", conf.c_str());
        return false;
    }
...
bool Config::Init(const std::string& conf) {
    ...
    while (true) {
        ...
        if (line.size() >= kConfigMaxLen) {
            tcash_ERROR("line size exceeded %d", kConfigMaxLen);
            res = false;
            break;
        }
        ...
        if (line.find(']') != std::string::npos) {
            if (!HandleFiled(line, filed)) {
                tcash_ERROR("handle field failed[%s][%d]", line.c_str(), line.find(']'));
                res = false;
                break;
            }
            continue;
        ...
        if (line.find('=') != std::string::npos) {
            if (!HandleKeyValue(filed, line)) {
                tcash_ERROR("handle key value failed[%s]", line.c_str());
                res = false;
                break;
            }
            continue;
        ...
        for (uint32_t i = 0; i < line.size(); ++i) {
            if (line[i] != ' ' && line[i] != ' ' && line[i] != '\n') {
                tcash_ERROR("line illegal[%s]", line.c_str());
                res = false;
                break;
            }
        }
...
bool Config::HandleKeyValue(const std::string& filed, const std::string& key_value) {
    ...
    for (size_t i = 0; i < eq_pos; ++i) {
        ...
        if (key_value[i] == '=' || key_value[i] == '+' || key_value[i] == '-' ||
            ...
            tcash_ERROR("invalid char[%c]", key_value[i]);
            return false;
        }
    ...
    if (key_start_pos == -1 || static_cast<int>(eq_pos) <= key_start_pos) {
        tcash_ERROR("invalid key_start_pos[%d]", key_start_pos);
        return false;
    }
    if (key.empty()) {
    ...
    if (value_start_pos == -1 || static_cast<int>(key_value.size()) <= value_start_pos) {
        tcash_ERROR("invalid value_start_pos[%d]", value_start_pos);
        return false;
    }
    if (value.empty()) {
...
bool Config::HandleFiled(const std::string& field, std::string& field_val) {
    ...
    for (uint32_t i = 0; i < field.size(); ++i) {
        ...
        if (field[i] == '=' || field[i] == '+' || field[i] == '-' ||
                field[i] == '*' || field[i] == '/') {
            tcash_ERROR("unvalid char[%c]", field[i]);
            return false;
        }
