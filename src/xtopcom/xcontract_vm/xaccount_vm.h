...
struct xtcash_account_vm_execution_assemble {
    std::vector<data::xcons_transaction_ptr_t> txs;
    xobject_ptr_t<base::xvbstate_t> block_state;
    data::xblock_consensus_para_t cs_para;
};
using xaccount_vm_execution_assemble_t = xtcash_account_vm_execution_assemble;

class xtcash_account_vm : public xvm_executor_face_t {
private:
    std::unique_ptr<contract_runtime::user::xuser_action_runtime_t> user_action_runtime_{tcash::make_unique<contract_runtime::user::xuser_action_runtime_t>()};
    std::unique_ptr<contract_runtime::system::xsystem_action_runtime_t> sys_action_runtime_;

public:
    // xtcash_account_vm() = default;
    xtcash_account_vm(xtcash_account_vm const &) = delete;
    xtcash_account_vm & operator=(xtcash_account_vm const &) = delete;
    xtcash_account_vm(xtcash_account_vm &&) = default;
    xtcash_account_vm & operator=(xtcash_account_vm &&) = default;
    ~xtcash_account_vm() override = default;

    explicit xtcash_account_vm(observer_ptr<contract_runtime::system::xsystem_contract_manager_t> const & system_contract_manager);
...
private:
    contract_runtime::xtransaction_execution_result_t execute_action(std::unique_ptr<data::xbasic_tcash_action_t const> action,
...
using xaccount_vm_t = xtcash_account_vm;
