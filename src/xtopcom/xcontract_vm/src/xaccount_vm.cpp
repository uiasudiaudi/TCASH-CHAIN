...
#include "xdata/xtcash_action_generator.h"
...
xtcash_account_vm::xtcash_account_vm(observer_ptr<contract_runtime::system::xsystem_contract_manager_t> const & system_contract_manager)
  : sys_action_runtime_{tcash::make_unique<contract_runtime::system::xsystem_action_runtime_t>(system_contract_manager)} {
}

xaccount_vm_output_t xtcash_account_vm::execute(std::vector<data::xcons_transaction_ptr_t> const & txs,
    ...
    xdbg("[xtcash_account_vm::execute] tx size: %" PRIu64, result_size);
    xdbg("[xtcash_account_vm::execute] param, clock: %" PRIu64 ", timestamp: %" PRIu64 ", table_account: %s, table_height: %" PRIu64 ", total_lock_tgas: %" PRIu64,
         param.clock,
         param.timestamp,
    ...
    try {
        for (i = 0; i < result_size; i++) {
            ...
            if (action_result.status.ec) {
                xwarn("[xtcash_account_vm::execute] tx[%" PRIu64 "] failed, category: %s, msg: %s, abort all txs after!",
                      i,
    ...
    } catch (tcash::error::xtcash_error_t & eh) {
    ...
    // exception
    if (i < result_size) {
        xwarn("[xtcash_account_vm::execute] tx[%" PRIu64 "] failed, abort all txs after!", i);
...
contract_runtime::xtransaction_execution_result_t xtcash_account_vm::execute_action(std::unique_ptr<data::xbasic_tcash_action_t const> action,
    ...
    case data::xtcash_action_type_t::event: {
        assert(false);
    ...
    default: {
        xerror("[xtcash_account_vm::execute_action] error action type: %" PRIi32, action->type());
    ...
    xdbg("[xtcash_account_vm::execute] followup_tx size: %" PRIu64, result.output.followup_transaction_data.size());
#if defined(DEBUG)
    ...
    for (size_t i = 0; i < result.output.followup_transaction_data.size(); i++) {
        ...
        xdbg("[xtcash_account_vm::execute] followup_tx[%zu] schedule_type: %" PRIi32 ", execute_type: %" PRIi32, i, followup_tx.schedule_type, followup_tx.execute_type);
        if (followup_tx.schedule_type == contract_common::xfollowup_transaction_schedule_type_t::immediately) {
            if (followup_tx.execute_type == contract_common::xenum_followup_transaction_execute_type::unexecuted) {
                ...
                if (followup_result.status.ec) {
                    if (followup_result.status.ec != make_error_code(contract_runtime::error::xerrc_t::account_state_not_changed)) {
                        xwarn("[xtcash_account_vm::execute] followup_tx[%zu] failed, category: %s, msg: %s, break!",
                              i,
                ...
                xerror("[xtcash_account_vm::execute_action] error followup_tx execute type: %" PRIi32, followup_tx.execute_type);
                assert(false);
            }
            ...
            xerror("[xtcash_account_vm::execute_action] error followup_tx schedule type: %" PRIi32, followup_tx.schedule_type);
            assert(false);
        }
    }
...
void xtcash_account_vm::preprocess(std::vector<data::xcons_transaction_ptr_t> const & txs, state_accessor::xstate_accessor_t & sa, xaccount_vm_execution_result_t & r) {
    ...
    if (recv_tx_num_new != 0) {
        ...
        auto recv_tx_num_bytes = sa.get_property_cell_value<state_accessor::properties::xproperty_type_t::map>(
            ...
            ec);
        tcash::error::throw_error(ec);
        auto recv_tx_num = recv_tx_num_bytes.empty() ? 0 : tcash::from_bytes<uint64_t>(recv_tx_num_bytes);
        sa.set_property_cell_value<state_accessor::properties::xproperty_type_t::map>(
            state_accessor::properties::xtypeless_property_identifier_t{data::XPROPERTY_TX_INFO, state_accessor::properties::xproperty_category_t::system},
            data::XPROPERTY_TX_INFO_RECVTX_NUM,
            tcash::to_bytes<std::string>(tcash::to_string(recv_tx_num_new + recv_tx_num)),
            ec);
        tcash::error::throw_error(ec);
        xinfo("[xtcash_account_vm::preprocess] recv_tx_num add: %" PRIu64 " + %" PRIu64 " = %" PRIu64, recv_tx_num, recv_tx_num_new, recv_tx_num_new + recv_tx_num);
...
void xtcash_account_vm::abort(const size_t start_index, const size_t size, xaccount_vm_execution_result_t & result) {
...
xaccount_vm_output_t xtcash_account_vm::pack(std::vector<data::xcons_transaction_ptr_t> const & txs,
    ...
    tcash::error::throw_error(ec);
    auto last_nonce = last_nonce_bytes.empty() ? 0 : tcash::from_string<uint64_t>(tcash::from_bytes<std::string>(last_nonce_bytes));
    ...
    tcash::error::throw_error(ec);
    auto last_hash = last_hash_bytes.empty() ? uint256_t{} : tcash::from_bytes<uint256_t>(last_hash_bytes);
    xinfo("[xtcash_account_vm::pack] pack last_nonce: %" PRIu64, last_nonce);
    ...
    for (size_t i = 0; i < result.transaction_results.size(); i++) {
        ...
        if (r.status.ec) {
            xwarn("[xtcash_account_vm::pack] tx[%" PRIu64 "] failed, category: %s, msg: %s", i, r.status.ec.category().name(), r.status.ec.message().c_str());
            tx->set_current_exec_status(data::enum_xunit_tx_exec_status::enum_xunit_tx_exec_status_fail);
            if (tx->is_send_tx() || tx->is_self_tx()) {
                xwarn("[xtcash_account_vm::pack] fail send/self tx[%" PRIu64 "] add to failed assemble", i);
                output.failed_tx_assemble.emplace_back(tx);
            } else if (tx->is_recv_tx()) {
                xwarn("[xtcash_account_vm::pack] fail recv tx[%" PRIu64 "] add to success assemble", i);
                output.success_tx_assemble.emplace_back(tx);
            } else {
                xerror("[xtcash_account_vm::pack] invalid tx type: %" PRIi32, tx->get_tx_type());
                assert(false);
            }
        } else {
            ...
            xdbg("[xtcash_account_vm::pack] tx[%" PRIu64 "] add to success assemble", i);
            output.success_tx_assemble.emplace_back(tx);
            for (auto & follow_up : r.output.followup_transaction_data) {
                ...
                if (follow_up.schedule_type == contract_common::xfollowup_transaction_schedule_type_t::immediately) {
                    ...
                    xdbg("[xtcash_account_vm::pack] immediately followup_tx of tx[%" PRIu64 "] add to success assemble", i);
                    ...
                    xdbg("[xtcash_account_vm::pack] delay followup_tx of tx[%" PRIu64 "] add to delay assemble, set last nonce: %" PRIu64, i, last_nonce);
                    ...
                    xerror("[xtcash_account_vm::pack] invalid follow_up tx schedule type: %" PRIi32, follow_up.schedule_type);
                    assert(false);
                }
            }
        }
