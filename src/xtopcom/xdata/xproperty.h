...
XINLINE_CONSTEXPR char const * XPROPERTY_ASSET_tcash                       = "tcash";
...
public:
    ...
    bool is_tcash_token() const {return m_token_name.empty() || XPROPERTY_ASSET_tcash == m_token_name;};
    const std::string & token_symbol() const {return m_token_name;}
    uint64_t amount() const {return m_amount;}
    std::string m_token_name{XPROPERTY_ASSET_tcash};
    uint64_t    m_amount{0};
};
