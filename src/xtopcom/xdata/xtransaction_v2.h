...
class xtcash_target_address {
    common::xaccount_address_t original_;
    common::xaccount_address_t adjusted_;

public:
    xtcash_target_address() = default;
    xtcash_target_address(xtcash_target_address const &) = default;
    xtcash_target_address & operator=(xtcash_target_address const &) = default;
    xtcash_target_address(xtcash_target_address &&) = default;
    xtcash_target_address & operator=(xtcash_target_address &&) = default;
    ~xtcash_target_address() = default;

private:
    explicit xtcash_target_address(common::xaccount_address_t original) noexcept;
    explicit xtcash_target_address(common::xaccount_address_t original, common::xtable_id_t optional_table_id);

public:
    static xtcash_target_address build_from(common::xaccount_address_t const & src, common::xaccount_address_t dst);
...
using xtarget_address_t = details::xtcash_target_address;
...
class xtransaction_v2_t : public xbase_dataunit_t<xtransaction_v2_t, xdata_type_transaction_v2>, public xtransaction_t, public xstatistic::xstatistic_obj_face_t {
 ...
 public:
    ...
    virtual tcash::evm_common::u256 get_amount_256() const noexcept override { return 0; }
    virtual bool is_tcash_transfer() const noexcept override { return m_transaction_type == xtransaction_type_transfer && (m_token_name.empty() || XPROPERTY_ASSET_tcash == m_token_name); }
