...
void xeth_header_t::decodeRLP(evm_common::RLP const& _r, std::error_code & ec) {
    ...
    try
    {
        ...
        if (!coinbase_r.isEmpty()) {
            ...
            if (ec) {
                xerror("xeth_header_t::decodeRLP fail miner,%s", tcash::to_hex(_r[field].toString()).c_str());
                return;
            }
        }
        ...
        if (!logbloom_r.isEmpty()) {
            ...
            if (ec) {
                xerror("xeth_header_t::decodeRLP fail bloom,%s", tcash::to_hex(_r[field].toString()).c_str());
                return;
            }
        }
        ...
        if (!txroot_r.isEmpty()) {
            ...
            if (_bytes.size() != 32) {
                ...
                xerror("xeth_header_t::decodeRLP fail tx root,%s", tcash::to_hex(_r[field].toString()).c_str());
                return;
            }
        ...
        if (!receiptsroot_r.isEmpty()) {
            ...
            if (_bytes.size() != 32) {
                ec = common::error::xerrc_t::invalid_eth_header;
                xerror("xeth_header_t::decodeRLP fail receipts root,%s", tcash::to_hex(_r[field].toString()).c_str());
                return;
            }
        ...
        if (!stateroot_r.isEmpty()) {
            ...
            if (_bytes.size() != 32) {
                ec = common::error::xerrc_t::invalid_eth_header;
                xerror("xeth_header_t::decodeRLP fail state root,%s", tcash::to_hex(_r[field].toString()).c_str());
                return;
            }
    ...
    {
        xwarn("xeth_header_t::decodeRLP invalid,field=%d,%s", field, tcash::to_hex(_r[field].toString()).c_str());
        ec = common::error::xerrc_t::invalid_rlp_stream;
    }
}
...
std::string xeth_header_t::dump() const {
    ...
    xprintf(local_param_buf,sizeof(local_param_buf),"{%d,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%s,%s,%zu}",
        ...
        m_gaslimit,
        m_gasused,
        base::xhash64_t::digest(tcash::to_string(m_transactions_root.asBytes())),
        base::xhash64_t::digest(tcash::to_string(m_receipts_root.asBytes())),   
        base::xhash64_t::digest(tcash::to_string(m_state_root.asBytes())),   
        base::xhash64_t::digest(tcash::to_string(m_extra_data)),   
        base::xhash64_t::digest(m_logBloom.to_hex_string()),   
        m_coinbase.to_hex_string().c_str(),
        tcash::to_hex_shrink_0((tcash::evm_common::h256)m_baseprice).c_str(),
        encodeBytes().size());
