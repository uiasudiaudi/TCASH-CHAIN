...
int32_t ntp_sync(const std::vector<std::string>& ntp_server_list) {
    ...
    for (auto& ntp_host : ntp_server_list) {
...
#endif
        ntp_cmd_successed = tcash::base::xsys_utl::system_cmd(cmd.c_str());
#elif defined (__LINUX_PLATFORM__)
#ifdef DEBUG
        ...
        if (ntp_cmd_successed != 0) {
            ...
            xut_timev = 0ULL;
            xit_err = tcash::ntp_get_time(ntp_host.c_str(), NTP_PORT, 5000, &xut_timev);
            if (0 == xit_err) {
                // successfully synced with current ntp server
                tcash::bv_output(ntp_host.c_str(), xut_timev);
                if (tcash::ntp_settimeofday(xut_timev) == 0) {
...
int set_limits() {
    ...
    const int total_cpu_cores = tcash::base::xsys_utl::sys_cpu_cores();
    xkinfo("System total cpu cores:%d, and kernel:%s", total_cpu_cores, tcash::base::xsys_utl::kernel_version().c_str());
