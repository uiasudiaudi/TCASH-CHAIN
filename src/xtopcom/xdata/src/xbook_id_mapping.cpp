...
mapping_to_validator_group(std::uint16_t const book_id,
    ...
    auto const & advance_group_id = tcash::get<common::xgroup_id_t>(advance_group_info);
    auto const & new_book_range   = tcash::get<std::pair<std::uint16_t, std::uint16_t>>(advance_group_info);
