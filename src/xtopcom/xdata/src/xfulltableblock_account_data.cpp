...
int32_t xtcash_fulltableblock_account_data::do_read(base::xstream_t & stream) {
    auto const size = stream.size();
    stream >> account_data;
    return size - stream.size();
}

int32_t xtcash_fulltableblock_account_data::do_write(base::xstream_t & stream) const {
    auto const size = stream.size();
    stream << account_data;
...
int32_t xtcash_fulltableblock_group_data::do_read(base::xstream_t & stream) {
    auto const size = stream.size();
    stream >> group_data;
    return size - stream.size();
}

int32_t xtcash_fulltableblock_group_data::do_write(base::xstream_t & stream) const {
    auto const size = stream.size();
    stream << group_data;
    return stream.size() - size;
}
...
int32_t xtcash_fulltableblock_statistic_accounts::do_read(base::xstream_t & stream) {
    auto const size = stream.size();
    stream >> accounts_detail;
    return size - stream.size();
}

int32_t xtcash_fulltableblock_statistic_accounts::do_write(base::xstream_t & stream) const {
    auto const size = stream.size();
    stream << accounts_detail;
    return stream.size() - size;
}
