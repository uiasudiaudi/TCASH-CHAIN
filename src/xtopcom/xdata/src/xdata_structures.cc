...
bool xreg_node_info::has_enough_tickets() const noexcept {
    ...
    return true;
#else
    return m_vote_amount * tcash_UNIT >= deposit();
#endif
}
...
uint64_t xreg_node_info::rec_stake() const noexcept {
    uint64_t stake = 0;
    if (could_be_rec()) {
        stake = m_account_mortgage / tcash_UNIT + m_vote_amount / 2;
    }
    return stake;
}
...
xtcash_allowance::xtcash_allowance(data_type d) noexcept(std::is_nothrow_move_constructible<data_type>::value) : data_{std::move(d)} {
}

xtcash_allowance::iterator xtcash_allowance::begin() noexcept {
    return data_.begin();
}

xtcash_allowance::const_iterator xtcash_allowance::begin() const noexcept {
    return data_.begin();
}

xtcash_allowance::const_iterator xtcash_allowance::cbegin() const noexcept {
    return data_.cbegin();
}

xtcash_allowance::iterator xtcash_allowance::end() noexcept {
    return data_.end();
}

xtcash_allowance::const_iterator xtcash_allowance::end() const noexcept {
    return data_.end();
}

xtcash_allowance::const_iterator xtcash_allowance::cend() const noexcept {
    return data_.cend();
}

bool xtcash_allowance::empty() const noexcept {
    return data_.empty();
}

xtcash_allowance::size_type xtcash_allowance::size() const noexcept {
    return data_.size();
}

std::pair<xtcash_allowance::iterator, bool> xtcash_allowance::insert(value_type const & value) {
    return data_.insert(value);
}

xtcash_allowance::iterator xtcash_allowance::insert(const_iterator hint, const value_type & value) {
    return data_.insert(hint, value);
}

xtcash_allowance::size_type xtcash_allowance::count(key_type const & key) const {
    return data_.count(key);
}

xtcash_allowance::iterator xtcash_allowance::find(key_type const & key) {
    return data_.find(key);
}

xtcash_allowance::const_iterator xtcash_allowance::find(key_type const & key) const {
    return data_.find(key);
}

xtcash_allowance::data_type const & xtcash_allowance::raw_data() const noexcept {
    return data_;
}
