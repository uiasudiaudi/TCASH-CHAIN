...
void xeth_accesstuple_t::decodeRLP(evm_common::RLP const& _r, std::error_code & ec) {
    ...
    if (!_bytes.empty()) {
        ...
        if (ec) {
            xwarn("xxeth_accesstuple_t::decodeRLP fail to addr.%s", tcash::to_hex(_bytes).c_str());
            return;
        }
    }
    if (!_r[1].isList()) {
...
data::xeth_transaction_t  xeth_transaction_t::build_from(std::string const& rawtx_bin, eth_error & ec) {
    ...
    xbytes_t _rawtx_bs = tcash::from_hex(rawtx_bin, ec.error_code);
    if (ec.error_code) {
        return _tx;
    }
...
void xeth_transaction_t::decodeRLP_eip1599(bool includesig, evm_common::RLP const& _r, eth_error & ec) {
    ...
    {
        xwarn("xeth_transaction_t::decodeRLP_eip1599 invalid,field=%d,%s", field, tcash::to_hex(_r[field].toString()).c_str());
...
uint256_t xeth_transaction_t::get_tx_hash() const {
    if (m_transaction_hash.empty()) {
        ...
        std::string _str = tcash::to_string(_bytes);
        // std::cout << "tx_hash_bytes=" << tcash::to_hex(_str) << std::endl;
...
common::xeth_address_t xeth_transaction_t::get_from() const {
    if (m_from.is_zero()) {
        ...
        std::string _str = tcash::to_string(_bytes);
        // std::cout << "unsign_bytes=" << tcash::to_hex(_bytes) << std::endl;
        ...
        tcash::utl::xecdsasig_t sig((uint8_t *)szSign);
        uint8_t szOutput[65] = {0};
        tcash::utl::xsecp256k1_t::get_publickey_from_signature(sig, m_unsign_hash, szOutput);
        tcash::utl::xecpubkey_t pubkey(szOutput);
        std::string _addr = pubkey.to_raw_eth_address();
        xbytes_t addrbytes = tcash::to_bytes(_addr);
        std::error_code ec;
        common::xeth_address_t _from = common::xeth_address_t::build_from(addrbytes, ec);
        if (ec) {
            xwarn("xeth_transaction_t::get_from fail addr.%s,size=%zu", tcash::to_hex(addrbytes).c_str(), addrbytes.size());
        } else {
            m_from = _from;
        }
...
std::string xeth_transaction_t::dump() const
{
    ...
    xbytes_t txhash_bs = tcash::to_bytes(txhash);
    xprintf(local_param_buf, sizeof(local_param_buf),"{xeth_transaction_t:hash=%s,from=%s,to=%s}",
        tcash::to_hex_prefixed(txhash_bs).c_str(), get_from().to_hex_string().c_str(),  m_to.to_hex_string().c_str());
