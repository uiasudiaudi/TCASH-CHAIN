...
xcheckpoints_map_t xtcash_chain_checkpoint::m_checkpoints_map;

void xtcash_chain_checkpoint::load() {
    json j_data;
#ifdef CHECKPOINT_TEST
#    define CHECKPOINT_DATA_FILE "checkpoint_data.json"
    xinfo("[xtcash_chain_checkpoint::load] load from file");
    std::ifstream data_file(CHECKPOINT_DATA_FILE);
    if (data_file.good()) {
        data_file >> j_data;
        data_file.close();
    } else {
        xwarn("[xtcash_chain_checkpoint::load] file %s open error, none cp data used!", CHECKPOINT_DATA_FILE);
    }
#else
    xinfo("[xtcash_chain_checkpoint::load] load from code");
    ...
    auto load_data = [](json & j) -> xcheckpoints_map_t {
        ...
        for (auto it = j.cbegin(); it != j.cend(); it++) {
            ...
            xinfo("[xtcash_chain_checkpoint::load] load cp height: %lu", clock);
        }
        return m;
    };
    m_checkpoints_map = load_data(j_data);
    j_data.clear();
    xinfo("[xtcash_chain_checkpoint::load] cp data size: %zu", m_checkpoints_map.size());
}

xcheckpoint_data_t xtcash_chain_checkpoint::get_latest_checkpoint(common::xaccount_address_t const & account, std::error_code & ec) {
    auto it = m_checkpoints_map.find(account);
    if (it == m_checkpoints_map.end()) {
        xdbg("[xtcash_chain_checkpoint::get_latest_checkpoint] %s not found!", account.to_string().c_str());
...
xcheckpoints_t xtcash_chain_checkpoint::get_checkpoints(common::xaccount_address_t const & account, std::error_code & ec) {
    auto it = m_checkpoints_map.find(account);
    if (it == m_checkpoints_map.end()) {
        xwarn("[xtcash_chain_checkpoint::get_checkpoints] %s not found!", account.to_string().c_str());
        ec = error::xenum_errc::checkpoint_not_found;
        return {};
    }
    return it->second;
