...
common::xelection_epoch_t const & xtcash_election_group_result::group_epoch() const noexcept {
    return m_group_epoch;
}

common::xelection_epoch_t & xtcash_election_group_result::group_epoch() noexcept {
    return m_group_epoch;
}
void xtcash_election_group_result::group_epoch(common::xelection_epoch_t epoch) noexcept {
...
bool xtcash_election_group_result::empty_at(common::xslot_id_t const & slot_id) const noexcept {
    auto const & it = m_nodes.find(slot_id);
    if (it == std::end(m_nodes)) {
        return true;
    }

    return tcash::get<xelection_info_bundle_t>(*it).account_address().empty();
}
...
v1::xelection_group_result_t xtcash_election_group_result::v1() const {
