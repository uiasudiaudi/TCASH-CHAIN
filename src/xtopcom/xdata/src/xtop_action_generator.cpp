...
std::unique_ptr<data::xbasic_tcash_action_t const> xtcash_action_generator::generate(xobject_ptr_t<data::xcons_transaction_t> const & tx) {
    std::error_code ec;
...
std::unique_ptr<data::xbasic_tcash_action_t const> xtcash_action_generator::generate(xobject_ptr_t<data::xcons_transaction_t> const & tx, std::error_code & ec) {
    ...
    if (target_address.empty()) {
        ...
        return tcash::make_unique<data::xevm_consensus_action_t>(tx);
    } 
    ...
    case base::enum_vaccount_addr_type_secp256k1_eth_user_account:
        return tcash::make_unique<data::xsystem_consensus_action_t>(tx);
...
std::vector<std::unique_ptr<data::xbasic_tcash_action_t const>> xtcash_action_generator::generate(std::vector<xobject_ptr_t<data::xcons_transaction_t>> const & txs) {
    std::error_code ec;
...
std::vector<std::unique_ptr<data::xbasic_tcash_action_t const>> xtcash_action_generator::generate(std::vector<xobject_ptr_t<data::xcons_transaction_t>> const & txs,
                                                                                              std::error_code & ec) {
    std::vector<std::unique_ptr<data::xbasic_tcash_action_t const>> r;
    r.reserve(txs.size());
