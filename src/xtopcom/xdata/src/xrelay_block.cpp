...
using namespace tcash::evm_common;
...
void xrelay_election_group_t::dump() const
{
    ...
    for (auto election : elections_vector) {
        std::string public_key_x = tcash::HexDecode(election.public_key_x.hex());
        std::string public_key_y = tcash::HexDecode(election.public_key_y.hex());
        ...
        xdbg("xrelay_election_group_t::dump  address %s   public_key_x %s public_key_y %s  ",
            tcash::to_hex(address).c_str(), election.public_key_x.hex().c_str(), election.public_key_y.hex().c_str());
    }
}
...
xrelay_signature_t::xrelay_signature_t(const std::string& sign_str)
{
    ...
    r = tcash::evm_common::fromBigEndian<tcash::evm_common::u256>(r_bytes);
    xbytes_t s_bytes(compact_signature + 33, compact_signature + 65);
    s = tcash::evm_common::fromBigEndian<tcash::evm_common::u256>(s_bytes);
}
