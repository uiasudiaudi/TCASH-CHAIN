...
xtransaction_ptr_t xtx_factory::create_genesis_tx_with_balance(const std::string & account, int64_t tcash_balance) {
    // genesis tx should use v1 tx
    xtransaction_ptr_t tx = xtx_factory::create_tx(xtransaction_version_1);
    data::xproperty_asset asset(tcash_balance);
...
// XTODO xtcash_contract_manager::setup_chain use this api to create genesis tx
...
xtransaction_ptr_t xtx_factory::create_ethcall_v3_tx(const std::string & from, const std::string & to, const std::string & data, const tcash::evm_common::u256 & value, 
                                                     const tcash::evm_common::u256 & gas, evm_common::u256 const& _maxGasPrice, evm_common::u256 const& _maxPriorityFee) {
    std::error_code ec;
    common::xaccount_address_t _tcash_from_addr(from);
    common::xeth_address_t _eth_from_addr = common::xeth_address_t::build_from(_tcash_from_addr, ec);

    common::xeth_address_t _eth_to_addr;
    if (!to.empty()) {
        common::xaccount_address_t _tcash_to_addr(to);
        _eth_to_addr = common::xeth_address_t::build_from(_tcash_to_addr, ec);
    }

    xbytes_t _data_bs = tcash::to_bytes(data);
