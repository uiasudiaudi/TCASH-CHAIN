...
void  xblockdump_t::dump_action(std::string const& prefix_str, base::xvaction_t const& action) {
    ...
    << " txhash:" << tcash::to_hex_prefixed(action.get_org_tx_hash()) 
...
void  xblockdump_t::dump_output_entity(std::string const& prefix_str, base::xvoutentity_t* entity) {
    ...
    for (auto & v : entity->get_values()) {
        std::cout << prefix_str << prefix_str << "key:" << v.first << " value:" << tcash::to_hex_prefixed(v.second) << std::endl;
    }
}
...
void  xblockdump_t::dump_input_resources(base::xvblock_t* block) {
    ...
    for (auto & v : _map) {
        std::cout << "--" << "key:" << tcash::to_hex_prefixed(v.first) << " value_size:" << v.second.size() << std::endl;
    }
}
...
void  xblockdump_t::dump_unitblock(base::xvblock_t* block) {
    ...
    std::cout << "--binlog_hash:" << tcash::to_hex_prefixed(block->get_binlog_hash()) 
    << " state_hash:" << tcash::to_hex_prefixed(block->get_fullstate_hash()) << std::endl;
