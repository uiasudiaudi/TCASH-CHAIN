...
base::xvblock_t*   xblocktool_t::create_genesis_lightunit(const std::string & account, int64_t tcash_balance) {
    xtransaction_ptr_t tx = xtx_factory::create_genesis_tx_with_balance(account, tcash_balance);
    ...
    xobject_ptr_t<base::xvcanvas_t> canvas = make_object_ptr<base::xvcanvas_t>();
    {
        ...
        propobj->set(base::tcash_BEGIN_GMTIME, canvas.get());
    }
    if (tcash_balance > 0) {
        auto propobj = bstate->new_token_var(XPROPERTY_BALANCE_AVAILABLE, canvas.get());
        auto balance = propobj->deposit(base::vtoken_t(tcash_balance), canvas.get());
        xassert(balance == tcash_balance);
    }
...
base::xvblock_t * xblocktool_t::create_genesis_lightunit(std::string const & account, chain_data::data_processor_t const & data) {
    xtransaction_ptr_t tx = xtx_factory::create_genesis_tx_with_balance(account, data.tcash_balance);
    ...
    if (data.tcash_balance + data.lock_token + data.lock_balance > 0) {
        auto propobj = bstate->new_token_var(XPROPERTY_BALANCE_AVAILABLE, canvas.get());
        auto balance = propobj->deposit(base::vtoken_t(data.tcash_balance + data.lock_token + data.lock_balance), canvas.get());
        xassert(balance == data.tcash_balance + data.lock_token + data.lock_balance);
    }
...
std::vector<xlightunit_action_t> xblocktool_t::unpack_all_txreceipt_action(base::xvblock_t* commit_block) {
    ...
    xblockextract_t::loop_tcash_txactions(commit_block, f);
