...
std::string xtcash_block_data::to_json_string() const {
    return to_json_object<Json::Value>().asString();
}

int32_t xtcash_block_data::do_read(base::xstream_t & stream) {
...
int32_t xtcash_block_data::do_write(base::xstream_t & stream) const {
    auto const size = stream.size();
    stream << transaction_count;
    stream << block_count;
...
std::string xtcash_consensus_vote_data::to_json_string() const {
    return to_json_object<Json::Value>().asString();
}

int32_t xtcash_consensus_vote_data::do_read(base::xstream_t & stream) {
    auto const size = stream.size();
    stream >> block_count;
    stream >> vote_count;
    return size - stream.size();
...
int32_t xtcash_consensus_vote_data::do_write(base::xstream_t & stream) const {
    auto const size = stream.size();
    stream << block_count;
    stream << vote_count;
    return stream.size() - size;
...
std::string xtcash_account_related_statistics_data::to_json_string() const {
    return to_json_object<Json::Value>().asString();
}

int32_t xtcash_account_related_statistics_data::do_read(base::xstream_t & stream) {
    auto const size = stream.size();
    stream >> block_data;
    stream >> vote_data;
    return size - stream.size();
...
int32_t xtcash_account_related_statistics_data::do_write(base::xstream_t & stream) const {
    auto const size = stream.size();
    stream << block_data;
    stream << vote_data;
    return stream.size() - size;
...
std::string xtcash_group_releated_statistics_data::to_json_string() const {
    return to_json_object<Json::Value>().asString();
}

int32_t xtcash_group_releated_statistics_data::do_read(base::xstream_t & stream) {
    auto const size = stream.size();
    stream >> account_statistics_data;
    return size - stream.size();
}

int32_t xtcash_group_releated_statistics_data::do_write(base::xstream_t & stream) const {
    auto const size = stream.size();
    stream << account_statistics_data;
    return stream.size() - size;
}
...
std::string xtcash_election_related_statistics_data::to_json_string() const {
    return to_json_object<Json::Value>().asString();
}

int32_t xtcash_election_related_statistics_data::do_read(base::xstream_t & stream) {
    auto const size = stream.size();
    stream >> group_statistics_data;
    return size - stream.size();
}

int32_t xtcash_election_related_statistics_data::do_write(base::xstream_t & stream) const {
    auto const size = stream.size();
    stream << group_statistics_data;
    return stream.size() - size;
}
...
/// xstatistics_data_t
std::string xtcash_statistics_data::to_json_string() const {
    return to_json_object<Json::Value>().asString();
}

int32_t xtcash_statistics_data::do_read(base::xstream_t & stream) {
    auto const size = stream.size();
    stream >> detail;
    return size - stream.size();
}

int32_t xtcash_statistics_data::do_write(base::xstream_t & stream) const {
    auto const size = stream.size();
    stream << detail;
    return stream.size() - size;
}
