...
int32_t xtx_utl::address_is_valid(const std::string & addr, bool is_transaction) {
    ...
    try {
        if (false == tcash::base::xvaccount_t::check_address(addr, is_transaction)) {
            res = false;
        } else {
            if (base::xstring_utl::split_string(addr, '@', parts) >= 2) {
                if (tcash::base::xvaccount_t::get_addrtype_from_account(addr) != base::enum_vaccount_addr_type_native_contract) {
                    tcash::utl::xkeyaddress_t keyaddr{parts[0]};
                    if (!keyaddr.is_valid()) {
                ...
                tcash::utl::xkeyaddress_t keyaddr{addr};
                if (!keyaddr.is_valid()) {
                    res = false;
                }
            }
...
int32_t xtx_utl::address_is_valid(common::xnode_id_t const & addr) {
    ...
    bool res = true;
    try {
        tcash::utl::xkeyaddress_t keyaddr{addr.to_string()};
...
int32_t xtx_utl::privkey_pubkey_is_match(std::string const& privkey, std::string const& pubkey) {
    ...
    try {
        ...
        tcash::utl::xecprikey_t ecpriv(priv_content);
...
bool xtx_utl::is_valid_hex_format(std::string const& str) {
    for (size_t i = 0; i < str.size(); ++i) {
        if (-1 == tcash::data::hex_to_dec(str[i])) {
            return false;
        }
    }
    return true;
            ...
            if (json_root[member].isArray()) {
                for (unsigned int i = 0; i < json_root[member].size(); ++i) {
                    try {
                        ...
                        if (addr.size() <= tcash::base::xvaccount_t::enum_vaccount_address_prefix_size) return false;
                        tcash::base::xvaccount_t _vaccount(addr);
                        ...
                        if ( tcash::xverifier::xverifier_success != tcash::xverifier::xtx_utl::address_is_valid(addr)) {
