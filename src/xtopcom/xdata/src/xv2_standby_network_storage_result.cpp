...
xstandby_network_result_t xtcash_standby_network_storage_result::network_result(common::xnode_type_t const node_type) const {
    ...
    for (auto const & p : m_results) {
        ...
        auto const & standby_node_info = tcash::get<election::v2::xstandby_node_info_t>(p);
        for (auto & stake : standby_node_info.stake_container) {
            if (node_type == tcash::get<common::xnode_type_t const>(stake))
...
v1::xstandby_network_storage_result_t xtcash_standby_network_storage_result::v1() const {
