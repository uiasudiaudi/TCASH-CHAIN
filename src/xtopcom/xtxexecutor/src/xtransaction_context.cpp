...
int32_t xtransaction_run_contract::target_action_exec() {
    ...
    if (get_amount() != 0) {
        auto ret = m_account_ctx->tcash_token_transfer_in(get_amount());
        if (0 != ret) {
            return ret;
        }
    }
...
int32_t xtransaction_transfer::source_fee_exec() {
    ...
    if (!is_contract_address(common::xaccount_address_t{ m_trans->get_source_addr() })) {
        ...
        if (get_asset().is_tcash_token()) {
            auto transfer_amount = get_amount();
            xdbg("xtransaction_transfer::source_fee_exec_1 tx tcash_token");
            if (transfer_amount) {
            ...
            xdbg("xtransaction_transfer::source_fee_exec_1 tx no_tcash_token");
...
int32_t xtransaction_pledge_token_vote::source_fee_exec(){
    m_lock_token = m_account_ctx->get_tcash_by_vote(get_vote_num(), get_lock_duration());
