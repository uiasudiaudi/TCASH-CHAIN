...
class xtransaction_transfer : public xtransaction_face_t{
 public:
    ...
    int32_t check() override {
        ...
        // min transfer amount
        if (get_asset().is_tcash_token() && amount <= 0) { // just allow zero
             return xverifier::xverifier_error::xverifier_error_transfer_tx_min_amount_invalid;
        }
        
        if (!get_asset().is_tcash_token() && amount256 <= 0) { // just allow zero
             return xverifier::xverifier_error::xverifier_error_transfer_tx_min_amount_invalid;
        }

        if (get_asset().is_tcash_token() && amount > TOTAL_ISSUANCE){
