...
void GossipFilter::PrintRepeatMap() {
#ifndef NDEBUG
    {
        ...
        tcash_DEBUG("gossipfilter repeat recv avg:%d max:%d mapsize:%d total_recv:%d max_recv_hash:%u",
                avg_recv_times, max_recv, repeat_map_.size(), total_recv_count, max_recv_hash);

        // do clear
        if (repeat_map_.size() > 100000) {
            repeat_map_.clear();
            tcash_DEBUG("gossipfilter repeat map clear, now size:%d", repeat_map_.size());
        }
    }
#endif
}
...
bool GossipFilter::FilterMessage(transport::protobuf::RoutingMessage& message) {
    ...
    if (!message.has_msg_hash()) {
        tcash_DEBUG("filter failed, gossip msg(%d) should set msg_hash", message.type());
        return true;;
    }
#ifndef NDEBUG
    ...
    if (FindData(message.msg_hash())) {
        //tcash_DEBUG("GossipFilter FindData, filter msg");
        return true;
    }
    if (!AddData(message.msg_hash())) {
        tcash_WARN("GossipFilter already exist, filter msg");
        return true;
    }
    return false;
}
