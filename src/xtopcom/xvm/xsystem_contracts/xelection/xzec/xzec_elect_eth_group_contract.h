...
class xtcash_zec_elect_eth_contract final : public xelect_consensus_group_contract_t {
    using xbase_t = xelect_consensus_group_contract_t;

public:
    XDECLARE_DELETED_COPY_DEFAULTED_MOVE_SEMANTICS(xtcash_zec_elect_eth_contract);
    XDECLARE_DEFAULTED_OVERRIDE_DESTRUCTOR(xtcash_zec_elect_eth_contract);

    explicit xtcash_zec_elect_eth_contract(common::xnetwork_id_t const & network_id);

    xcontract_base * clone() override {
        return new xtcash_zec_elect_eth_contract(network_id());
    }
...
#endif
    BEGIN_CONTRACT_WITH_PARAM(xtcash_zec_elect_eth_contract)
    CONTRACT_FUNCTION_PARAM(xtcash_zec_elect_eth_contract, on_timer);
...
using xzec_elect_eth_contract_t = xtcash_zec_elect_eth_contract;
