...
void xrec_consortium_proposal_contract::setup() {
    ...
    tcash::chain_data::data_processor_t data;
    tcash::chain_data::xtcash_chain_data_processor::get_contract_data(common::xlegacy_account_address_t{SELF_ADDRESS()}, data);
    tcash_TOKEN_INCREASE(data.tcash_balance);
}
...
bool xrec_consortium_proposal_contract::get_proposal_info(const std::string & proposal_id, proposal_info & proposal) {
    ...
    } catch (tcash::error::xtcash_error_t const &) {
        xdbg("[xrec_consortium_proposal_contract::get_proposal_info] can't find proposal for id: %s", proposal_id.c_str());
        return false;  // not exist
    }

    tcash::base::xstream_t stream(base::xcontext_t::instance(), (uint8_t *)value.data(), value.size());
...
void xrec_consortium_proposal_contract::submitProposal(const std::string & target,
    ...
    tcash::base::xstream_t stream(base::xcontext_t::instance(), (uint8_t *)trx_ptr->get_source_action_para().data(), trx_ptr->get_source_action_para().size());
    ...
    XCONTRACT_ENSURE(asset_out.m_amount >= min_tcc_proposal_deposit * tcash_UNIT, "[xrec_consortium_proposal_contract::submitProposal] deposit less than minimum proposal deposit!");
    ...
    tcash::base::xstream_t newstream(base::xcontext_t::instance());
...
void xrec_consortium_proposal_contract::tccVote(std::string & proposal_id, bool option) {
    ...
    if (proposal.voting_status == voting_failure || proposal.voting_status == voting_success) {
        ...
        if (proposal.voting_status == voting_success) {
            ...
            tcash::base::xstream_t stream(base::xcontext_t::instance());
            stream.reset();
            ...
            case proposal_type::proposal_delete_parameter:
                ...
                break;
            case proposal_type::proposal_update_parameter_incremental_add:
                new_list = tcash::config::xconfig_utl::incremental_add_bwlist(MAP_GET(ONCHAIN_PARAMS, proposal.parameter), proposal.new_value);
                MAP_SET(ONCHAIN_PARAMS, proposal.parameter, new_list);
                STRING_SET(CURRENT_VOTED_PROPOSAL, voted_proposal);
                break;
            case proposal_type::proposal_update_parameter_incremental_delete:
                new_list = tcash::config::xconfig_utl::incremental_delete_bwlist(MAP_GET(ONCHAIN_PARAMS, proposal.parameter), proposal.new_value);
                MAP_SET(ONCHAIN_PARAMS, proposal.parameter, new_list);
                STRING_SET(CURRENT_VOTED_PROPOSAL, voted_proposal);
                break;
...
void xrec_consortium_proposal_contract::check_bwlist_proposal(std::string const& bwlist) {
    ...
    for (auto const& v: vec_member) {
        XCONTRACT_ENSURE(v.size() > tcash::base::xvaccount_t::enum_vaccount_address_prefix_size, "[xrec_consortium_proposal_contract::check_bwlist_proposal]  target value error, addr not support");
        auto const addr_type = tcash::base::xvaccount_t::get_addrtype_from_account(v);
        XCONTRACT_ENSURE(tcash::base::xvaccount_t::is_unit_address_type(addr_type), "[xrec_consortium_proposal_contract::check_bwlist_proposal]  target value error, addr type not support");
        XCONTRACT_ENSURE(tcash::xverifier::xverifier_success == tcash::xverifier::xtx_utl::address_is_valid(v), "[xrec_consortium_proposal_contract::check_bwlist_proposal]  target value error, addr invalid");
    }
...
void xrec_consortium_proposal_contract::check_cross_chain_contract_tx_list_proposal(std::string const& tx_list_config_str) {
    ...
    for (auto& str : str_vec) {
        ...
        tcash::base::xstring_utl::tolower_string(chain_bits);
