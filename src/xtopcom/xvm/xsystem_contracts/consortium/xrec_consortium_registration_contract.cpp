...
void xrec_consortium_registration_contract::setup() {
    ...
    ::chain_data::x_chain_data_processor::get_contract_data(common::xlegacy_account_address_t{SELF_ADDRESS()}, data);
    _TOKEN_INCREASE(data._balance);
    ...
    data::system_contract::xreg_node_info node_info;
    {
        common::xnetwork_id_t network_id{::config::to_chainid(XGET_CONFIG(chain_name))};
...
void xrec_consortium_registration_contract::registerNode(const std::string & miner_type_name,
    ...
    common::xnetwork_id_t nid{::config::to_chainid(XGET_CONFIG(chain_name))};
