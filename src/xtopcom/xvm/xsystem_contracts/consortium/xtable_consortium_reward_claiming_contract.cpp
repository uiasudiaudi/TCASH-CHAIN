...
NS_BEG3(tcash, xvm, consortium)

xtcash_table_consortium_reward_claiming_contract::xtcash_table_consortium_reward_claiming_contract(common::xnetwork_id_t const & network_id) : xbase_t{network_id} {
}

void xtcash_table_consortium_reward_claiming_contract::setup() {
    uint32_t table_id = 0;
    if (!EXTRACT_TABLE_ID(SELF_ADDRESS(), table_id)) {
        xwarn("[xtcash_table_consortium_reward_claiming_contract::setup] EXTRACT_TABLE_ID failed, node reward pid: %d, account: %s", getpid(), SELF_ADDRESS().to_string().c_str());
        return;
    }
    xdbg("[xtcash_table_consortium_reward_claiming_contract::setup] table id: %d", table_id);
...
xcontract::xcontract_base * xtcash_table_consortium_reward_claiming_contract::clone() {
    return new xtcash_table_consortium_reward_claiming_contract{network_id()};
}

void xtcash_table_consortium_reward_claiming_contract::update_working_reward_record(common::xaccount_address_t const & account, data::system_contract::xreward_node_record const & record) {
...
void xtcash_table_consortium_reward_claiming_contract::update(common::xaccount_address_t const & node_account, uint64_t issuance_clock_height, ::uint128_t reward) {
    auto const & self_address = SELF_ADDRESS();
    xinfo("[xtcash_table_consortium_reward_claiming_contract::update] self_address: %s, account: %s, reward: [%llu, %u], pid: %d",
    ...
    data::system_contract::xreward_node_record record;
    {
        ...
        if (ret)
            xwarn("[xtcash_table_consortium_reward_claiming_contract::update] get property empty, node account %s", node_account.to_string().c_str());
        if (!value_str.empty()) {
...
void xtcash_table_consortium_reward_claiming_contract::recv_node_reward(uint64_t issuance_clock_height, std::map<std::string, ::uint128_t> const & rewards) {
    XMETRICS_TIME_RECORD("sysContract_tableRewardClaiming_recv_node_reward");

    auto const & source_address = SOURCE_ADDRESS();
    auto const & self_address = SELF_ADDRESS();

    xinfo("[xtcash_table_consortium_reward_claiming_contract::recv_node_reward] pid: %d, source_address: %s, self_address: %s, issuance_clock_height:%llu, rewards size: %d",
          getpid(),
    ...
    XCONTRACT_ENSURE((sys_contract_zec_reward_addr == source_address), "[xtcash_table_consortium_reward_claiming_contract::recv_node_reward] working reward is not from zec workload contract but from " + source_address);
    ...
    for (auto const & account_reward : rewards) {
        ...
        xinfo("[xtcash_table_consortium_reward_claiming_contract::recv_node_reward] account: %s, reward: [%llu, %u]\n",
              account.c_str(),
              static_cast<uint64_t>(reward / data::system_contract::REWARD_PRECISION),
              static_cast<uint32_t>(reward % data::system_contract::REWARD_PRECISION));
...
void xtcash_table_consortium_reward_claiming_contract::claimNodeReward() {
    ...
    xinfo("[xtcash_table_consortium_reward_claiming_contract::claimNodeReward] balance: %" PRIu64 ", account: %s, cur_time: %" PRIu64, GET_BALANCE(), account.to_string().c_str(), cur_time);
    auto min_node_reward = XGET_ONCHAIN_GOVERNANCE_PARAMETER(min_node_reward);
    // transfer to account
    xinfo("[xtcash_table_consortium_reward_claiming_contract::claimNodeReward] reward: [%" PRIu64 ", %" PRIu32 "], reward_str: %s, reward_upper: %" PRIu64 ", reward_lower: %" PRIu64
...
int32_t xtcash_table_consortium_reward_claiming_contract::get_working_reward_record(common::xaccount_address_t const & account, data::system_contract::xreward_node_record & record) {
    std::string value_str;

    {
        ...
        if (ret) {
            xdbg("[xtcash_table_consortium_reward_claiming_contract::get_working_reward_record] account: %s not exist", account.to_string().c_str());
            return -1;
        }
    }
