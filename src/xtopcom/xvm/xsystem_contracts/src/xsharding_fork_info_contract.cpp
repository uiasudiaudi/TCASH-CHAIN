...
xtcash_sharding_fork_info_contract::xtcash_sharding_fork_info_contract(common::xnetwork_id_t const & network_id) : xbase_t{network_id} {
}

void xtcash_sharding_fork_info_contract::setup() {
    xinfo("xtcash_sharding_fork_info_contract::setup");
...
void xtcash_sharding_fork_info_contract::on_timer(common::xlogic_time_t const trigger_time) {
    auto current_time = TIME();
    xinfo("xtcash_sharding_fork_info_contract::on_timer % " PRIu64 " triggered at %" PRIu64 "", current_time, trigger_time);
    ...
    if (chain_fork::xutility_t::is_forked(fork_points::v10902_table_tickets_reset, current_time) && STRING_GET(data::XPROPERTY_CONTRACT_TABLE_FORK_INFO_KEY).empty()) {
        xinfo("xtcash_sharding_fork_info_contract::do_fork: v10902_table_tickets_reset");
    ...
    ///     xinfo("xtcash_sharding_fork_info_contract::do_fork: TEST_FORK");
