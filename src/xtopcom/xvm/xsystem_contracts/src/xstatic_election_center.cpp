...
std::vector<standby_node_info> xstatic_election_center::get_standby_config() {
    ...
    auto & config_register = tcash::config::xconfig_register_t::get_instance();
    ...
    tcash::base::xstring_utl::split_string(config_infos, ',', node_standby_infos);
    ...
    for (auto const & nodes : node_standby_infos) {
        ...
        tcash::base::xstring_utl::split_string(nodes, ':', one_node_info);
        std::string node_id_str = one_node_info[0];
        std::string node_pub_key_str = one_node_info[1];
        std::vector<std::string> type_stake_pairs;
        tcash::base::xstring_utl::split_string(one_node_info[2], '|', type_stake_pairs);
        ...
        for (auto const & each_pair : type_stake_pairs) {
            ...
            tcash::base::xstring_utl::split_string(each_pair, '.', type_stake);
...
std::vector<node_info> xstatic_election_center::get_static_election_nodes(std::string const & key) {
    ...
    tcash::base::xstring_utl::split_string(config_infos, ',', nodes_info);
...
void xstatic_election_center::calc_static_tcash_consensus_election_nodes() {
    auto & config_register = tcash::config::xconfig_register_t::get_instance();
    ...
    tcash::base::xstring_utl::split_string(consensus_infos, '|', group_string_value);
    ...
    for (auto const & group_info : group_string_value) {
        ...
        tcash::base::xstring_utl::split_string(group_info, ':', type_detail);
        auto group_type = type_detail[0];          // auditor or validator
        auto detailed_node_info = type_detail[1];  // multi nodes info.
        std::vector<std::string> nodes_info;
        tcash::base::xstring_utl::split_string(detailed_node_info, ',', nodes_info);
        std::vector<node_info> res;
        for (auto nodes : nodes_info) {
            std::vector<std::string> one_node_info;
            tcash::base::xstring_utl::split_string(nodes, '.', one_node_info);
        ...
        tcash_consensus_group_map.insert({group_id_value, res});
    }
}
...
std::vector<node_info> xstatic_election_center::get_static_tcash_consensus_election_nodes(uint8_t group_id_value) {
    assert(group_id_value == 1 || group_id_value == 2 || group_id_value == 64 || group_id_value == 65 || group_id_value == 66 || group_id_value == 67);
    return tcash_consensus_group_map.at(group_id_value);
}
