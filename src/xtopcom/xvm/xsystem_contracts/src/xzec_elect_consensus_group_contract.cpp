...
using tcash::data::election::xelection_association_result_store_t;
...
using tcash::data::election::xstandby_result_store_t;
using tcash::data::election::xstandby_result_t;

NS_BEG4(tcash, xvm, system_contracts, zec)

xtcash_zec_elect_consensus_group_contract::xtcash_zec_elect_consensus_group_contract(common::xnetwork_id_t const & network_id) : xbase_t{network_id} {}
...
void xtcash_zec_elect_consensus_group_contract::swap_election_result(common::xlogic_time_t const current_time) {
    ...
    std::unordered_map<tcash::common::xgroup_id_t, tcash::data::election::xelection_result_store_t> all_cluster_election_result_store;
...
void xtcash_zec_elect_consensus_group_contract::elect_config_nodes(common::xlogic_time_t const current_time) {
    uint64_t latest_height = get_blockchain_height(sys_contract_zec_elect_consensus_addr);
    ...
    for (auto const & property : property_names) {
        ...
        // elect in:
        auto adv_group_node_infos = xstatic_election_center::instance().get_static_tcash_consensus_election_nodes(adv_group_id.value());
        ...
        for (auto _index = 0; _index < pre_associate; ++_index) {
            ...
            auto val_group_node_infos = xstatic_election_center::instance().get_static_tcash_consensus_election_nodes(val_group_id.value());
...
void xtcash_zec_elect_consensus_group_contract::setup() {
...
void xtcash_zec_elect_consensus_group_contract::on_timer(common::xlogic_time_t const current_time) {
#ifdef STATIC_CONSENSUS
...
void xtcash_zec_elect_consensus_group_contract::elect(common::xzone_id_t const zone_id,
    ...
    for (auto index = 0; index < auditor_group_count; ++index) {
        tcash::common::xgroup_id_t auditor_gid{static_cast<tcash::common::xgroup_id_t::value_type>(common::xauditor_group_id_value_begin + index)};
...
bool xtcash_zec_elect_consensus_group_contract::elect_auditor_validator(common::xzone_id_t const & zone_id,
                                                                      common::xcluster_id_t const & cluster_id,
    ...
    for (auto const & assoc_validator_group_id : associated_validator_group_ids) {
        ...
        for (auto const & validator_node_info : assoc_validator_group_nodes) {
            auto const & validator_node_id = tcash::get<data::election::xelection_info_bundle_t>(validator_node_info).account_address();
    ...
    for (auto const & other_auditor_group_id : all_auditor_group_id) {
        ...
        for (auto const & auditor_node_info : other_auditor_group_nodes) {
            auto const & auditor_node_id = tcash::get<data::election::xelection_info_bundle_t>(auditor_node_info).account_address();
    ...
    xwarn("%s stcashs to filter auditor standbys (standby size %zu)", log_prefix.c_str(), effective_auditor_standbys.size());
    ...
    for (auto const & validator_group_id : associated_validator_group_ids) {
        ...
        xwarn("%s stcashs to filter validator standbys (standby size %zu)", log_prefix.c_str(), effective_validator_standbys.size());
...
bool xtcash_zec_elect_consensus_group_contract::elect_auditor(common::xzone_id_t const & zid,
...
bool xtcash_zec_elect_consensus_group_contract::elect_validator(common::xzone_id_t const & zid,
                                                              common::xcluster_id_t const & cid,
...
bool xtcash_zec_elect_consensus_group_contract::genesis_elected() const {
