...
using tcash::data::election::xelection_network_result_t;
using tcash::data::election::xstandby_network_result_t;
using tcash::data::election::xstandby_node_info_t;
using tcash::data::election::xstandby_result_t;
...
xtcash_election_awared_data::xtcash_election_awared_data(common::xnode_id_t const & account,
...
xtcash_election_awared_data::xtcash_election_awared_data(common::xnode_id_t const & account,
                                                     ...
                                                     bool genesis,
                                                     uint64_t raw_credit_score)
  : xtcash_election_awared_data{account, stake, 0, public_key, miner_type, genesis, raw_credit_score} {
}

bool xtcash_election_awared_data::operator<(xtcash_election_awared_data const & other) const noexcept {
...
common::xnode_id_t const & xtcash_election_awared_data::account() const noexcept {
    return m_account;
}

uint64_t xtcash_election_awared_data::stake() const noexcept {
    return m_stake;
}

uint64_t xtcash_election_awared_data::comprehensive_stake() const noexcept {
    return m_comprehensive_stake;
}

xpublic_key_t const & xtcash_election_awared_data::public_key() const noexcept {
    return m_public_key;
}

void xtcash_election_awared_data::comprehensive_stake(uint64_t const s) noexcept {
    m_comprehensive_stake = s;
}

bool xtcash_election_awared_data::genesis() const noexcept {
    return m_genesis;
}

common::xminer_type_t xtcash_election_awared_data::miner_type() const noexcept {
    return m_miner_type;
}

uint64_t xtcash_election_awared_data::raw_credit_score() const noexcept {
    return m_raw_credit_score;
}

xtcash_elect_consensus_group_contract::xtcash_elect_consensus_group_contract(common::xnetwork_id_t const & network_id) : xbase_t{network_id} {}
...
bool xtcash_elect_consensus_group_contract::elect_group(common::xzone_id_t const & zid,
    ...
    try {
        ...
        xwarn("%s xtcash_error_t exception caught. category: %s msg: %s", log_prefix.c_str(), eh.code().category().name(), eh.what());
        throw;
...
void xtcash_elect_consensus_group_contract::handle_elected_out_data(std::vector<common::xfts_merkle_tree_t<common::xnode_id_t>::value_type> const & chosen_out,
    ...
    for (auto const & out : chosen_out) {
        XMETRICS_PACKET_INFO(XCONSENSUS_ELECTION "elect_out",
                             ...
                             gid.to_string(),
                             "node_id",
                             tcash::get<common::xnode_id_t>(out).to_string(),
                             "stake",
                             static_cast<int64_t>(tcash::get<common::xstake_t>(out)));
        ...
        auto elect_out_pos = std::find_if(
            std::begin(election_group_result), std::end(election_group_result), [&out](std::pair<common::xslot_id_t const, xelection_info_bundle_t> const & node_info) {
                return tcash::get<common::xnode_id_t>(out) == tcash::get<xelection_info_bundle_t>(node_info).account_address();
            });
...
void xtcash_elect_consensus_group_contract::handle_elected_in_data(std::vector<common::xfts_merkle_tree_t<common::xnode_id_t>::value_type> const & chosen_in,
    ...
    for (auto const & in : chosen_in) {
        auto const & node_id = tcash::get<common::xnode_id_t>(in);
...
bool xtcash_elect_consensus_group_contract::do_normal_election(common::xzone_id_t const & zid,
                                                             common::xcluster_id_t const & cid,
                                                             common::xgroup_id_t const & gid,
                                                             common::xnode_type_t const node_type,
    ...
    for (auto const & standby_info : standby_result) {
        effective_standby_result.emplace_back(tcash::get<common::xnode_id_t const>(standby_info),
                                              tcash::get<xstandby_node_info_t>(standby_info).stake(node_type),
                                              minimum_comprehensive_stake,
                                              tcash::get<xstandby_node_info_t>(standby_info).consensus_public_key,
                                              tcash::get<xstandby_node_info_t>(standby_info).miner_type,
                                              tcash::get<xstandby_node_info_t>(standby_info).genesis,
                                              tcash::get<xstandby_node_info_t>(standby_info).raw_credit_score(node_type));
    }
    ...
    for (auto it = std::begin(effective_standby_result); it != std::end(effective_standby_result);) {
        ...
        if (tcash::get<bool>(current_group_nodes.find(standby_node_id))) {
    ...
    for (auto const & standby : fts_standbys) {
        xdbg("%s fts node id %s stake %" PRIu64, log_prefix.c_str(), tcash::get<common::xnode_id_t>(standby).to_string().c_str(), tcash::get<common::xstake_t>(standby));
    }
#endif
    ...
    for (auto & node_info : result_nodes) {
        auto & election_info_bundle = tcash::get<xelection_info_bundle_t>(node_info);
    ...
    if (elect_out_count > 0) {
        ...
        std::sort(std::begin(fts_current_nodes),
                  ...
                  [](common::xfts_merkle_tree_t<common::xnode_id_t>::value_type const & lhs, common::xfts_merkle_tree_t<common::xnode_id_t>::value_type const & rhs) {
                      auto const lhs_stake = tcash::get<common::xstake_t>(lhs);
                      auto const rhs_stake = tcash::get<common::xstake_t>(rhs);
                      if (lhs_stake != rhs_stake) {
                          return lhs_stake > rhs_stake;
                      }
                      return tcash::get<common::xnode_id_t>(lhs) > tcash::get<common::xnode_id_t>(rhs);
                  });
        auto const tcash_stake = tcash::get<common::xstake_t>(fts_current_nodes.front());
        std::for_each(std::begin(fts_current_nodes), std::end(fts_current_nodes), [&fts_current_nodes, tcash_stake](common::xfts_merkle_tree_t<common::xnode_id_t>::value_type & v) {
            auto const comprehensive_stake = tcash::get<common::xstake_t>(v);
            xdbg("watch_fts_start node:%s stake:%" PRIu64, tcash::get<common::xnode_id_t>(v).to_string().c_str(), comprehensive_stake);
            assert(comprehensive_stake > 0);
            tcash::get<common::xstake_t>(v) = tcash_stake * basic_comprehensive_stake / comprehensive_stake;
        });
    ...
    for (auto & node_info : result_nodes) {
        auto & election_info_bundle = tcash::get<data::election::xelection_info_bundle_t>(node_info);
...
bool xtcash_elect_consensus_group_contract::do_shrink_election(common::xzone_id_t const & zid,
                                                             common::xcluster_id_t const & cid,
                                                             common::xgroup_id_t const & gid,
                                                             common::xnode_type_t const node_type,
                                                             std::uint64_t const random_seed,
