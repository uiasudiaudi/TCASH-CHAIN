...
void xrec_proposal_contract::submitProposal(const std::string & target,
    ...
    XCONTRACT_ENSURE(asset_out.m_amount >= min_tcc_proposal_deposit * tcash_UNIT, "[xrec_proposal_contract::submitProposal] deposit less than minimum proposal deposit!");
...
void xrec_proposal_contract::check_bwlist_proposal(std::string const& bwlist) {
    ...
    for (auto const& v: vec_member) {
        XCONTRACT_ENSURE(v.size() > tcash::base::xvaccount_t::enum_vaccount_address_prefix_size, "[xrec_proposal_contract::check_bwlist_proposal]  target value error, addr not support");
        auto const addr_type = tcash::base::xvaccount_t::get_addrtype_from_account(v);
        XCONTRACT_ENSURE(tcash::base::xvaccount_t::is_unit_address_type(addr_type), "[xrec_proposal_contract::check_bwlist_proposal]  target value error, addr type not support");
        XCONTRACT_ENSURE(tcash::xverifier::xverifier_success == tcash::xverifier::xtx_utl::address_is_valid(v), "[xrec_proposal_contract::check_bwlist_proposal]  target value error, addr invalid");
    }
...
void xrec_proposal_contract::check_cross_chain_contract_tx_list_proposal(std::string const& tx_list_config_str) {
    ...
    for (auto& str : str_vec) {
        ...
        // std::string& tcashic = config_str_vec[1];
