...
xtcash_rec_standby_pool_contract::xtcash_rec_standby_pool_contract(common::xnetwork_id_t const & network_id) : xbase_t{network_id} {}

void xtcash_rec_standby_pool_contract::setup() {
    ...
    for (auto const & node_info : static_consensus_nodes_info) {
        ...
        for (auto const & _pair : node_info.type_stake_pair) {
            common::xnode_type_t const & node_type = tcash::get<common::xnode_type_t>(_pair);
            uint64_t stake = tcash::get<uint64_t>(_pair);
    ...
    for (auto & standby_network_result_info : standby_result_store) {
        auto & standby_network_storage_result = tcash::get<election::v0::xstandby_network_storage_result_t>(standby_network_result_info);
...
void xtcash_rec_standby_pool_contract::nodeJoinNetwork2(common::xaccount_address_t const & node_id,
    ...
    MAP_COPY_GET(tcash::data::system_contract::XPORPERTY_CONTRACT_REG_KEY, map_nodes, sys_contract_rec_registration_addr);
    ...
    common::xnetwork_id_t nid{tcash::config::to_chainid(XGET_CONFIG(chain_name))};
    ...
    tcash::base::xstream_t param_stream(base::xcontext_t::instance());
...
bool xtcash_rec_standby_pool_contract::nodeJoinNetworkImpl(std::string const & program_version,
...
bool xtcash_rec_standby_pool_contract::update_standby_node(data::system_contract::xreg_node_info const & reg_node,
    ...
    if (reg_node.can_be_auditor()) {
        ...
        new_node_info.raw_credit_score(common::xnode_type_t::consensus_auditor, reg_node.raw_credit_score_data(tcash::common::xnode_type_t::consensus_auditor));
    }
    if (reg_node.can_be_validator()) {
        ...
        new_node_info.raw_credit_score(common::xnode_type_t::consensus_validator, reg_node.raw_credit_score_data(tcash::common::xnode_type_t::consensus_validator));
    }
    ...
    if (evm_enabled && reg_node.can_be_evm_auditor()) {
        ...
        xdbg("xrec_standby_pool_contract_t::update_standby_node account %s credit score %" PRIu64,
             reg_node.m_account.to_string().c_str(),
             reg_node.raw_credit_score_data(common::xnode_type_t::consensus_auditor));
        new_node_info.raw_credit_score(common::xnode_type_t::evm_auditor, reg_node.raw_credit_score_data(tcash::common::xnode_type_t::consensus_auditor));
    }
    if (evm_enabled && reg_node.can_be_evm_validator()) {
        ...
        xdbg("xrec_standby_pool_contract_t::update_standby_node account %s credit score %" PRIu64,
             reg_node.m_account.to_string().c_str(),
             reg_node.raw_credit_score_data(common::xnode_type_t::consensus_validator));
        new_node_info.raw_credit_score(common::xnode_type_t::evm_validator, reg_node.raw_credit_score_data(tcash::common::xnode_type_t::consensus_validator));
    }
    if (relay_enabled && reg_node.can_be_relay()) {
        ...
        new_node_info.raw_credit_score(common::xnode_type_t::relay, reg_node.raw_credit_score_data(tcash::common::xnode_type_t::relay));
    }
...
bool xtcash_rec_standby_pool_contract::update_activated_state(xstandby_network_storage_result_t & standby_network_storage_result,
...
bool xtcash_rec_standby_pool_contract::update_standby_result_store(std::map<common::xnode_id_t, data::system_contract::xreg_node_info> const & registration_data,
                                                                 data::election::xstandby_result_store_t & standby_result_store,
                                                                 data::system_contract::xactivation_record const & activation_record,
                                                                 common::xlogic_time_t const current_logic_time) {
    bool updated{false};
    for (auto & standby_network_result_info : standby_result_store) {
        assert(tcash::get<common::xnetwork_id_t const>(standby_network_result_info).value() == base::enum_test_chain_id ||
               tcash::get<common::xnetwork_id_t const>(standby_network_result_info).value() == base::enum_main_chain_id);

        auto & standby_network_storage_result = tcash::get<election::xstandby_network_storage_result_t>(standby_network_result_info);
        for (auto it = standby_network_storage_result.begin(); it != standby_network_storage_result.end();) {
            auto const & node_id = tcash::get<common::xnode_id_t const>(*it);
            auto & node_info = tcash::get<election::xstandby_node_info_t>(*it);
            ...
            if (registration_iter == std::end(registration_data)) {
                ...
                auto const & reg_node = tcash::get<data::system_contract::xreg_node_info>(*registration_iter);
...
void xtcash_rec_standby_pool_contract::on_timer(common::xlogic_time_t const current_time) {
#ifdef STATIC_CONSENSUS
