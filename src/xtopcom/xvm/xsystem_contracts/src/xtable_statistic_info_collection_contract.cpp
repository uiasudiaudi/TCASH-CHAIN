...
void xtable_statistic_info_collection_contract::on_collect_statistic_info(xstatistics_data_t const& statistic_data,  xfulltableblock_statistic_accounts const& statistic_accounts, uint64_t block_height, int64_t tgas) {
    ...
    XCONTRACT_ENSURE(base_addr == tcash::sys_contract_sharding_statistic_info_addr || base_addr == tcash::sys_contract_eth_table_statistic_info_addr, "invalid source addr's call!");
...
data::system_contract::xunqualified_node_info_v1_t xtable_statistic_info_collection_contract::process_statistic_data(
    tcash::data::xstatistics_data_t const & block_statistic_data,
    xfulltableblock_statistic_accounts const & statistic_accounts) {
    ...
    for (auto const & static_item: block_statistic_data.detail) {
        ...
        for (auto const & group_item: elect_statistic.group_statistics_data) {
            ...
            if (tcash::common::has<tcash::common::xnode_type_t::consensus_auditor>(group_addr.type()) || tcash::common::has<tcash::common::xnode_type_t::evm_auditor>(group_addr.type())) {
            ...
            } else if (tcash::common::has<tcash::common::xnode_type_t::consensus_validator>(group_addr.type()) || tcash::common::has<tcash::common::xnode_type_t::evm_validator>(group_addr.type())) {// process validator group
                for (std::size_t slotid = 0; slotid < group_account_data.account_statistics_data.size(); ++slotid) {
                    auto const & account_addr = group_accounts.account_data[slotid];
                ...
                tcash::error::throw_error(ec, "[xtable_statistic_info_collection_contract][do_unqualified_node_slash] invalid group");
            }
...
std::map<common::xgroup_address_t, xgroup_workload_t> xtable_statistic_info_collection_contract::get_workload_from_data(xstatistics_data_t const & statistic_data, xfulltableblock_statistic_accounts const& statistic_accounts) {
    ...
    for (auto const & static_item : statistic_data.detail) {
        ...
        for (auto const & group_item : elect_statistic.group_statistics_data) {
            ...
            xgroup_related_statistics_data_t const & group_account_data = group_item.second;
#if defined(DEBUG)
            xvip2_t const & group_xvip2 = tcash::common::xip2_t{group_addr.network_id(),
                                                              group_addr.zone_id(),
