...
NS_BEG4(tcash, xvm, system_contracts, reward)

xtcash_table_reward_claiming_contract::xtcash_table_reward_claiming_contract(common::xnetwork_id_t const & network_id) : xbase_t{network_id} {
}

void xtcash_table_reward_claiming_contract::setup() {
    const int old_tables_count = 256;
    uint32_t table_id = 0;
    if (!EXTRACT_TABLE_ID(SELF_ADDRESS(), table_id)) {
        xwarn("[xtcash_table_reward_claiming_contract::setup] EXTRACT_TABLE_ID failed, node reward pid: %d, account: %s", getpid(), SELF_ADDRESS().to_string().c_str());
        return;
    }
    xdbg("[xtcash_table_reward_claiming_contract::setup] table id: %d", table_id);
    ...
    tcash_TOKEN_INCREASE(acc_token);
}

xcontract::xcontract_base * xtcash_table_reward_claiming_contract::clone() {
    return new xtcash_table_reward_claiming_contract{network_id()};
}

void xtcash_table_reward_claiming_contract::update_vote_reward_record(common::xaccount_address_t const & account, data::system_contract::xreward_record const & record) {
...
void xtcash_table_reward_claiming_contract::recv_voter_dividend_reward(uint64_t issuance_clock_height, std::map<std::string, ::uint128_t> const & rewards) {
    auto const & source_address = SOURCE_ADDRESS();
    auto const & self_address = SELF_ADDRESS();

    xinfo("[xtcash_table_reward_claiming_contract::recv_voter_dividend_reward] self address: %s, source address: %s, issuance_clock_height: %llu",
          self_address.to_string().c_str(),
          source_address.c_str(),
          issuance_clock_height);

    if (rewards.size() == 0) {
        xwarn("[xtcash_table_reward_claiming_contract::recv_voter_dividend_reward] rewards size 0");
        return;
    }

    XCONTRACT_ENSURE(sys_contract_zec_reward_addr == source_address, "xtcash_table_reward_claiming_contract::recv_voter_dividend_reward from invalid address: " + source_address);
    ...
    XCONTRACT_ENSURE(data::xdatautil::extract_parts(self_address.to_string(), base_addr, table_id),
                     "xtcash_table_reward_claiming_contract::recv_voter_dividend_reward: extract table id failed");
    ...
    try {
        ...
        xwarn("[xtcash_table_reward_claiming_contract::recv_voter_dividend_reward] MAP_COPY_GET XPORPERTY_CONTRACT_POLLABLE_KEY error:%s", e.what());
    }
    ...
    for (auto i = 1; i <= data::system_contract::XPROPERTY_SPLITED_NUM; ++i) {
        ...
        xdbg("[xtcash_table_reward_claiming_contract::recv_voter_dividend_reward] vote maps %s size: %d, pid: %d", property_name.c_str(), voters.size(), getpid());
        ...
        for (auto const & entity : voters) {
            ...
            stream >> votes_table;
            data::system_contract::xreward_record record;
            xinfo("[xtcash_table_reward_claiming_contract::recv_voter_dividend_reward] voter: %s", account.c_str());
...
void xtcash_table_reward_claiming_contract::add_voter_reward(uint64_t issuance_clock_height,
                                                           std::map<std::string, uint64_t> & votes_table,
    ...
    for (auto const & adv_vote : votes_table) {
        ...
        xdbg(
            "[xtcash_table_reward_claiming_contract::recv_voter_dividend_reward] adv node: %s, node_vote_reward: [%llu, %u], node_total_votes: %llu, voter_node_votes: "
...
void xtcash_table_reward_claiming_contract::claimVoterDividend() {
    ...
    xinfo("[xtcash_table_reward_claiming_contract::claimVoterDividend] balance:%llu, account: %s, pid: %d, cur_time: %llu, last_claim_time: %llu\n",
          GET_BALANCE(),
    ...
    // transfer to account
    xinfo("[xtcash_table_reward_claiming_contract::claimVoterDividend] timer round: %" PRIu64 ", account: %s, reward:: [%llu, %u], pid:%d\n",
          TIME(),
          account.to_string().c_str(),
...
void xtcash_table_reward_claiming_contract::update_working_reward_record(common::xaccount_address_t const & account, data::system_contract::xreward_node_record const & record) {
    base::xstream_t stream(base::xcontext_t::instance());
    record.serialize_to(stream);
    auto value_str = std::string((char *)stream.data(), stream.size());
    {
...
void xtcash_table_reward_claiming_contract::update(common::xaccount_address_t const & node_account, uint64_t issuance_clock_height, ::uint128_t reward) {
    auto const & self_address = SELF_ADDRESS();
    xinfo("[xtcash_table_reward_claiming_contract::update] self_address: %s, account: %s, reward: [%llu, %u], pid: %d",
          self_address.to_string().c_str(),
    ...
    data::system_contract::xreward_node_record record;
    {
        ...
        if (ret)
            xwarn("[xtcash_table_reward_claiming_contract::update] get property empty, node account %s", node_account.to_string().c_str());
        if (!value_str.empty()) {
...
void xtcash_table_reward_claiming_contract::recv_node_reward(uint64_t issuance_clock_height, std::map<std::string, ::uint128_t> const & rewards) {
    auto const & source_address = SOURCE_ADDRESS();
    auto const & self_address = SELF_ADDRESS();

    xinfo("[xtcash_table_reward_claiming_contract::recv_node_reward] pid: %d, source_address: %s, self_address: %s, issuance_clock_height:%llu, rewards size: %d",
          getpid(),
          source_address.c_str(),
          self_address.to_string().c_str(),
          issuance_clock_height,
    ...
    XCONTRACT_ENSURE(sys_contract_zec_reward_addr == source_address,
                     "[xtcash_table_reward_claiming_contract::recv_node_reward] working reward is not from zec workload contract but from " + source_address);

    for (auto const & account_reward : rewards) {
        auto const & account = account_reward.first;
        auto const & reward = account_reward.second;

        xinfo("[xtcash_table_reward_claiming_contract::recv_node_reward] account: %s, reward: [%llu, %u]\n",
              account.c_str(),
              static_cast<uint64_t>(reward / data::system_contract::REWARD_PRECISION),
              static_cast<uint32_t>(reward % data::system_contract::REWARD_PRECISION));
...
void xtcash_table_reward_claiming_contract::claimNodeReward() {
    common::xaccount_address_t account{SOURCE_ADDRESS()};
    ...
    uint64_t cur_time = TIME();
    xinfo("[xtcash_table_reward_claiming_contract::claimNodeReward] balance: %" PRIu64 ", account: %s, cur_time: %" PRIu64, GET_BALANCE(), account.to_string().c_str(), cur_time);
    auto min_node_reward = XGET_ONCHAIN_GOVERNANCE_PARAMETER(min_node_reward);
    // transfer to account
    xinfo("[xtcash_table_reward_claiming_contract::claimNodeReward] reward: [%" PRIu64 ", %" PRIu32 "], reward_str: %s, reward_upper: %" PRIu64 ", reward_lower: %" PRIu64
...
int32_t xtcash_table_reward_claiming_contract::get_working_reward_record(common::xaccount_address_t const & account, data::system_contract::xreward_node_record & record) {
    std::string value_str;

    {
        int32_t ret = MAP_GET2(data::system_contract::XPORPERTY_CONTRACT_NODE_REWARD_KEY, account.to_string(), value_str);
        if (ret) {
            xdbg("[xtcash_table_reward_claiming_contract::get_working_reward_record] account: %s not exist", account.to_string().c_str());
            return -1;
        }
    }
...
int32_t xtcash_table_reward_claiming_contract::get_vote_reward_record(common::xaccount_address_t const & account, data::system_contract::xreward_record & record) {
    uint32_t sub_map_no = (utl::xxh32_t::digest(account.to_string()) % data::system_contract::XPROPERTY_SPLITED_NUM) + 1;
    std::string property;
...

    {
        if (!MAP_FIELD_EXIST(property, account.to_string())) {
            xdbg("[xtcash_table_reward_claiming_contract::get_vote_reward_record] property: %s, account %s not exist", property.c_str(), account.to_string().c_str());
            return -1;
        } else {
