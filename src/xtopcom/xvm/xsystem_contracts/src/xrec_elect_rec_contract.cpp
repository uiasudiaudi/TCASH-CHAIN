...
xtcash_rec_elect_rec_contract::xtcash_rec_elect_rec_contract(common::xnetwork_id_t const & network_id) : xbase_t{network_id} {}

void xtcash_rec_elect_rec_contract::setup() {
...
void xtcash_rec_elect_rec_contract::on_timer(common::xlogic_time_t const current_time) {
    XCONTRACT_ENSURE(SOURCE_ADDRESS() == SELF_ADDRESS().to_string(), "xtcash_rec_elect_rec_contract instance is triggled by " + SOURCE_ADDRESS());
    XCONTRACT_ENSURE(SELF_ADDRESS().to_string() == sys_contract_rec_elect_rec_addr, "xtcash_rec_elect_rec_contract instance is not triggled by sys_contract_rec_elect_rec_addr");
    // XCONTRACT_ENSURE(current_time <= TIME(), u8"xtcash_rec_elect_rec_contract::on_timer current_time > consensus leader's time");
