...
xtcash_elect_nonconsensus_group_contract::xtcash_elect_nonconsensus_group_contract(common::xnetwork_id_t const & network_id) : xbase_t{network_id} {}

bool xtcash_elect_nonconsensus_group_contract::elect_group(common::xzone_id_t const & zid,
    ...
    for (auto & new_node_info : standby_result) {
        auto const & node_id = tcash::get<xnode_id_t const>(new_node_info);
        auto const & node_standby_info = tcash::get<xstandby_node_info_t>(new_node_info);
    ...
    if (node_change) {
        ...
        for (auto const & node_info : new_group_result) {
            xdbg("%s elected in %s", log_prefix.c_str(), tcash::get<xelection_info_bundle_t>(node_info).account_address().to_string().c_str());
        }
#endif
