...
enum class xtcash_enum_action_type : uint8_t { invalid, source_action, target_action, confirm_action, self_action };
using xaction_type_t = xtcash_enum_action_type;

enum class xtcash_enum_property_name_type : uint8_t { invalid, full_name, short_name };
using xproperty_name_type_t = xtcash_enum_property_name_type;

class xtcash_contract_state {
private:
...
public:
    xtcash_contract_state() = default;
    xtcash_contract_state(xtcash_contract_state const &) = delete;
    xtcash_contract_state & operator=(xtcash_contract_state const &) = delete;
    xtcash_contract_state(xtcash_contract_state &&) = default;
    xtcash_contract_state & operator=(xtcash_contract_state &&) = default;
    ~xtcash_contract_state() = default;

    xtcash_contract_state(common::xaccount_address_t action_account_addr,
                        observer_ptr<state_accessor::xstate_accessor_t> const sa,
                        xcontract_execution_param_t const & execution_param);

    xtcash_contract_state(common::xaccount_address_t action_account_addr, statectx::xstatectx_face_ptr_t const state_ctx);

private:
    explicit xtcash_contract_state(common::xaccount_address_t const & account_address);
    explicit xtcash_contract_state(common::xaccount_address_t const & account_address, uint64_t const height);

public:

    static std::unique_ptr<xtcash_contract_state> build_from(common::xaccount_address_t const & account_address);
    static std::unique_ptr<xtcash_contract_state> build_from(common::xaccount_address_t const & account_address, std::error_code & ec);

    static std::unique_ptr<xtcash_contract_state> build_from(common::xaccount_address_t const & account_address, uint64_t const height);
    static std::unique_ptr<xtcash_contract_state> build_from(common::xaccount_address_t const & account_address, uint64_t const height, std::error_code & ec);
    ...
    /// @brief Create property. Throw xtcash_error_t exception when error occurs.
    /// @param property_id The property identifier.
    ...
    /// @brief Check if the property identified by the property ID exists or not. Throw xtcash_error_t exception when error occurs.
    ...
    /// @brief Clear property. This operation liks STL container's clear() API. Throws xtcash_error_t exception when any error occurs.
    /// @param property_id Property ID.
    ...
    /// @brief Update property cell value. Only map and deque are supported. Throws xtcash_error_t exception if any error occurs.
    /// @param proprty_id Property ID.
    /// @param key Cell position key.
    /// @param value Cell's new value.
    template <state_accessor::properties::xproperty_type_t PropertyTypeV>
    ...
    /// @brief Get property cell value. Only map and deque are supported. Throws xtcash_error_t exception when any error occurs.
    /// @param property_id Property ID.
    /// @param key Cell position key.
    /// @return Cell value.
    template <state_accessor::properties::xproperty_type_t PropertyTypeV>
    ...
    /// @brief Check if property cell key exist. Only map and deque are supported. Throws xtcash_error_t exception if any error occurs.
    /// @param property_id Property ID.
    /// @param key Cell position key.
    /// @return exist or not.
    template <state_accessor::properties::xproperty_type_t PropertyTypeV>
    ...
    /// @brief Set property. Throw xtcash_error_t exception when error occurs.
    /// @param property_id Property ID.
    /// @param value Value to be set.
    template <state_accessor::properties::xproperty_type_t PropertyTypeV>
    void set_property(state_accessor::properties::xtypeless_property_identifier_t const & property_id,
    ...
    /// @brief Get property. Throw xtcash_error_t exception when error occurs.
    /// @param property_id Property ID.
    /// @return Property value.
    template <state_accessor::properties::xproperty_type_t PropertyTypeV>
    ...
    /// @brief Get property of specific address. Throw xtcash_error_t exception when error occurs.
    /// @param property_id Property ID.
    /// @param address Address to get.
    /// @return Property value.
    template <state_accessor::properties::xproperty_type_t PropertyTypeV>
    ...
    /// @brief Get code. If current state is a contract state, returns the contract code. If error occurs, xtcash_error_t exception will be thrown.
    /// @param property_id Property ID.
    /// @return The bytecode.
    ...
    /// @brief Deploy bytecode. If error occurs, xtcash_error_t exception will be thrown.
    /// @param property_id Property ID.
    /// @param bin_code The bytecode to be deployed.
    ...
    /// @brief Get code of canvas. If error occurs, xtcash_error_t exception will be thrown.
    /// @return The bincode of canvas.
    ...
    /// @brief Get full state bin of state. If error occurs, xtcash_error_t exception will be thrown.
    /// @param ec Log the error code in the deployment logic.
    ...
    /// @brief Get the balance from the state. Throw xtcash_error_t exception when any error occurs.
    /// @param property_id Property ID.
    /// @param symbol Simbol of the token.
    /// @return The token amount.
    ...
    /// @brief Deposit tokens into state. Throw xtcash_error_t exception when any error occurs.
    /// @param property_id Property ID.
    /// @param tokens Tokens to deposit.
    ...
    /// @brief Withdraw tokens from state. Throw xtcash_error_t exception when any error occurs.
    /// @param property_id Property ID.
    /// @param symbol Simbol of the token.
    /// @return The token object.
    ...
    /// @brief Transfer tokens from one property the other property in same account state. Throw xtcash_error_t exception when any error occurs.
...
using xcontract_state_t = xtcash_contract_state;
