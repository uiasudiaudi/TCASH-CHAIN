...
public:
    tcash::contract_common::xcontract_state_t* context_;
...
protected:
    void SetUp() override {
        ...
        tcash::xobject_ptr_t<xvbstate_t> vbstate;
        ...
        std::shared_ptr<xproperty_access_control_t> api_ = std::make_shared<xproperty_access_control_t>(tcash::make_observer(vbstate.get()), ac_data);

        tcash::contract_common::xcontract_metadata_t meta;
        context_ =  new tcash::contract_common::xcontract_state_t{tcash::common::xaccount_address_t{address}, tcash::make_observer(api_.get())};
    }
...
TEST_F(test_property_object, simple_map_property) {
    ...
    xtcash_map_property<std::string, std::string> map_prop{"test_map1", nullptr};
...
TEST_F(test_property_object, obj_map_property) {
    // map<string, object>
    xtcash_map_property<std::string, test_data> obj_prop{"test_map2", nullptr};
