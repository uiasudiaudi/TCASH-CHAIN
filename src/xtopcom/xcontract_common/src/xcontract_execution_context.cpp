...
xtcash_contract_execution_context::xtcash_contract_execution_context(std::unique_ptr<data::xbasic_tcash_action_t const> action, observer_ptr<xcontract_state_t> s) noexcept
  : m_contract_state{s}, m_action{std::move(action)} {
}

observer_ptr<xcontract_state_t> xtcash_contract_execution_context::contract_state() const noexcept {
    assert(m_contract_state != nullptr);
    return m_contract_state;
}

void xtcash_contract_execution_context::contract_state(observer_ptr<xcontract_state_t> new_state) noexcept {
    m_contract_state = new_state;
}

data::xconsensus_action_stage_t xtcash_contract_execution_context::consensus_action_stage() const noexcept {
    return m_stage;
}

void xtcash_contract_execution_context::consensus_action_stage(data::xconsensus_action_stage_t const stage) noexcept {
    m_stage = stage;
}

xcontract_execution_result_t const & xtcash_contract_execution_context::execution_result() const noexcept {
    return m_execution_result;
}

void xtcash_contract_execution_context::add_followup_transaction(data::xcons_transaction_ptr_t tx, xfollowup_transaction_schedule_type_t type) {
    m_execution_result.output.followup_transaction_data.emplace_back(std::move(tx), type);
#if defined(DEBUG)
    xdbg("xtcash_contract_execution_context::add_followup_transaction add followup tx, now size: %zu", m_execution_result.output.followup_transaction_data.size());
...
std::vector<xfollowup_transaction_datum_t> const & xtcash_contract_execution_context::followup_transaction() const noexcept {
    return m_execution_result.output.followup_transaction_data;
}

void xtcash_contract_execution_context::input_receipt_data(data::xreceipt_data_store_t const& receipt_data) {
    m_receipt_data = receipt_data;
}

data::xreceipt_data_store_t& xtcash_contract_execution_context::output_receipt_data() noexcept {
    return m_execution_result.output.receipt_data;
}

xbyte_buffer_t xtcash_contract_execution_context::input_receipt_data(std::string const & key) const {
    return m_receipt_data.receipt_data_item(key);
}

void xtcash_contract_execution_context::remove_input_receipt_data(std::string const & key) {
    m_receipt_data.remove_item(key);
}

//observer_ptr<xbasic_contract_t> xtcash_contract_execution_context::system_contract(common::xaccount_address_t const & address) const noexcept {
//    return m_system_contract(address);
//}

//void xtcash_contract_execution_context::system_contract(xcontract_object_cb_t cb) noexcept {
//    m_system_contract = cb;
//}

data::xaction_consensus_exec_status xtcash_contract_execution_context::action_consensus_result() const noexcept {
    ...
    case data::xtcash_action_type_t::system: {
...
common::xaccount_address_t xtcash_contract_execution_context::sender() const {
    common::xaccount_address_t ret;
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system:
    case data::xtcash_action_type_t::evm:
...
common::xaccount_address_t xtcash_contract_execution_context::recver() const {
    common::xaccount_address_t ret;
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system: {
    ...
    case data::xtcash_action_type_t::user: {
...
common::xaccount_address_t xtcash_contract_execution_context::contract_address() const {
    return recver();
}

data::enum_xtransaction_type xtcash_contract_execution_context::transaction_type() const noexcept {
...
std::string xtcash_contract_execution_context::action_name() const {
...
std::string xtcash_contract_execution_context::source_action_name() const {
    std::string ret;
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system: {
...
std::string xtcash_contract_execution_context::target_action_name() const {
    std::string ret;
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system: {
...
data::enum_xaction_type xtcash_contract_execution_context::action_type() const {
...
data::enum_xaction_type xtcash_contract_execution_context::source_action_type() const {
...
data::enum_xaction_type xtcash_contract_execution_context::target_action_type() const {
    data::enum_xaction_type ret{data::enum_xaction_type::xaction_type_asset_out};
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system: {
...
xbytes_t xtcash_contract_execution_context::action_data() const {
    xbytes_t ret;
    switch (consensus_action_stage()) {
    case data::xconsensus_action_stage_t::send:
    case data::xconsensus_action_stage_t::confirm:
...
xbytes_t xtcash_contract_execution_context::source_action_data() const {
    xbytes_t ret;
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system: {
...
xbytes_t xtcash_contract_execution_context::target_action_data() const {
    xbytes_t ret;
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system: {
...
data::xconsensus_action_stage_t xtcash_contract_execution_context::action_stage() const {
...
uint64_t xtcash_contract_execution_context::deposit() const {
    uint64_t ret{0};
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system: {
...
std::string xtcash_contract_execution_context::digest_hex() const {
    std::string ret{};
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system: {
...
uint64_t xtcash_contract_execution_context::last_nonce() const {
    uint64_t ret{0};
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system: {
...
uint64_t xtcash_contract_execution_context::nonce() const {
    uint64_t ret{0};
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system: {
...
uint256_t xtcash_contract_execution_context::hash() const {
    uint256_t ret{};
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system: {
...
uint32_t xtcash_contract_execution_context::size() const {
    uint32_t ret{0};
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system: {
...
uint32_t xtcash_contract_execution_context::used_tgas() const {
    uint32_t ret{0};
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system: {
...
uint32_t xtcash_contract_execution_context::used_disk() const {
    uint32_t ret{0};
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system: {
...
uint32_t xtcash_contract_execution_context::last_action_send_tx_lock_tgas() const {
    uint32_t ret{0};
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system: {
...
uint32_t xtcash_contract_execution_context::last_action_used_deposit() const {
    uint32_t ret{0};
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system: {
...
uint32_t xtcash_contract_execution_context::last_action_recv_tx_use_send_tx_tgas() const {
    uint32_t ret{0};
    switch (m_action->type()) {
    case data::xtcash_action_type_t::system: {
...
data::enum_xunit_tx_exec_status xtcash_contract_execution_context::last_action_exec_status() const {
...
data::xproperty_asset xtcash_contract_execution_context::asset() const {
    ...
    xdbg("[xtcash_contract_execution_context::asset] parse asset");
    ...
    xdbg("[xtcash_contract_execution_context::asset] asset: %" PRIu64, asset_out.m_amount);
    return asset_out;
}

void xtcash_contract_execution_context::nonce_preprocess(std::error_code & ec) {
    ...
    if (!time_property_exist) {
        auto create_time = contract_state()->time() == 0 ? base::tcash_BEGIN_GMTIME : contract_state()->time();
...
xcontract_execution_fee_t xtcash_contract_execution_context::action_preprocess(std::error_code & ec) {
...
xcontract_execution_fee_t xtcash_contract_execution_context::execute_default_source_action(std::error_code & ec) {
    xassert(sender() == contract_state()->state_account_address());
    xdbg("[xtcash_contract_execution_context::execute_default_source_action] %s to %s", sender().to_string().c_str(), recver().to_string().c_str());

    if (sender().to_string() == sys_contract_zec_reward_addr) {
        xdbg("[xtcash_contract_execution_context::execute_default_source_action] reward contract issue, ignore");
        return {};
    }
    if (base::xvaccount_t::is_contract_address_type(sender().type()) && base::xvaccount_t::is_user_address_type(recver().type())) {
        xdbg("[xtcash_contract_execution_context::execute_default_source_action] contract to user, ignore");
        return {};
    }
...
#endif
    xdbg("[xtcash_contract_execution_context::execute_default_source_action] service_fee to burn: %" PRIu64, service_fee);
    if (service_fee > 0) {
    ...
    if (data::is_sys_contract_address(recver()) && target_action_name() == "nodeJoinNetwork2") {
        contract_state()->deposit(balance_prop, common::xtoken_t{10000000000, common::SYMBOL_tcash_TOKEN});
    }
#endif
    auto const tx_deposit = deposit();
    xdbg("[xtcash_contract_execution_context::execute_default_source_action] deposit: %" PRIu64, tx_deposit);
    auto const asset_ = asset();
    xdbg("[xtcash_contract_execution_context::execute_default_source_action] asset: %" PRIu64, asset_.m_amount);
    auto const balance = contract_state()->balance(balance_prop);
    xdbg("[xtcash_contract_execution_context::execute_default_source_action] balance: %s", evm_common::toBigEndianString(balance).c_str());
    if (balance < asset_.m_amount + tx_deposit) {
        xwarn("[xtcash_contract_execution_context::execute_default_source_action] balance not enough, balance: %s, asset: %" PRIu64 ", deposit: %" PRIu64,
    ...
    if (!data::is_sys_contract_address(sender())) {
        ...
        if (ec) {
            xwarn("[xtcash_contract_execution_context::execute_default_source_action] update_tgas_disk_sender failed, catagory: %s, msg: %s",
                  ec.category().name(),
                  ec.message().c_str());
            return {};
        }
    ...
    if (tx_deposit > 0) {
        xdbg("[xtcash_contract_execution_context::execute_default_source_action] do deposit to lock: %" PRIu64, tx_deposit);
        contract_state()->transfer_internal(balance_prop, lock_balance_prop, tx_deposit);
    }
    xdbg("[xtcash_contract_execution_context::execute_default_source_action] deposit to lock ok: %" PRIu64, tx_deposit);

    return fee_change;
}

xcontract_execution_fee_t xtcash_contract_execution_context::execute_default_target_action(std::error_code & ec) {
    ...
    xdbg("[xtcash_contract_execution_context::execute_default_target_action] %s to %s", sender().to_string().c_str(), recver().to_string().c_str());
    ...
    if (!data::is_sys_contract_address(sender())) {
        ...
        xdbg("[xtcash_contract_execution_context::execute_default_target_action] tx: %s, lock_tgas: %" PRIu32 ", use_send_tx_tgas: %" PRIu32 ", used_deposit: %" PRIu32,
             digest_hex().c_str(),
...
xcontract_execution_fee_t xtcash_contract_execution_context::execute_default_confirm_action(std::error_code & ec) {
    xassert(sender() == contract_state()->state_account_address());
    xdbg("[xtcash_contract_execution_context::execute_default_confirm_action] %s to %s", sender().to_string().c_str(), recver().to_string().c_str());
    ...
    xdbg("[xtcash_contract_execution_context::execute_default_confirm_action] tx: %s, deposit: %u, recv_tx_use_send_tx_tgas: %llu, used_deposit: %u, lock_tgas: %u, status: %d",
         digest_hex().c_str(),
         tx_deposit,
         target_used_tgas,
         used_deposit,
    ...
    xdbg("[xtcash_contract_execution_context::execute_default_confirm_action] tgas out lock_tgas: %" PRIu64, tx_deposit);
    if (tx_deposit > 0) {
        contract_state()->transfer_internal(lock_balance_prop, balance_prop, tx_deposit);
    }
    xdbg("[xtcash_contract_execution_context::execute_default_confirm_action] deposit out lock: %" PRIu64, tx_deposit);
    ...
    if (used_deposit > 0) {
        xdbg("[xtcash_contract_execution_context::execute_default_confirm_action] use deposit for tgas: %" PRIu64 ", burn!", used_deposit);
...
void xtcash_contract_execution_context::update_tgas_disk_sender(bool is_contract, contract_common::xcontract_execution_fee_t & fee_change, std::error_code & ec) {
    // step0: check deposit
    auto const tx_deposit = deposit();
    ...
    if (!data::is_sys_contract_address(sender()) && tx_deposit < min_tx_deposit) {
        xwarn("[xtcash_contract_execution_context::update_tgas_disk_sender] not_enough_deposit, sender: %s, recver: %s, deposit: %" PRIu64 ", min_deposit %" PRIu64,
    ...
    xdbg("[xtcash_contract_execution_context::update_tgas_disk_sender] tx_deposit: %" PRIu64 ", cost_tgas: %" PRIu32 ", cost_disk: %" PRIu32, tx_deposit, tgas_usage, disk_usage);
    // step1: used tgas
    ...
    xdbg("[xtcash_contract_execution_context::update_tgas_disk_sender] tgas_self_usage: %" PRIu64 ", tgas_deposit_usage: %" PRIu64, tgas_self_usage, tgas_deposit_usage);
    // step2: disk
    ...
    xdbg("[xtcash_contract_execution_context::update_tgas_disk_sender] disk_usage: %" PRIu64, disk_usage);
    // step3: frozen tgas
    ...
    xdbg("[xtcash_contract_execution_context::update_tgas_disk_sender] frozen_tgas: %" PRIu64 ", available_tgas: %" PRIu64 ", lock_tgas: %" PRIu64,
         frozen_tgas,
         available_tgas,
         lock_tgas);
    if (is_contract && frozen_tgas > 0) {
        ...
        xdbg("[xtcash_contract_execution_context::update_tgas_disk_sender] tgas to lock_tgas: %" PRIu64, frozen_tgas);
    }
}

void xtcash_contract_execution_context::calc_used_tgas(uint64_t deposit, uint64_t & cur_tgas_usage, uint64_t & deposit_usage, std::error_code & ec) const {
    xdbg("[xtcash_contract_execution_context::calc_used_tgas] last_hour: %" PRIu64 ", timer_height: %" PRIu64 ", no decay used_tgas: %" PRIu64 ", decayed used_tgas: %" PRIu64
    ...
    if (cur_tgas_usage > (available_tgas + deposit / XGET_ONCHAIN_GOVERNANCE_PARAMETER(tx_deposit_gas_exchange_ratio))) {
        xwarn("[xtcash_contract_execution_context::calc_used_tgas] not enough pledge token tgas, cur_tgas_usage: %" PRIu64 ", available_tgas: %" PRIu64 ", deposit: %" PRIu64,
              cur_tgas_usage,
              available_tgas,
              deposit);
        ...
        cur_tgas_usage = available_tgas;
        xdbg("[xtcash_contract_execution_context::calc_used_tgas] available_tgas: %" PRIu64 ", cur_tgas_usage: %" PRIu64 ", tx_deposit_usage: %" PRIu64 ", deposit: %" PRIu64,
             available_tgas,
             deposit_usage,
             deposit);
    }
...
uint64_t xtcash_contract_execution_context::calc_available_tgas() const {
...
uint64_t xtcash_contract_execution_context::calc_total_tgas() const {
    uint64_t max_tgas{0};
    ...
    auto token_price = calc_token_price();
    auto total_tgas = pledge_token * token_price / tcash_UNIT + calc_free_tgas();
...
uint64_t xtcash_contract_execution_context::calc_free_tgas() const {
...
uint64_t xtcash_contract_execution_context::calc_decayed_tgas() const {
...
uint64_t xtcash_contract_execution_context::calc_cost_tgas(bool is_contract) const {
#ifdef ENABLE_SCALE
    uint16_t amplify = 5;
#else
    uint16_t amplify = 1;
...
uint64_t xtcash_contract_execution_context::calc_cost_disk(bool is_contract) const {
#ifdef ENABLE_SCALE
...
void xtcash_contract_execution_context::incr_used_tgas(uint64_t num, std::error_code & ec) {
    ...
    xdbg("[xtcash_contract_execution_context::incr_used_tgas] last_hour_set: %" PRIu64 ", tgas_used this time: %" PRIu64 ", decayed: %" PRIu64 ", total: %" PRIu64,
         time,
         num,
         decayed_tgas,
         num + decayed_tgas);
...
uint64_t xtcash_contract_execution_context::calc_token_price() const {
    ...
    auto token_price = XGET_ONCHAIN_GOVERNANCE_PARAMETER(total_gas_shard) * XGET_CONFIG(validator_group_count) * tcash_UNIT / total_pledge_token;
    xdbg("[xtcash_contract_execution_context::calc_token_price] get total pledge token from beacon: %" PRIu64 " + %" PRIu64 " = %" PRIu64 ", price: %" PRIu64,
