...
xtcash_string_property::xtcash_string_property(std::string const & name, xcontract_face_t * contract)
    : xbasic_property_t{name, state_accessor::properties::xproperty_type_t::string, make_observer(contract)} {
}

xtcash_string_property::xtcash_string_property(std::string const & name, std::unique_ptr<xcontract_state_t> state_owned) : xbasic_property_t {name, state_accessor::properties::xproperty_type_t::string, std::move(state_owned)} {
}

void xtcash_string_property::set(std::string const & value) {
...
void xtcash_string_property::clear() {
    assert(associated_state() != nullptr);
    associated_state()->clear_property(id());
}

std::string xtcash_string_property::value() const {
    assert(associated_state() != nullptr);
    return associated_state()->xcontract_state_t::get_property<state_accessor::properties::xproperty_type_t::string>(typeless_id());
}

//std::string xtcash_string_property::value(common::xaccount_address_t const & contract) const {
...
size_t xtcash_string_property::size() const {
    assert(associated_state() != nullptr);
    return associated_state()->property_size(id());
}

bool xtcash_string_property::empty() const {
    return size() == 0;
}
