...
static state_accessor::properties::xproperty_category_t lookup_property_category(std::string const & name,
    ...
    do {
        ...
        auto const & inner_dict = tcash::get<std::unordered_map<state_accessor::properties::xproperty_type_t, state_accessor::properties::xproperty_category_t>>(*it);
        auto const inner_it = inner_dict.find(type);
        if (inner_it == std::end(inner_dict)) {
            break;
        }

        property_category = tcash::get<state_accessor::properties::xproperty_category_t>(*inner_it);
    } while (false);

    return property_category;
}

xtcash_basic_property::xtcash_basic_property(std::string const & name, state_accessor::properties::xproperty_type_t type, observer_ptr<xcontract_face_t> associated_contract) noexcept
...
xtcash_basic_property::xtcash_basic_property(std::string const & name, state_accessor::properties::xproperty_type_t type, std::unique_ptr<xcontract_state_t> state_owned)
...
void xtcash_basic_property::initialize() {
    assert(m_associated_contract != nullptr);
...
state_accessor::properties::xproperty_identifier_t const & xtcash_basic_property::id() const {
    return m_id;
}

state_accessor::properties::xtypeless_property_identifier_t xtcash_basic_property::typeless_id() const {
    return static_cast<state_accessor::properties::xtypeless_property_identifier_t>(m_id);
}

common::xaccount_address_t xtcash_basic_property::owner() const {
    return m_owner;
}

common::xaccount_address_t xtcash_basic_property::accessor() const {
    assert(m_associated_contract != nullptr);
    return m_associated_contract->contract_state()->state_account_address();
}

observer_ptr<xcontract_state_t const> xtcash_basic_property::associated_state() const noexcept {
...
observer_ptr<xcontract_state_t> xtcash_basic_property::associated_state() noexcept {
    if (m_associated_contract != nullptr) {
        auto assoc_state = m_associated_contract->contract_state();
        if (assoc_state != nullptr) {
            return assoc_state;
