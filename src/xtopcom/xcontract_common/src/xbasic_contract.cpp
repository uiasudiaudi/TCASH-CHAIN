...
xtcash_contract_metadata::xtcash_contract_metadata(common::xaccount_address_t const& account, xcontract_type_t type)
    : m_type(type), m_account(account) {
}

common::xaccount_address_t xtcash_basic_contract::address() const {
...
xcontract_type_t xtcash_basic_contract::type() const {
    return m_contract_meta.m_type;
}

void xtcash_basic_contract::register_property(properties::xbasic_property_t * property) {
    m_property_initializer.register_property(make_observer(property));
}

common::xnetwork_id_t xtcash_basic_contract::network_id() const {
    return common::xnetwork_id_t{config::to_chainid(XGET_CONFIG(chain_name))};
}

evm_common::u256 xtcash_basic_contract::balance() const {
    return m_balance.amount();
}

common::xtoken_t xtcash_basic_contract::withdraw(evm_common::u256 amount) {
    return m_balance.withdraw(amount);
}

void xtcash_basic_contract::deposit(common::xtoken_t token) {
    assert(m_balance.symbol() == token.symbol());
    m_balance.deposit(std::move(token));
}

observer_ptr<xcontract_state_t> xtcash_basic_contract::contract_state() const noexcept {
    assert(m_associated_execution_context != nullptr);
    return m_associated_execution_context->contract_state();
}

void xtcash_basic_contract::asset_to_next_action(common::xtoken_t token) {
...
data::enum_xaction_type xtcash_basic_contract::action_type() const {
    return m_associated_execution_context->action_type();
}

data::enum_xaction_type xtcash_basic_contract::source_action_type() const {
    return m_associated_execution_context->source_action_type();
}

data::enum_xaction_type xtcash_basic_contract::target_action_type() const {
    return m_associated_execution_context->target_action_type();
}

xbyte_buffer_t xtcash_basic_contract::action_data() const {
    return m_associated_execution_context->action_data();
}

common::xtoken_t xtcash_basic_contract::last_action_asset(std::error_code & ec) const {
    assert(!ec);
...
data::enum_xtransaction_type xtcash_basic_contract::transaction_type() const {
    return m_associated_execution_context->transaction_type();
}

common::xaccount_address_t xtcash_basic_contract::sender() const {
    return m_associated_execution_context->sender();
}

common::xaccount_address_t xtcash_basic_contract::recver() const {
    return m_associated_execution_context->recver();
}

void xtcash_basic_contract::call(common::xaccount_address_t const & target_addr,
    ...
    if (type == xfollowup_transaction_schedule_type_t::immediately) {
        ...
        xdbg_info("xtcash_basic_contract::call tx:%s,from:%s,to:%s,amount:%ld,nonce:%ld",
...
void xtcash_basic_contract::sync_call(common::xaccount_address_t const & target_addr,
                                    std::string const & method_name,
                                    std::string const & method_params,
                                    xfollowup_transaction_schedule_type_t type) {
    ...
    tx->set_different_source_target_address(address().to_string(), target_addr.to_string());
    cons_tx = make_object_ptr<data::xcons_transaction_t>(tx.get());
    std::unique_ptr<data::xbasic_tcash_action_t const> action = tcash::make_unique<data::xsystem_consensus_action_t>(cons_tx);
    // exec
    // auto obj = m_associated_execution_context->system_contract(target_addr);
    auto ctx = tcash::make_unique<contract_common::xcontract_execution_context_t>(std::move(action), contract_state());
    ...
    // auto result = obj->execute(tcash::make_observer(ctx));
...
void xtcash_basic_contract::transfer(common::xaccount_address_t const & target_addr, uint64_t amount, xfollowup_transaction_schedule_type_t type, std::error_code & ec) {
    ...
    //    xwarn("xtcash_basic_contract::transfer fail to create user contract transaction from:%s,to:%s,amount:%lu", address().value().c_str(), target_addr.value().c_str(), amount);
    ...
    if (type == xfollowup_transaction_schedule_type_t::immediately) {
        ...
        contract_state()->latest_followup_tx_hash(cons_tx->get_tx_hash_256());
        contract_state()->latest_followup_tx_nonce(cons_tx->get_tx_nonce());
        xdbg_info("xtcash_basic_contract::transfer tx:%s,from:%s,to:%s,amount:%ld,nonce:%ld",
                  tx->get_digest_hex_str().c_str(),
                  address().to_string().c_str(),
                  target_addr.to_string().c_str(),
                  amount,
...
void xtcash_basic_contract::transfer(common::xaccount_address_t const & target_addr, uint64_t amount, xfollowup_transaction_schedule_type_t type) {
    std::error_code ec;
    transfer(target_addr, amount, type, ec);
    tcash::error::throw_error(ec);
}

void xtcash_basic_contract::delay_followup(xfollowup_transaction_delay_param_t const & param) {
    ...
    if (task.action == data::system_contract::XTRANSFER_ACTION) {
        ...
        base::xstream_t stream(base::xcontext_t::instance());
        map.emplace(task.contract, tcash::from_string<uint64_t>(task.params));
        stream << map;
...
void xtcash_basic_contract::delay_followup(std::vector<xfollowup_transaction_delay_param_t> const & params) {
    ...
    for (auto const & param : params) {
        ...
        if (task.action == data::system_contract::XTRANSFER_ACTION) {
            std::map<std::string, uint64_t> map;
            base::xstream_t stream(base::xcontext_t::instance());
            map.emplace(task.contract, tcash::from_string<uint64_t>(param.method_params));
            stream << map;
            task.params = std::string((char *)stream.data(), stream.size());
        } else {
            task.params = param.method_params;
...
void xtcash_basic_contract::exec_delay_followup() {
    ...
    xinfo("[xtcash_basic_contract::exec_delay_followup] tasks size: %zu, task_num_per_round: %u", tasks.size(), task_num_per_round);
    ...
    for (size_t i = 0; i < task_num_per_round; i++) {
        ...
        if (task.action == data::system_contract::XREWARD_CLAIMING_ADD_NODE_REWARD || task.action == data::system_contract::XREWARD_CLAIMING_ADD_VOTER_DIVIDEND_REWARD) {
            ...
            for (auto const & issue : issuances) {
                xinfo("[xtcash_basic_contract::exec_delay_followup] action: %s, contract account: %s, issuance: %llu, onchain_timer_round: %llu",
                      task.action.c_str(),
                      issue.first.c_str(),
                      issue.second,
                      task.onchain_timer_round);
...
std::vector<xfollowup_transaction_datum_t> xtcash_basic_contract::followup_transaction() const {
    return m_associated_execution_context->followup_transaction();
}

void xtcash_basic_contract::reset_execution_context(observer_ptr<xcontract_execution_context_t> exe_ctx) {
    m_associated_execution_context = exe_ctx;
}

observer_ptr<properties::xproperty_initializer_t const> xtcash_basic_contract::property_initializer() const noexcept {
    return make_observer(std::addressof(m_property_initializer));
}

xbyte_buffer_t xtcash_basic_contract::receipt_data(std::string const & key) const {
    return m_associated_execution_context->input_receipt_data(key);
}

void xtcash_basic_contract::write_receipt_data(std::string const & key, xbyte_buffer_t value, std::error_code & ec) {
...
common::xlogic_time_t xtcash_basic_contract::time() const {
    return m_associated_execution_context->contract_state()->time();
}

common::xlogic_time_t xtcash_basic_contract::timestamp() const {
    return m_associated_execution_context->contract_state()->timestamp();
}

std::string const & xtcash_basic_contract::random_seed() const noexcept {
    return m_associated_execution_context->contract_state()->random_seed();
}

uint64_t xtcash_basic_contract::height() const {
