...
xtcash_http_server_base::~xtcash_http_server_base() {
    m_server->_.stcash();
}

xtcash_http_server_base::xtcash_http_server_base(std::string const & server_ip, uint16_t server_port) {
    m_server = std::make_shared<HttpServerWrapper>(server_ip, server_port);
}

bool xtcash_http_server_base::register_json_request_callback(std::string const & path,
...
void xtcash_http_server_base::start_server(bool block) {
    if (block) {
        m_server->_.start();
    } else {
