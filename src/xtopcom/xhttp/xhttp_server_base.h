...
enum class xtcash_http_server_method_type {
    no_support = 0,
    GET = 1,
    POST = 2,
};
using xhttp_server_method_type_t = xtcash_http_server_method_type;

/// @brief http server base
class xtcash_http_server_base : public std::enable_shared_from_this<xtcash_http_server_base> {
public:
    xtcash_http_server_base() = delete;
    virtual ~xtcash_http_server_base();

    xtcash_http_server_base(std::string const & server_ip, uint16_t server_port);
...
using xhttp_server_base_t = xtcash_http_server_base;
