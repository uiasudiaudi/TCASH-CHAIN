...
xtcash_https_client_base::xtcash_https_client_base(std::string const & url) {
...
xtcash_https_client_base::~xtcash_https_client_base() {
    m_client->_.stcash();
}

std::string xtcash_https_client_base::request_get() {
    auto res = m_client->_.request("GET", default_request_path);
    return res->content.string();
}

std::string xtcash_https_client_base::request_get(std::string const & path) {
    auto res = m_client->_.request("GET", path);
    return res->content.string();
}

std::string xtcash_https_client_base::request_post_string(std::string const & path, std::string const & request) {
    auto res = m_client->_.request("POST", path, request);
    return res->content.string();
}

std::string xtcash_https_client_base::request_post_json(std::string const & path, std::string const & json_request) {
