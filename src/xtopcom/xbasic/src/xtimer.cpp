...
xtcash_timer::xtcash_timer(std::shared_ptr<xasio_io_context_wrapper_t> io_object,
...
xtcash_timer::~xtcash_timer() noexcept {
    std::error_code ec;
    m_timer.cancel(ec);
}

bool
xtcash_timer::expired() const noexcept {
...
xtcash_base_timer_wrapper::xtcash_base_timer_wrapper(observer_ptr<xbase_io_context_wrapper_t> io_context, std::chrono::milliseconds ms_in_future, timeout_callback_t callback)
    : base_t(io_context->context(), io_context->thread_id()), m_callback{std::move(callback)}, m_timeout{ms_in_future} {
}

bool xtcash_base_timer_wrapper::expired() const noexcept {
    return !const_cast<xtcash_base_timer_wrapper *>(this)->is_active();
}

void xtcash_base_timer_wrapper::start() {
    assert(!this->running());
    this->running(true);
    base_t::start(m_timeout.count(), 0);
}

void xtcash_base_timer_wrapper::stcash() {
    base_t::stcash();
    close();
    assert(this->running());
    this->running(false);
...
bool xtcash_base_timer_wrapper::on_timer_fire(int32_t const thread_id,
...
xtcash_base_timer::xtcash_base_timer(observer_ptr<xbase_io_context_wrapper_t> io_object, std::chrono::milliseconds ms_in_future, timeout_callback_t callback)
  : m_timer{make_object_ptr<xbase_timer_wrapper_t>(std::move(io_object), std::move(ms_in_future), std::move(callback))} {
    assert(io_object);
    m_timer->start();
}

xtcash_base_timer::~xtcash_base_timer() noexcept {
    m_timer->stcash();
}

bool xtcash_base_timer::expired() const noexcept {
    std::lock_guard<std::mutex> lock{m_timer_mutex};
    return m_timer->expired();
}

void xtcash_base_timer::wait() {
