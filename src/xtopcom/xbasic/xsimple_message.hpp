...
class xtcash_simple_message final : public xhashable_t<xtcash_simple_message<MessageT>>, public xstatistic::xstatistic_obj_face_t {
    XSTATIC_ASSERT(std::is_enum<MessageT>::value);

public:
    using message_type = MessageT;
    using hash_result_type = typename xhashable_t<xtcash_simple_message<MessageT>>::hash_result_type;
...
public:
    xtcash_simple_message() : xstatistic::xstatistic_obj_face_t(xstatistic::enum_statistic_undetermined) {}
    xtcash_simple_message(xtcash_simple_message const &) = default;
    xtcash_simple_message & operator=(xtcash_simple_message const & obj) {
    ...
    xtcash_simple_message(xtcash_simple_message && obj) : xstatistic::xstatistic_obj_face_t(xstatistic::message_id_to_class_type((uint32_t)obj.id())) {
        // xdbg("xtcash_simple_message move construct this id:%u, obj id:%u", (uint32_t)m_id_or_type, (uint32_t)obj.id());
        m_payload = obj.m_payload;
        m_id_or_type = obj.m_id_or_type;
    }
    // xtcash_simple_message & operator=(xtcash_simple_message &&) = default;

    xtcash_simple_message(xbyte_buffer_t msg_payload, message_type const id_or_type)
    ...
    ~xtcash_simple_message() {statistic_del();}

    bool operator==(xtcash_simple_message const & other) const noexcept {
...
template <typename MessageT>
using xsimple_message_t = xtcash_simple_message<MessageT>;
