...
T from_bytes(xbytes_t const & input) {
    std::error_code ec;
    auto ret = tcash::from_bytes<T>(input, ec);
...
class xtcash_bytes_helper {
    xbytes_t const & bytes_;

public:
    xtcash_bytes_helper(xtcash_bytes_helper const &) = delete;
    xtcash_bytes_helper & operator=(xtcash_bytes_helper const &) = delete;
    xtcash_bytes_helper(xtcash_bytes_helper &&) = delete;
    xtcash_bytes_helper & operator=(xtcash_bytes_helper &&) = delete;
    ~xtcash_bytes_helper() = default;

    explicit xtcash_bytes_helper(xbytes_t const & bytes) noexcept;

    bool start_with(xbytes_t const & prefix) const noexcept;
};
using xbytes_helper_t = xtcash_bytes_helper;
