...
struct xtcash_aligned_object final {
    XSTATIC_ASSERT(sizeof(T) <= alignment);
    alignas(alignment) T payload;
};

template <typename T, std::size_t alignment>
using xaligned_object_t = xtcash_aligned_object<T, alignment>;
...
template<typename T>
struct xtcash_make_unique
{
    using single_object = std::unique_ptr<T>;
};

template<typename T>
struct xtcash_make_unique<T[]>
{
    using array_object = std::unique_ptr<T[]>;
};

template<typename T, size_t Bound>
struct xtcash_make_unique<T[Bound]>
{
    struct invalid_type {};
};

// tcash::make_unique for single objects
template<typename T, typename ... ArgsT>
typename xtcash_make_unique<T>::single_object
make_unique(ArgsT && ... args) {
    return std::unique_ptr<T>(new T{ std::forward<ArgsT>(args)... });
}

// tcash::make_unique for arrays of unknown bound
template<typename T>
typename xtcash_make_unique<T>::array_object
make_unique(std::size_t const n) {
    return std::unique_ptr<T>(new typename std::remove_extent<T>::type[n]{});
}

// disable tcash::make_unique for arrays of known bound
template<typename T, typename... ArgsT>
typename xtcash_make_unique<T>::invalid_type
