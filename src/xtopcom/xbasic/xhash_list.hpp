...
public:
    ...
    void
    swap(xtcash_hash_list & other) noexcept {
    ...
    push_back(std::pair<KeyT, T> const & value) {
        auto const map_iterator = m_map.find(tcash::get<key_type>(value));
        if (map_iterator != std::end(m_map)) {
            auto const lit = tcash::get<list_iterator>(*map_iterator);
        ...
        m_map[tcash::get<key_type>(value)] = std::prev(m_list.end());
    }
    ...
    push_back(std::pair<KeyT, T> && value) {
        ...
        auto key = tcash::get<key_type>(value);
    ...
    push_front(std::pair<KeyT, T> const & value) {
        ...
        m_map[tcash::get<key_type>(m_list.front())] = m_list.begin();
    }
    ...
    void
    pop_back() {
        m_map.erase(tcash::get<key_type const>(m_list.back()));
        m_list.pop_back();
    }

    void
    pop_front() {
        m_map.erase(tcash::get<key_type const>(m_list.front()));
        m_list.pop_front();
    }
    ...
    erase(key_type const & key) {
        ...
        auto const lci = tcash::get<list_iterator>(*map_iterator);
    ...
    at(key_type const & key, xhash_list_update_direction_t const direction) {
        ...
        if (map_iterator != std::end(m_map)) {
            ...
            value = tcash::get<mapped_type>(std::move(*li));
        ...
        switch (direction) {
            case xhash_list_update_direction_t::to_back:
                ...
                m_map[tcash::get<key_type const>(m_list.back())] = std::prev(m_list.end());
                return tcash::get<mapped_type>(m_list.back());

            case xhash_list_update_direction_t::to_front:
                m_list.push_front({ key, std::move(value) });
                m_map[tcash::get<key_type const>(m_list.front())] = m_list.begin();
                return tcash::get<mapped_type>(m_list.front());
...
using xhash_list_t = xtcash_hash_list<KeyT, ValueT>;


template <typename KeyT, typename T>
class xtcash_hash_list2 final
{
private:
...
public:
    ...
    xtcash_hash_list2(xtcash_hash_list2 const &)             = delete;
    xtcash_hash_list2 & operator=(xtcash_hash_list2 const &) = delete;
    xtcash_hash_list2(xtcash_hash_list2 &&)                  = default;
    xtcash_hash_list2 & operator=(xtcash_hash_list2 &&)      = default;
    ~xtcash_hash_list2()                                   = default;

    explicit
    xtcash_hash_list2(std::size_t const size_limit)
        : m_size_limit{ size_limit } {
    }

    void
    swap(xtcash_hash_list2 & other) noexcept {
        m_list.swap(other.m_list);
        m_map.swap(other.m_map);
    }
...
template <typename KeyT, typename ValueT>
using xhash_list2_t = xtcash_hash_list2<KeyT, ValueT>;
NS_END1
