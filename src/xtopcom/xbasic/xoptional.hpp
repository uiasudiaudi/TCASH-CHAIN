...
public:
    ...
    optional(optional const & other)
        : m_value{ static_cast<bool>(other) ? tcash::make_unique<T>(*other) : nullptr }
    {
    }
    ...
    optional(U && v) : m_value{ tcash::make_unique<T>(std::forward<U>(v)) }
    {
    }
    ...
    operator=(optional const & other) {
        if (this != &other) {
            if (static_cast<bool>(other)) {
                m_value = tcash::make_unique<T>(*other);
            } else {
                m_value.reset();
            }
        }
    ...
    operator=(U && v) {
       if (m_value == nullptr) {
           m_value = tcash::make_unique<U>(std::move(v));
       } else {
           *m_value = std::move(v);
       }
