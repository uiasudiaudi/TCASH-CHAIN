...
bool xbatch_packer::do_state_sync(uint64_t sync_height) {
    ...
    xh256_t const table_bstate_hash(tcash::to_bytes(table_bstate_hash_str));
    xh256_t const sync_block_hash(tcash::to_bytes(sync_block->get_block_hash()));
...
bool  xbatch_packer::on_timer_stcash(const int32_t errorcode, const int32_t thread_id, const int64_t timer_id, const int64_t cur_time_ms, const int32_t timeout_ms, const int32_t timer_repeat_ms) {
    // xunit_dbg("xbatch_packer::on_timer_stcash,this=%p", this);
    return true;
}
...
bool  xbatch_packer::on_replicate_finish(const base::xvevent_t & event,xcsobject_t* from_child,const int32_t cur_thread_id,const uint64_t timenow_ms)  //call from lower layer to higher layer(parent)
{
    ...
    return true; //stcash handle anymore
}
