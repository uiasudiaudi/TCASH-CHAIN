...
xcons_service_mgr_ptr xcons_mgr_build(std::string const & node_account,
    ...
    auto work_pool = make_object_ptr<base::xworkerpool_t_impl<5>>(tcash::base::xcontext_t::instance());
    // not set work pool to bft, so that bft always directly call in one thread.
    // auto xbft_work_pool = make_object_ptr<base::xworkerpool_t_impl<3>>(tcash::base::xcontext_t::instance());
