...
bool xnetwork_proxy::send_out(common::xmessage_id_t const & id, const xvip2_t & from_addr, const xvip2_t & to_addr, base::xstream_t & stream, const std::string & account) {
    ...
    tcash::vnetwork::xmessage_t msg({stream.data(), stream.data() + stream.size()}, id);
...
void xnetwork_proxy::on_message(tcash::vnetwork::xvnode_address_t const & sender, tcash::vnetwork::xvnode_address_t const & receiver, tcash::vnetwork::xmessage_t const & message) {
    ...
    if (cb != nullptr) {
        base::xstream_t stream(tcash::base::xcontext_t::instance(), (uint8_t *)(message.payload().data()), (uint32_t)message.payload().size());
...
void xnetwork_proxy::send_receipt_msg(std::shared_ptr<vnetwork::xvnetwork_driver_face_t> net_driver,
    ...
    try {
        ...
        tcash::base::xautostream_t<4096> stream(tcash::base::xcontext_t::instance());
