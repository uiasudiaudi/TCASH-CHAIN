...
bool xstatestore_impl_t::start(const xobject_ptr_t<base::xiothread_t> & iothread, const xobject_ptr_t<base::xiothread_t> & iothread_for_prune) {
    ...
    m_started = true;
    m_prune_dispather = make_object_ptr<statestore_prune_dispatcher_t>(tcash::base::xcontext_t::instance(), iothread_for_prune->get_thread_id());
    m_para->set_prune_dispatcher(make_observer(m_prune_dispather.get()));

    // todo(nathan):use timer to update mpt and table state.
    m_timer = new xstatestore_timer_t(tcash::base::xcontext_t::instance(), iothread->get_thread_id(), this);
    m_timer->start(0, 1000);
    m_store_block_listen_id = get_mbus()->add_listener(tcash::mbus::xevent_major_type_store, std::bind(&xstatestore_impl_t::on_block_to_db_event, this, std::placeholders::_1));
    m_state_sync_listen_id = get_mbus()->add_listener(tcash::mbus::xevent_major_type_state_sync, std::bind(&xstatestore_impl_t::on_state_sync_event, this, std::placeholders::_1));
    return false;
}
...
void xstatestore_impl_t::on_state_sync_result(mbus::xevent_state_sync_ptr_t state_sync_event) {
    ...
    xstate_sync_info_t sync_info(state_sync_event->height,state_sync_event->root_hash,state_sync_event->table_state_hash, tcash::to_string(state_sync_event->table_block_hash.to_bytes()));
...
std::vector<std::pair<common::xaccount_address_t, base::xaccount_index_t>> xstatestore_impl_t::get_all_accountindex(base::xvblock_t * table_block, std::error_code & ec) const {
    ...
    if (state_root.empty()) {
        ...
        auto const & leafs = tcash::evm_common::trie::xtrie_simple_iterator_t::trie_leafs(state_root, make_observer(xtrie_db));
        for (auto & leaf : leafs) {
