...
#ifndef CRYPtcashP_QUEUE_H
#define CRYPtcashP_QUEUE_H
...
class CRYPtcashP_DLL ByteQueue : public Bufferless<BufferedTransformation>
{
public:
...
class CRYPtcashP_DLL LazyPutter
{
public:
	LazyPutter(ByteQueue &bq, const byte *inString, size_t size)
		: m_bq(bq) {bq.LazyPut(inString, size);}
	~LazyPutter()
		{try {m_bq.FinalizeLazyPut();} catch(const Exception&) {CRYPtcashP_ASSERT(0);}}
protected:
...
#ifndef __BORLANDC__
NAMESPACE_BEGIN(std)
template<> inline void swap(CryptcashP::ByteQueue &a, CryptcashP::ByteQueue &b)
{
	a.swap(b);
}
NAMESPACE_END
