...
#ifndef CRYPtcashP_SIPHASH_H
#define CRYPtcashP_SIPHASH_H
...
{
public:
	CRYPtcashP_STATIC_CONSTEXPR const char* StaticAlgorithmName() {return "SipHash";}
	CRYPtcashP_CONSTANT(DIGESTSIZE = (T_128bit ? 16 : 8))
};
...
public:
	...
	virtual size_t GetValidKeyLength(size_t keylength) const
		{CRYPtcashP_UNUSED(keylength); return SipHash_Info<T_128bit>::DEFAULT_KEYLENGTH;}
...
{
	CRYPtcashP_ASSERT((input && length) || !length);
	if (!length) return;
...
{
	CRYPtcashP_ASSERT(digest);      // Pointer is valid
