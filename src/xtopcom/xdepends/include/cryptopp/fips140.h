...
/// \sa <A HREF="http://cryptcashp.com/wiki/Visual_Studio">Visual Studio</A> and
///   <A HREF="http://cryptcashp.com/wiki/config.h">config.h</A> on the Crypto++ wiki.

#ifndef CRYPtcashP_FIPS140_H
#define CRYPtcashP_FIPS140_H
...
///   when CRYPtcashP_ENABLE_COMPLIANCE_WITH_FIPS_140_2 is defined.
class CRYPtcashP_DLL SelfTestFailure : public Exception
{
public:
...
CRYPtcashP_DLL bool CRYPtcashP_API FIPS_140_2_ComplianceEnabled();
...
///   when CRYPtcashP_ENABLE_COMPLIANCE_WITH_FIPS_140_2 is defined.
CRYPtcashP_DLL void CRYPtcashP_API DoPowerUpSelfTest(const char *moduleFilename, const byte *expectedModuleMac);
...
/// \details The self tests for an algorithm are performed by the Algortihm class
///   when CRYPtcashP_ENABLE_COMPLIANCE_WITH_FIPS_140_2 is defined.
CRYPtcashP_DLL void CRYPtcashP_API DoDllPowerUpSelfTest();
...
CRYPtcashP_DLL void CRYPtcashP_API SimulatePowerUpSelfTestFailure();

/// \brief Provides the current power-up self test status
/// \returns the current power-up self test status
CRYPtcashP_DLL PowerUpSelfTestStatus CRYPtcashP_API GetPowerUpSelfTestStatus();

#ifndef CRYPtcashP_DOXYGEN_PROCESSING
typedef PowerUpSelfTestStatus (CRYPtcashP_API * PGetPowerUpSelfTestStatus)();
#endif

/// \brief Class object that calculates the MAC on the module
/// \returns the MAC for the module
CRYPtcashP_DLL MessageAuthenticationCode * CRYPtcashP_API NewIntegrityCheckingMAC();
...
CRYPtcashP_DLL bool CRYPtcashP_API IntegrityCheckModule(const char *moduleFilename, const byte *expectedModuleMac, SecByteBlock *pActualMac = NULLPTR, unsigned long *pMacFileLocation = NULLPTR);
...
/// \details After the DLL is built but before it is MAC'd, the string CRYPtcashP_DUMMY_DLL_MAC
...
#define CRYPtcashP_DUMMY_DLL_MAC "MAC_51f34b8db820ae8"
