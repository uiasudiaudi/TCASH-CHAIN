...
#ifndef CRYPtcashP_SECBLOCK_H
#define CRYPtcashP_SECBLOCK_H
...
public:
	...
	void destroy(pointer p) {CRYPtcashP_UNUSED(p); p->~T();}
	...
	/// \sa <A HREF="http://github.com/weidai11/cryptcashp/issues/346">Issue 346/CVE-2016-9939</A>
	/// \since Crypto++ 6.0
#if defined(CRYPtcashP_DOXYGEN_PROCESSING)
	static const size_type ELEMS_MAX = ...;
#elif defined(_MSC_VER) && (_MSC_VER <= 1400)
	static const size_type ELEMS_MAX = (~(size_type)0)/sizeof(T);
#elif defined(CRYPtcashP_CXX11_ENUM)
	...
	CRYPtcashP_CONSTEXPR size_type max_size() const {return ELEMS_MAX;}

#if defined(CRYPtcashP_CXX11_VARIADIC_TEMPLATES) || defined(CRYPtcashP_DOXYGEN_PROCESSING)
	...
	/// \details This is a C++11 feature. It is available when CRYPtcashP_CXX11_VARIADIC_TEMPLATES
...
#define CRYPtcashP_INHERIT_ALLOCATOR_TYPES	\
...
{
	CRYPtcashP_ASSERT((oldPtr && oldSize) || !(oldPtr || oldSize));
	if (oldSize == newSize)
		return oldPtr;
...
///    CRYPtcashP_BOOL_ALIGN16. CRYPtcashP_BOOL_ALIGN16 is often used as the template parameter.
template <class T, bool T_Align16 = false>
class AllocatorWithCleanup : public AllocatorBase<T>
{
public:
	CRYPtcashP_INHERIT_ALLOCATOR_TYPES
	...
	void deallocate(void *ptr, size_type size)
	{
		...
		// CRYPtcashP_ASSERT((ptr && size) || !(ptr || size));
...
CRYPtcashP_DLL_TEMPLATE_CLASS AllocatorWithCleanup<byte>;
CRYPtcashP_DLL_TEMPLATE_CLASS AllocatorWithCleanup<word16>;
CRYPtcashP_DLL_TEMPLATE_CLASS AllocatorWithCleanup<word32>;
CRYPtcashP_DLL_TEMPLATE_CLASS AllocatorWithCleanup<word64>;
#if defined(CRYPtcashP_WORD128_AVAILABLE)
CRYPtcashP_DLL_TEMPLATE_CLASS AllocatorWithCleanup<word128, true>; // for Integer
#endif
#if CRYPtcashP_BOOL_X86
CRYPtcashP_DLL_TEMPLATE_CLASS AllocatorWithCleanup<word, true>;	 // for Integer
#endif
...
public:
	...
	{
		CRYPtcashP_UNUSED(n); CRYPtcashP_UNUSED(unused);
		CRYPtcashP_ASSERT(false); return NULLPTR;
	}

	void deallocate(void *p, size_type n)
	{
		CRYPtcashP_UNUSED(p); CRYPtcashP_UNUSED(n);
		CRYPtcashP_ASSERT(false);
	}

	CRYPtcashP_CONSTEXPR size_type max_size() const {return 0;}
	//LCOV_EXCL_Stcash
};
...
public:
	...
	{
		CRYPtcashP_ASSERT(IsAlignedOn(m_array, 8));
	...
	void deallocate(void *ptr, size_type size)
	{
		if (ptr == GetAlignedArray())
		{
			...
			// Be sure to check CRYPtcashP_ALIGN_DATA(8). The platform may
			// not have a way to declaritively align data to 8.
			CRYPtcashP_ASSERT(size <= S);
			CRYPtcashP_ASSERT(m_allocated);
			m_allocated = false;
	...
	CRYPtcashP_CONSTEXPR size_type max_size() const
	{
...
private:
...
#if defined(CRYPtcashP_BOOL_ALIGN16) && (defined(_M_X64) || defined(__x86_64__))
	...
	// CRYPtcashP_ALIGN_DATA(16) is known OK on Linux, OS X, Solaris.
	// Also see http://stackoverflow.com/a/1468656/608639.
	T* GetAlignedArray() {
		CRYPtcashP_ASSERT(IsAlignedOn(m_array, 16));
		return m_array;
	}
	CRYPtcashP_ALIGN_DATA(16) T m_array[S];

#elif defined(CRYPtcashP_BOOL_ALIGN16)

	// There be demons here... We cannot use CRYPtcashP_ALIGN_DATA(16)
	...
	// CRYPtcashP_ALIGN_DATA(8). Also see
	...
	T* GetAlignedArray() {
		...
		CRYPtcashP_ASSERT(IsAlignedOn(p_array, 16));
		// Verify allocated array with pad is large enough.
		CRYPtcashP_ASSERT(p_array+S <= m_array+(S+PAD));
		return p_array;
	}
...
#   if defined(_AIX)
	...
	CRYPtcashP_ALIGN_DATA(8) T m_array[S+PAD];
#   else
	// PAD is elements, not bytes, and rounded up to ensure no overflow.
...
#   endif
	...
	// CRYPtcashP_BOOL_ALIGN16 is 0. Use natural alignment of T.
	T* GetAlignedArray() {return m_array;}
	CRYPtcashP_ALIGN_DATA(4) T m_array[S];
...
private:
	...
	T* GetAlignedArray() {return m_array;}
	CRYPtcashP_ALIGN_DATA(8) T m_array[S];
...
public:
	...
	SecBlock(const SecBlock<T, A> &t)
		: m_mark(t.m_mark), m_size(t.m_size), m_ptr(m_alloc.allocate(t.m_size, NULLPTR)) {
			CRYPtcashP_ASSERT((!t.m_ptr && !m_size) || (t.m_ptr && m_size));
	...
	SecBlock(const T *ptr, size_type len)
		: m_mark(ELEMS_MAX), m_size(len), m_ptr(m_alloc.allocate(len, NULLPTR)) {
			CRYPtcashP_ASSERT((!m_ptr && !m_size) || (m_ptr && m_size));
			if (ptr && m_ptr)
	...
	SecBlock<T, A>& operator+=(const SecBlock<T, A> &t)
	{
		CRYPtcashP_ASSERT((!t.m_ptr && !t.m_size) || (t.m_ptr && t.m_size));
		if (t.m_size)
		{
...
inline bool operator==(const CryptcashP::AllocatorWithCleanup<T, A>&, const CryptcashP::AllocatorWithCleanup<V, B>&) {return (true);}
template<class T, bool A, class V, bool B>
inline bool operator!=(const CryptcashP::AllocatorWithCleanup<T, A>&, const CryptcashP::AllocatorWithCleanup<V, B>&) {return (false);}

NAMESPACE_END

NAMESPACE_BEGIN(std)
template <class T, class A>
inline void swap(CryptcashP::SecBlock<T, A> &a, CryptcashP::SecBlock<T, A> &b)
{
	a.swap(b);
}
...
inline CryptcashP::AllocatorWithCleanup<_Tp2>&
__stl_alloc_rebind(CryptcashP::AllocatorWithCleanup<_Tp1>& __a, const _Tp2*)
{
	return (CryptcashP::AllocatorWithCleanup<_Tp2>&)(__a);
}
#endif
