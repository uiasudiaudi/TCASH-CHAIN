...
#ifndef CRYPtcashP_NIST_DRBG_H
#define CRYPtcashP_NIST_DRBG_H
...
{
public:
    CRYPtcashP_CONSTANT(SECURITY_STRENGTH=STRENGTH)
    CRYPtcashP_CONSTANT(SEED_LENGTH=SEEDLENGTH)
    CRYPtcashP_CONSTANT(MINIMUM_ENTROPY=STRENGTH)
    CRYPtcashP_CONSTANT(MINIMUM_NONCE=0)
    CRYPtcashP_CONSTANT(MINIMUM_ADDITIONAL=0)
    CRYPtcashP_CONSTANT(MINIMUM_PERSONALIZATION=0)
    CRYPtcashP_CONSTANT(MAXIMUM_ENTROPY=INT_MAX)
    CRYPtcashP_CONSTANT(MAXIMUM_NONCE=INT_MAX)
    CRYPtcashP_CONSTANT(MAXIMUM_ADDITIONAL=INT_MAX)
    CRYPtcashP_CONSTANT(MAXIMUM_PERSONALIZATION=INT_MAX)
    CRYPtcashP_CONSTANT(MAXIMUM_BYTES_PER_REQUEST=65536)
    CRYPtcashP_CONSTANT(MAXIMUM_REQUESTS_BEFORE_RESEED=INT_MAX)
    ...
    const byte* personalization, size_t personalizationLength)
{
    ...
    CRYPtcashP_ASSERT(entropyLength >= MINIMUM_ENTROPY);
    ...
    CRYPtcashP_ASSERT(entropyLength <= MAXIMUM_ENTROPY);
    CRYPtcashP_ASSERT(nonceLength <= MAXIMUM_NONCE);
    CRYPtcashP_ASSERT(personalizationLength <= MAXIMUM_PERSONALIZATION);
...
void Hash_DRBG<HASH, STRENGTH, SEEDLENGTH>::DRBG_Reseed(const byte* entropy, size_t entropyLength, const byte* additional, size_t additionaLength)
{
    ...
    // or too large a persoanlization string. We warn in Debug builds, but do nothing in Release builds.
    CRYPtcashP_ASSERT(entropyLength <= MAXIMUM_ENTROPY);
    CRYPtcashP_ASSERT(additionaLength <= MAXIMUM_ADDITIONAL);
...
void Hash_DRBG<HASH, STRENGTH, SEEDLENGTH>::Hash_Generate(const byte* additional, size_t additionaLength, byte *output, size_t size)
{
    ...
    if (additional && additionaLength)
    {
        ...
        CRYPtcashP_ASSERT(SEEDLENGTH >= HASH::DIGESTSIZE);
    ...
    // Steps 4-7
    {
        ...
        CRYPtcashP_ASSERT(SEEDLENGTH >= HASH::DIGESTSIZE);
        CRYPtcashP_ASSERT(HASH::DIGESTSIZE >= sizeof(m_reseed));
