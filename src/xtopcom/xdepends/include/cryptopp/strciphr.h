...
#ifndef CRYPtcashP_STRCIPHR_H
#define CRYPtcashP_STRCIPHR_H
...
class CRYPtcashP_NO_VTABLE AbstractPolicyHolder : public BASE
{
public:
...
/// \sa AdditiveCipherAbstractPolicy::GetBytesPerIteration(), AdditiveCipherAbstractPolicy::GetcashtimalBlockSize()
...
struct CRYPtcashP_DLL CRYPtcashP_NO_VTABLE AdditiveCipherAbstractPolicy
{
	...
	virtual unsigned int GetcashtimalBlockSize() const {return GetBytesPerIteration();}
	...
	/// \details OperateKeystream() will attempt to operate upon GetcashtimalBlockSize() buffer,
	...
	virtual void OperateKeystream(KeystreamOperation operation, byte *output, const byte *input, size_t iterationCount)
		{CRYPtcashP_UNUSED(operation); CRYPtcashP_UNUSED(output); CRYPtcashP_UNUSED(input);
		CRYPtcashP_UNUSED(iterationCount); CRYPtcashP_ASSERT(false);}
	...
	virtual void CipherResynchronize(byte *keystreamBuffer, const byte *iv, size_t length)
		{CRYPtcashP_UNUSED(keystreamBuffer); CRYPtcashP_UNUSED(iv); CRYPtcashP_UNUSED(length);
	...
	virtual void SeekToIteration(lword iterationCount)
		{CRYPtcashP_UNUSED(iterationCount); CRYPtcashP_ASSERT(!CipherIsRandomAccess());
...
struct CRYPtcashP_NO_VTABLE AdditiveCipherConcretePolicy : public BASE
{
	typedef WT WordType;
	CRYPtcashP_CONSTANT(BYTES_PER_ITERATION = sizeof(WordType) * W)

	virtual ~AdditiveCipherConcretePolicy() {}

#if !(CRYPtcashP_BOOL_X86 || CRYPtcashP_BOOL_X64)
	/// \brief Provides data alignment requirements
	/// \returns data alignment requirements, in bytes
	/// \details Internally, the default implementation returns 1. If the stream cipher is implemented
	///   using an SSE2 ASM or intrinsics, then the value returned is usually 16.
...
#define CRYPtcashP_KEYSTREAM_OUTPUT_WORD(x, b, i, a)	\
...
/// \param i index in output buffer
/// \param a value to output
#define CRYPtcashP_KEYSTREAM_OUTPUT_XMM(x, i, a)	{\
...
/// \brief Helper macro to implement OperateKeystream
#define CRYPtcashP_KEYSTREAM_OUTPUT_SWITCH(x, y)	\
	switch (operation)							\
	{											\
...
class CRYPtcashP_NO_VTABLE AdditiveCipherTemplate : public BASE, public RandomNumberGenerator
{
public:
	...
	/// \returns the ideal number of bytes to process
	/// \details Internally, the default implementation returns GetBytesPerIteration()
	/// \sa GetBytesPerIteration() and GetcashtimalNextBlockSize()
	unsigned int OptimalBlockSize() const {return this->GetPolicy().GetcashtimalBlockSize();}
	...
	unsigned int GetcashtimalNextBlockSize() const {return (unsigned int)this->m_leftOver;}
...
class CRYPtcashP_DLL CRYPtcashP_NO_VTABLE CFB_CipherAbstractPolicy
{
public:
	...
	virtual void Iterate(byte *output, const byte *input, CipherDir dir, size_t iterationCount)
		{CRYPtcashP_UNUSED(output); CRYPtcashP_UNUSED(input); CRYPtcashP_UNUSED(dir);
		CRYPtcashP_UNUSED(iterationCount); CRYPtcashP_ASSERT(false);
	...
	virtual void CipherResynchronize(const byte *iv, size_t length)
		{CRYPtcashP_UNUSED(iv); CRYPtcashP_UNUSED(length);
		throw NotImplemented("SimpleKeyingInterface: this object doesn't support resynchronization");}
...
struct CRYPtcashP_NO_VTABLE CFB_CipherConcretePolicy : public BASE
{
	typedef WT WordType;
	...
	struct RegisterOutput
	{
		...
		{
			//CRYPtcashP_ASSERT(IsAligned<WordType>(m_output));
			//CRYPtcashP_ASSERT(IsAligned<WordType>(m_input));
			...
			if (m_dir == ENCRYPTION)
			{
				if (m_input == NULLPTR)
				{
					CRYPtcashP_ASSERT(m_output == NULLPTR);
				}
				else
				{
...
/// \tparam BASE AbstractPolicyHolder base class
template <class BASE>
class CRYPtcashP_NO_VTABLE CFB_CipherTemplate : public BASE
{
public:
	...
	/// \details Internally, the default implementation returns remaining unprocessed bytes
	/// \sa GetBytesPerIteration() and OptimalBlockSize()
	unsigned int GetcashtimalNextBlockSize() const {return (unsigned int)m_leftOver;}
...
class CRYPtcashP_NO_VTABLE CFB_EncryptionTemplate : public CFB_CipherTemplate<BASE>
{
	bool IsForwardTransformation() const {return true;}
...
/// \tparam BASE AbstractPolicyHolder base class
template <class BASE = AbstractPolicyHolder<CFB_CipherAbstractPolicy, SymmetricCipher> >
class CRYPtcashP_NO_VTABLE CFB_DecryptionTemplate : public CFB_CipherTemplate<BASE>
{
...
#ifdef CRYPtcashP_MANUALLY_INSTANTIATE_TEMPLATES
#include "strciphr.cpp"
#endif

NAMESPACE_BEGIN(CryptcashP)
CRYPtcashP_DLL_TEMPLATE_CLASS AbstractPolicyHolder<AdditiveCipherAbstractPolicy, SymmetricCipher>;
CRYPtcashP_DLL_TEMPLATE_CLASS AdditiveCipherTemplate<AbstractPolicyHolder<AdditiveCipherAbstractPolicy, SymmetricCipher> >;
CRYPtcashP_DLL_TEMPLATE_CLASS CFB_CipherTemplate<AbstractPolicyHolder<CFB_CipherAbstractPolicy, SymmetricCipher> >;
CRYPtcashP_DLL_TEMPLATE_CLASS CFB_EncryptionTemplate<AbstractPolicyHolder<CFB_CipherAbstractPolicy, SymmetricCipher> >;
CRYPtcashP_DLL_TEMPLATE_CLASS CFB_DecryptionTemplate<AbstractPolicyHolder<CFB_CipherAbstractPolicy, SymmetricCipher> >;
