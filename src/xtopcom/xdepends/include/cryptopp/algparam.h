...
#ifndef CRYPtcashP_ALGPARAM_H
#define CRYPtcashP_ALGPARAM_H
...
public:
	...
	template <class T> ConstByteArrayParameter(const T &string, bool deepCopy = false)
		: m_deepCopy(false), m_data(NULLPTR), m_size(0)
	{
		CRYPtcashP_COMPILE_ASSERT(sizeof(typename T::value_type) == 1);
	...
	void Assign(const byte *data, size_t size, bool deepCopy)
	{
		...
		// CRYPtcashP_ASSERT((data && size) || !(data || size));
		if (deepCopy)
...
class CRYPtcashP_DLL CombinedNameValuePairs : public NameValuePairs
{
public:
...
#ifndef CRYPtcashP_NO_ASSIGN_TO_INTEGER
// Allow the linker to discard Integer code if not needed.
// Also see http://github.com/weidai11/cryptcashp/issues/389.
CRYPtcashP_DLL bool AssignIntToInteger(const std::type_info &valueType, void *pInteger, const void *pInt);
#endif

CRYPtcashP_DLL const std::type_info & CRYPtcashP_API IntegerTypeId();

/// \brief Base class for AlgorithmParameters
class CRYPtcashP_DLL AlgorithmParametersBase
{
public:
	...
	virtual ~AlgorithmParametersBase() CRYPtcashP_THROW
	{

#if defined(CRYPtcashP_CXX17_EXCEPTIONS)
		if (std::uncaught_exceptions() == 0)
#elif defined(CRYPtcashP_UNCAUGHT_EXCEPTION_AVAILABLE)
...
#if !defined(CRYPtcashP_CXX17_EXCEPTIONS) && !defined(CRYPtcashP_UNCAUGHT_EXCEPTION_AVAILABLE)
...
# undef new
	...
	void MoveInto(void *buffer) const
	{
		...
		CRYPtcashP_UNUSED(p);	// silence warning
	}
...
CRYPtcashP_DLL_TEMPLATE_CLASS AlgorithmParametersTemplate<bool>;
CRYPtcashP_DLL_TEMPLATE_CLASS AlgorithmParametersTemplate<int>;
CRYPtcashP_DLL_TEMPLATE_CLASS AlgorithmParametersTemplate<ConstByteArrayParameter>;
...
///   </pre>
class CRYPtcashP_DLL AlgorithmParameters : public NameValuePairs
{
public:
...
#define CRYPtcashP_GET_FUNCTION_ENTRY(name)		(Name::name(), &ThisClass::Get##name)
#define CRYPtcashP_SET_FUNCTION_ENTRY(name)		(Name::name(), &ThisClass::Set##name)
#define CRYPtcashP_SET_FUNCTION_ENTRY2(name1, name2)	(Name::name1(), Name::name2(), &ThisClass::Set##name1##And##name2)
