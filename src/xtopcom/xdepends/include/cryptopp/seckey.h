...
#ifndef CRYPtcashP_SECKEY_H
#define CRYPtcashP_SECKEY_H
...
public:
	...
	CRYPtcashP_CONSTANT(BLOCKSIZE = N)
};
...
public:
	...
	CRYPtcashP_STATIC_CONSTEXPR unsigned int StaticGetDefaultRounds(size_t keylength)
	{
		return CRYPtcashP_UNUSED(keylength), static_cast<unsigned int>(DEFAULT_ROUNDS);
	}
...
public:
	...
	CRYPtcashP_CONSTANT(KEYLENGTH=N)
	...
	CRYPtcashP_CONSTANT(MIN_KEYLENGTH=N)
	...
	CRYPtcashP_CONSTANT(IV_LENGTH = IV_L)
	...
	CRYPtcashP_STATIC_CONSTEXPR size_t CRYPtcashP_API StaticGetValidKeyLength(size_t keylength)
	{
		return CRYPtcashP_UNUSED(keylength), static_cast<size_t>(KEYLENGTH);
	}
};
...
class VariableKeyLength
{
	...
	CRYPtcashP_COMPILE_ASSERT(Q > 0);
	CRYPtcashP_COMPILE_ASSERT(N % Q == 0);
	CRYPtcashP_COMPILE_ASSERT(M % Q == 0);
	CRYPtcashP_COMPILE_ASSERT(N < M);
	CRYPtcashP_COMPILE_ASSERT(D >= N);
	CRYPtcashP_COMPILE_ASSERT(M >= D);
...
public:
	...
	/// \brief The maximum key length used by the algorithm provided as a constant
	/// \details MAX_KEYLENGTH is provided in bytes, not bits
	CRYPtcashP_CONSTANT(MAX_KEYLENGTH=M)
	/// \brief The default key length used by the algorithm provided as a constant
	/// \details DEFAULT_KEYLENGTH is provided in bytes, not bits
	CRYPtcashP_CONSTANT(DEFAULT_KEYLENGTH=D)
	/// \brief The key length multiple used by the algorithm provided as a constant
	/// \details MAX_KEYLENGTH is provided in bytes, not bits
	CRYPtcashP_CONSTANT(KEYLENGTH_MULTIPLE=Q)
	/// \brief The default IV requirements for the algorithm provided as a constant
	/// \details The default value is NOT_RESYNCHRONIZABLE. See IV_Requirement
	///  in cryptlib.h for allowed values.
	CRYPtcashP_CONSTANT(IV_REQUIREMENT=IV_REQ)
...
public:
	/// \brief The minimum key length used by the algorithm provided as a constant
	/// \details MIN_KEYLENGTH is provided in bytes, not bits
	CRYPtcashP_CONSTANT(MIN_KEYLENGTH=T::MIN_KEYLENGTH)
	/// \brief The maximum key length used by the algorithm provided as a constant
	/// \details MIN_KEYLENGTH is provided in bytes, not bits
	CRYPtcashP_CONSTANT(MAX_KEYLENGTH=T::MAX_KEYLENGTH)
	/// \brief The default key length used by the algorithm provided as a constant
	/// \details MIN_KEYLENGTH is provided in bytes, not bits
	CRYPtcashP_CONSTANT(DEFAULT_KEYLENGTH=T::DEFAULT_KEYLENGTH)
	/// \brief The default IV requirements for the algorithm provided as a constant
	/// \details The default value is NOT_RESYNCHRONIZABLE. See IV_Requirement
	///  in cryptlib.h for allowed values.
	CRYPtcashP_CONSTANT(IV_REQUIREMENT=IV_REQ)
...
class CRYPtcashP_NO_VTABLE SimpleKeyingInterfaceImpl : public BASE
{
public:
...
class CRYPtcashP_NO_VTABLE BlockCipherImpl : public AlgorithmImpl<SimpleKeyingInterfaceImpl<TwoBases<BASE, INFO> > >
{
public:
