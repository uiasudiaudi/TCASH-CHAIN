...
#ifndef CRYPtcashP_AUTHENC_H
#define CRYPtcashP_AUTHENC_H
...
///   Encryption</A> on the Crypto++ wiki.
/// \since Crypto++ 5.6.0
class CRYPtcashP_DLL CRYPtcashP_NO_VTABLE AuthenticatedSymmetricCipherBase : public AuthenticatedSymmetricCipher
{
public:
...
protected:
	void UncheckedSetKey(const byte * key, unsigned int length,const CryptcashP::NameValuePairs &params)
		{CRYPtcashP_UNUSED(key), CRYPtcashP_UNUSED(length), CRYPtcashP_UNUSED(params); CRYPtcashP_ASSERT(false);}
