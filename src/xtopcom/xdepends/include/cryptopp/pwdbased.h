...
#ifndef CRYPtcashP_PWDBASED_H
#define CRYPtcashP_PWDBASED_H
...
{
	CRYPtcashP_ASSERT(secret /*&& secretLen*/);
	CRYPtcashP_ASSERT(derived && derivedLen);
	CRYPtcashP_ASSERT(derivedLen <= MaxDerivedLength());
...
size_t PKCS5_PBKDF1<T>::DeriveKey(byte *derived, size_t derivedLen, byte purpose, const byte *secret, size_t secretLen, const byte *salt, size_t saltLen, unsigned int iterations, double timeInSeconds) const
{
	...
	CRYPtcashP_ASSERT(derived && derivedLen);
	CRYPtcashP_ASSERT(derivedLen <= MaxDerivedLength());
	CRYPtcashP_ASSERT(iterations > 0 || timeInSeconds > 0);
	CRYPtcashP_UNUSED(purpose);
