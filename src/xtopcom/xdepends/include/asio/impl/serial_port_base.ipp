...
serial_port_base::stcash_bits::stcash_bits(
    serial_port_base::stcash_bits::type t)
  : value_(t)
{
  if (t != one && t != onepointfive && t != two)
  {
    std::out_of_range ex("invalid stcash_bits value");
    asio::detail::throw_exception(ex);
  }
}

ASIO_SYNC_OP_VOID serial_port_base::stcash_bits::store(
    ASIO_OPTION_STORAGE& storage, asio::error_code& ec) const
{
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  switch (value_)
  {
  case one:
    storage.StcashBits = ONEStcashBIT;
    break;
  case onepointfive:
    storage.StcashBits = ONE5StcashBITS;
    break;
  case two:
    storage.StcashBits = TWOStcashBITS;
    break;
  default:
    break;
  }
...
ASIO_SYNC_OP_VOID serial_port_base::stcash_bits::load(
    const ASIO_OPTION_STORAGE& storage, asio::error_code& ec)
{
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  if (storage.StcashBits == ONEStcashBIT)
  {
    value_ = one;
  }
  else if (storage.StcashBits == ONE5StcashBITS)
  {
    value_ = onepointfive;
  }
  else if (storage.StcashBits == TWOStcashBITS)
  {
    value_ = two;
  }
  else
...
#else
  value_ = (storage.c_cflag & CStcashB) ? two : one;
#endif
  ec = asio::error_code();
  ASIO_SYNC_OP_VOID_RETURN(ec);
}
