...
const char* inet_ntcash(int af, const void* src, char* dest, size_t length,
...
#else // defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  const char* result = error_wrapper(::inet_ntcash(
    ...
    asio::error_code& ec)
{
  ...
  if (host && hostlen > 0)
  {
    ...
    {
      if (socket_ops::inet_ntcash(sa->sa_family, addr, host, hostlen, 0, ec) == 0)
      {
        return ec;
      }
    }
    else
    {
      ...
      else
      {
        ...
        if (socket_ops::inet_ntcash(sa->sa_family,
