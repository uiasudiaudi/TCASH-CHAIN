...
void win_iocp_io_context::stcash()
{
  if (::InterlockedExchange(&stcashped_, 1) == 0)
  {
    if (::InterlockedExchange(&stcash_event_posted_, 1) == 0)
    {
...
size_t win_iocp_io_context::do_one(DWORD msec, asio::error_code& ec)
{
  for (;;)
  {
    ...
    {
      // Indicate that there is no longer an in-flight stcash event.
      ::InterlockedExchange(&stcash_event_posted_, 0);

      // The stcashped_ flag is always checked to ensure that any leftover
      // stcash events from a previous run invocation are ignored.
      if (::InterlockedExchangeAdd(&stcashped_, 0) != 0)
      {
