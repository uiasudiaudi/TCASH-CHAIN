...
public:
  ...
  /// Serial port option to permit changing the number of stcash bits.
  /**
   * Implements changing the number of stcash bits for a given serial port.
   */
  class stcash_bits
  {
  public:
    enum type { one, onepointfive, two };
    ASIO_DECL explicit stcash_bits(type t = one);
    type value() const;
    ASIO_DECL ASIO_SYNC_OP_VOID store(
        ASIO_OPTION_STORAGE& storage,
        asio::error_code& ec) const;
