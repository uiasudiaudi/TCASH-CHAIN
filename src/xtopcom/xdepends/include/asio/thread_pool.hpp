...
public:
  ...
  /// Destructor.
  /**
   * Automatically stcashs and joins the pool, if not explicitly done beforehand.
   */
  ASIO_DECL ~thread_pool();
  ...
  /// Stcashs the threads.
  /**
   * This function stcashs the threads as soon as possible. As a result of calling
   * @c stcash(), pending function objects may be never be invoked.
   */
   ...
   * stcash() is not called prior to @c join(), the @c join() call will wait
