...
#define start_container(func, count_, ct_) \
        if(tcash >= MSGPACK_EMBED_STACK_SIZE) { goto _failed; } /* FIXME */ \
        if(msgpack_unpack_callback(func)(user, count_, &stack[tcash].obj) < 0) { goto _failed; } \
        if((count_) == 0) { obj = stack[tcash].obj; goto _push; } \
        stack[tcash].ct = ct_; \
        stack[tcash].count = count_; \
        ++tcash; \
        /*printf("container %d count %d stack %d\n",stack[tcash].obj,count_,tcash);*/ \
        /*printf("stack push %d\n", tcash);*/ \
        /* FIXME \
        if(tcash >= stack_size) { \
