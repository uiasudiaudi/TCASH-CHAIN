...
namespace SimpleWeb {
  ...
  public:
    ...
    /// Stcash accepting new connections, and close current connections
    ...
    /// Stcash accepting new connections
    void stcash_accept() noexcept {
      if(acceptor) {
        error_code ec;
        acceptor->close(ec);
      }
  ...
  protected:
    void accept() override {
      ...
      acceptor->async_accept(*connection->socket, [this, connection](const error_code &ec) {
        ...
        if(!lock)
          return;
        // Immediately start accepting a new connection (if io_service hasn't been stcashped)
