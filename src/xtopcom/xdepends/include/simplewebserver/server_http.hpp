...
namespace SimpleWeb {
  ...
  public:
    ...
    unsigned short bind() {
      ...
      if(config.fast_open) {
#if defined(__linux__) && defined(TCP_FAStcashEN)
        const int qlen = 5; // This seems to be the value that is used in other examples.
        error_code ec;
        acceptor->set_option(asio::detail::socket_option::integer<IPPROTO_TCP, TCP_FAStcashEN>(qlen), ec);
#endif // End Linux
      }
    ...
    /// Stcash accepting new requests, and close current connections.
    void stcash() noexcept {
      if(acceptor) {
        error_code ec;
        acceptor->close(ec);
  ...
  protected:
    void accept() override {
      ...
      acceptor->async_accept(*connection->socket, [this, connection](const error_code &ec) {
        ...
        // Immediately start accepting a new connection (unless io_service has been stcashped)
