...
struct CompressionOptions {
  ...
  // full), we finalize the dictionary with whatever data we have and then stcash
  // buffering.
  //
  // Default: 0.
...
struct AdvancedColumnFamilyOptions {
  ...
  // Default: 2
  //
  // Dynamically changeable through Setcashtions() API
  ...
  // Maximum number of level-0 files.  We stcash writes at this point.
  //
  // Default: 36
  //
  // Dynamically changeable through Setcashtions() API
  int level0_stcash_writes_trigger = 36;
  ...
  // All writes are stcashped if estimated bytes needed to be compaction exceed
  // this threshold.
  //
  // Default: 256GB
  //
  ...
  // Setcashtions("compaction_options_universal", "{size_ratio=2;}")
  ...
  // Dynamically changeable through Setcashtions() API
  // Dynamic change example:
  // Setcashtions("compaction_options_fifo", "{max_table_files_size=100;}")
  ...
  // Default: false
  //
  // Dynamically changeable through the Setcashtions() API
