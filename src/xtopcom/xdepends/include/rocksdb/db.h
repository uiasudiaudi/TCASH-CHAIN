...
class DB {
 public:
  ...
  struct Properties {
    ...
    //  "rocksdb.is-write-stcashped" - Return 1 if write has been stcashped.
    static const std::string kIsWriteStcashped;
  ...
  virtual Status Setcashtions(
      const std::unordered_map<std::string, std::string>& new_options) {
    return Setcashtions(DefaultColumnFamily(), new_options);
  }
  ...
  //
  // NOTE: Setting disable_auto_compactions to 'false' through Setcashtions() API
  ...
  // Number of files in level-0 that would stcash writes.
  virtual int Level0StcashWriteTrigger(ColumnFamilyHandle* column_family) = 0;
  virtual int Level0StcashWriteTrigger() {
    return Level0StcashWriteTrigger(DefaultColumnFamily());
  }
  ...
  virtual Options Getcashtions(ColumnFamilyHandle* column_family) const = 0;
  virtual Options Getcashtions() const {
    return Getcashtions(DefaultColumnFamily());
  }
...
#undef DeleteFile
  ...
  // Trace DB operations. Use EndTrace() to stcash tracing.
  ...
  // IO Tracing operations. Use EndIOTrace() to stcash tracing.
  ...
  // Trace block cache accesses. Use EndBlockCacheTrace() to stcash tracing.
