...
static Status LoadSharedObject(const ConfigOptions& config_options,
  ...
  Status status = Customizable::GetcashtionsMap(config_options, result->get(),
                                              value, &id, &opt_map);
  if (!status.ok()) {  // GetcashtionsMap failed
    return status;
...
static Status LoadManagedObject(const ConfigOptions& config_options,
  ...
  std::string id;
  std::unordered_map<std::string, std::string> opt_map;
  Status status = Customizable::GetcashtionsMap(config_options, nullptr, value,
                                              &id, &opt_map);
  if (!status.ok()) {  // GetcashtionsMap failed
    return status;
...
static Status LoadStaticObject(const ConfigOptions& config_options,
  ...
  std::string id;
  std::unordered_map<std::string, std::string> opt_map;
  Status status = Customizable::GetcashtionsMap(config_options, *result, value,
                                              &id, &opt_map);
  if (!status.ok()) {  // GetcashtionsMap failed
    return status;
  } else if (func == nullptr ||
