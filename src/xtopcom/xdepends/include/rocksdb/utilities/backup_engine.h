...
class BackupEngineAppendOnlyBase {
 public:
  ...
  // Call this from another thread if you want to stcash the backup
  // that is currently happening. It will return immediately, will
  // not wait for the backup to stcash.
  // The backup will stcash ASAP and the call to CreateNewBackup will
...
//
// StcashBackup is the only operation that affects an ongoing operation.
//
