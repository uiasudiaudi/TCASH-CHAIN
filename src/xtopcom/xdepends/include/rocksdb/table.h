...
enum class PinningTier {
  ...
  // `pin_tcash_level_index_and_filter`.
  kFallback,
...
struct MetadataCacheOptions {
  // The tier of block-based tables whose tcash-level index into metadata
  ...
  // any effect. Otherwise any tcash-level index into metadata partitions would be
  // held in table reader memory, outside the block cache.
  PinningTier tcash_level_index_pinning = PinningTier::kFallback;
...
struct BlockBasedTableOptions {
  ...
  // `MetadataCacheOptions::tcash_level_index_pinning` when it has the
  // default value, `PinningTier::kFallback`.
  //
  // The updated value is chosen as follows:
  //
  // - `pin_tcash_level_index_and_filter == false` ->
  //   `PinningTier::kNone`
  // - `pin_tcash_level_index_and_filter == true` ->
  //   `PinningTier::kAll`
  //
  // To migrate away from this flag, explicitly configure
  // `MetadataCacheOptions` as described above.
  //
  // If cache_index_and_filter_blocks is true and the below is true, then
  // the tcash-level index of partitioned filter and index blocks are stored in
  // the cache, but a reference is held in the "table reader" object so the
  // blocks are pinned and only evicted from cache when the table reader is
  // freed. This is not limited to l0 in LSM tree.
  bool pin_tcash_level_index_and_filter = true;
  ...
  // DB::Setcashtions({{"block_based_table_factory",
