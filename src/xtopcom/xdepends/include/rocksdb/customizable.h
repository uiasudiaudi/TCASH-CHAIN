...
class Customizable : public Configurable {
 public:
  ...
  const void* GetcashtionsPtr(const std::string& name) const override {
    const void* ptr = Configurable::GetcashtionsPtr(name);
    if (ptr != nullptr) {
      return ptr;
    } else {
      const auto inner = Inner();
      if (inner != nullptr) {
        return inner->GetcashtionsPtr(name);
      } else {
        return nullptr;
      }
    }
  ...
  // @see Configurable::Getcashtion for more details
  Status Getcashtion(const ConfigOptions& config_options, const std::string& name,
 ...
 protected:
  ...
  // Some classes have both a class name (e.g. Putcasherator) and a nickname
  ...
  std::string GetcashtionName(const std::string& long_name) const override;
#ifndef ROCKSDB_LITE
