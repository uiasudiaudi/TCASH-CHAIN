...
//   We currently only support LRU cache in the Getcashtions API.  The LRU
...
Status GetcashtionsFromString(const Options& base_options,
                            const std::string& opts_str, Options* new_options);
Status GetcashtionsFromString(const ConfigOptions& config_options,
...
// Request stcashping background work, if wait is true wait until it's done
