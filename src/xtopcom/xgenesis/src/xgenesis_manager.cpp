...
#define CHECK_EC_RETURN(ec)                                                                                                                                                        \
    do {                                                                                                                                                                           \
        if (ec) {                                                                                                                                                                  \
            xwarn("[xtcash_genesis_manager] error occured, category: %s, msg: %s!", ec.category().name(), ec.message().c_str());                                                     \
            return;                                                                                                                                                                \
        }                                                                                                                                                                          \
    } while (0)
...
xtcash_genesis_manager::xtcash_genesis_manager(observer_ptr<base::xvblockstore_t> const & blockstore)
...
void xtcash_genesis_manager::load_accounts() {
    // step1: lock
    ...
    for (auto const & pair : system_contract_map) {
        auto const & address = tcash::get<common::xaccount_address_t const>(pair);
...
void xtcash_genesis_manager::release_accounts() {
    // lock
    std::lock_guard<std::mutex> guard(m_lock);
    // clear()
...
void xtcash_genesis_manager::create_genesis_of_root_account(std::error_code & ec) {
    if (m_root_finish == true) {
        return;
    }
    base::xvaccount_t account{data::xrootblock_t::get_rootblock_address()};
    xinfo("[xtcash_genesis_manager::create_genesis_of_root_account] account %s", account.get_account().c_str());
    // lock
    std::lock_guard<std::mutex> guard(m_lock);
    // check
    if (m_blockstore->exist_genesis_block(account)) {
        xinfo("[xtcash_genesis_manager::create_genesis_of_root_account] account: %s, rootblock already exists", account.get_account().c_str());
        return;
    }
    ...
    if (false == m_blockstore->store_block(account, rootblock)) {
        xerror("[xtcash_genesis_manager::create_genesis_of_root_account] account: %s, store rootblock failed", account.get_account().c_str());
        xassert(false);
        SET_EC_RETURN(ec, error::xenum_errc::genesis_block_store_failed);
    }
    m_root_finish = true;
    xinfo("[xtcash_genesis_manager::create_genesis_of_root_account] account: %s, create rootblock success", account.get_account().c_str());
}

base::xauto_ptr<base::xvblock_t> xtcash_genesis_manager::create_genesis_of_contract_account(common::xaccount_address_t const & account, xenum_create_src_t src, std::error_code & ec) {
#if 0
    // lock
    std::lock_guard<std::mutex> guard(m_lock);
...
#else
    xinfo("[xtcash_genesis_manager::create_genesis_of_contract_account] account %s", account.to_string().c_str());
    // lock
    std::lock_guard<std::mutex> guard(m_lock);
    ...
    if (src == xenum_create_src_t::init && m_blockstore->exist_unit(account.vaccount())) {
        ...
        if (existed_genesis_block->get_block_hash() == genesis_block->get_block_hash()) {
            xinfo("[xtcash_genesis_manager::create_genesis_of_contract_account] account %s, genesis block already exists", account.to_string().c_str());
            return nullptr;
        }
        if ((account.to_string().find(sys_contract_rec_elect_fullnode_addr) != std::string::npos) ||
            ...
            xwarn("[xtcash_genesis_manager::create_genesis_of_contract_account] account: %s genesis block exist but hash not match, replace it, %s, %s",
            ...
            xerror("[xtcash_genesis_manager::create_genesis_of_contract_account] account: %s genesis block exist but hash not match, %s, %s",
                   account.to_string().c_str(),
                   base::xstring_utl::to_hex(existed_genesis_block->get_block_hash()).c_str(),
                   base::xstring_utl::to_hex(genesis_block->get_block_hash()).c_str());
            xassert(false);
    ...
    xinfo("[xtcash_genesis_manager::create_genesis_of_contract_account] account: %s, create genesis block success", account.to_string().c_str());
    return genesis_block;
#endif
}

base::xauto_ptr<base::xvblock_t> xtcash_genesis_manager::create_genesis_of_evm_contract_account(base::xvaccount_t const & account, xenum_create_src_t src, std::error_code & ec) {
    xinfo("[xtcash_genesis_manager::create_genesis_of_evm_contract_account] account %s", account.get_account().c_str());
    if (account.get_account() == evm_eth_bridge_contract_address.to_string() || account.get_account() == evm_bsc_client_contract_address.to_string() ||
        ...
        bstate->load_string_var(data::system_contract::XPROPERTY_RESET_FLAG)->reset(tcash::to_string(0), canvas.get());
        // create
        ...
        if (src == xenum_create_src_t::init && m_blockstore->exist_unit(account)) {
            ...
            if (existed_genesis_block->get_block_hash() == genesis_block->get_block_hash()) {
                xinfo("[xtcash_genesis_manager::create_genesis_of_evm_contract_account] account %s, genesis block already exists", account.get_account().c_str());
                return nullptr;
            } else {
                // just delete it here and store new root block after
                xwarn("[xtcash_genesis_manager::create_genesis_of_evm_contract_account] account: %s genesis block exist but hash not match, replace it, %s, %s",
        ...
        xinfo("[xtcash_genesis_manager::create_genesis_of_contract_account] account: %s, create genesis block success", account.get_account().c_str());
        return genesis_block;
    }
...
base::xauto_ptr<base::xvblock_t> xtcash_genesis_manager::create_genesis_of_datauser_account(base::xvaccount_t const & account,
                                                                                          chain_data::data_processor_t const & data,
                                                                                          xenum_create_src_t src,
                                                                                          std::error_code & ec) {
    xinfo(
        "[xtcash_genesis_manager::create_genesis_of_datauser_account] account=%s, balance=%ld, burn_balance=%ld, tgas_balance=%ld, vote_balance=%ld, lock_balance=%ld, "
    ...
    // check
    if (src == xenum_create_src_t::init && m_blockstore->exist_unit(account)) {
        xinfo("[xtcash_genesis_manager::create_genesis_of_datauser_account] account: %s, genesis block already exists", account.get_account().c_str());
        return nullptr;
    }
    // create
    base::xauto_ptr<base::xvblock_t> genesis_block = data::xblocktool_t::create_genesis_lightunit(account.get_account(), data);
    xassert(genesis_block != nullptr);
    xinfo("[xtcash_genesis_manager::create_genesis_of_datauser_account] account: %s, create genesis block success", account.get_account().c_str());
    return genesis_block;
}

base::xauto_ptr<base::xvblock_t> xtcash_genesis_manager::create_genesis_of_genesis_account(base::xvaccount_t const & account,
                                                                                         uint64_t const data,
                                                                                         xenum_create_src_t src,
                                                                                         std::error_code & ec) {
    xinfo("[xtcash_genesis_manager::create_genesis_of_genesis_account] account: %s, balance: %lu", account.get_account().c_str(), data);
    // lock
    std::lock_guard<std::mutex> guard(m_lock);
    // check
    if (src == xenum_create_src_t::init && m_blockstore->exist_unit(account)) {
        xinfo("[xtcash_genesis_manager::create_genesis_of_genesis_account] account: %s, genesis block already exists", account.get_account().c_str());
        return nullptr;
    }
    // create
    base::xauto_ptr<base::xvblock_t> genesis_block = data::xblocktool_t::create_genesis_lightunit(account.get_account(), data);
    xassert(genesis_block != nullptr);
    xinfo("[xtcash_genesis_manager::create_genesis_of_genesis_account] account: %s, create genesis block success", account.get_account().c_str());
    return genesis_block;
}

base::xauto_ptr<base::xvblock_t> xtcash_genesis_manager::create_genesis_of_common_account(base::xvaccount_t const & account, xenum_create_src_t src, std::error_code & ec) {
    xdbg("[xtcash_genesis_manager::create_genesis_of_common_account] account: %s", account.get_account().c_str());
    // lock
    std::lock_guard<std::mutex> guard(m_lock);
    // check
    ...
    xdbg("[xtcash_genesis_manager::create_genesis_of_common_account] account: %s, create empty genesis block success", account.get_account().c_str());
    return genesis_block;
}

void xtcash_genesis_manager::store_block(base::xvaccount_t const & account, base::xvblock_t * block, std::error_code & ec) {
    if (account.is_unit_address()) {
        if (false == m_blockstore->store_unit(account, block)) {
            xerror("[xtcash_genesis_manager::store_block] account: %s, store genesis block failed", account.get_account().c_str());
            xassert(false);
            SET_EC_RETURN(ec, error::xenum_errc::genesis_block_store_failed);
        }
    } else {
...
void xtcash_genesis_manager::init_genesis_block(std::error_code & ec) {
    ...
    if (false == chain_data::xtcash_chain_data_processor::check_state()) {
        ...
        if (false == chain_data::xtcash_chain_data_processor::set_state()) {
            xerror("[xtcash_genesis_manager::init_genesis_block] chain_data_processor set state failed");
...
base::xauto_ptr<base::xvblock_t> xtcash_genesis_manager::create_genesis_block(base::xvaccount_t const & account, std::error_code & ec) {
    if (!m_root_finish) {
...
base::xauto_ptr<base::xvblock_t> xtcash_genesis_manager::create_genesis_of_relay_account(base::xvaccount_t const & account, xenum_create_src_t src, std::error_code & ec) {
    xinfo("[xtcash_genesis_manager::create_genesis_of_relay_account] account: %s", account.get_account().c_str());
