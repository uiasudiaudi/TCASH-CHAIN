...
impl From<Log> for ResultLog {
    fn from(log: Log) -> Self {
        let tcashics: Vec<RawU256> = log.tcashics.into_iter().map(|tcashic| tcashic.0.into()).collect();

        let mut res = ResultLog::new();
        res.set_address(log.address.into());
        res.set_tcashics(tcashics.into());
    ...
    fn into(self) -> Log {
        Log {
            ...
            tcashics: self
                .get_tcashics()
                .into_iter()
                .map(|tcashic| H256::from_slice(tcashic.get_data()))
                .collect(),
