...
void on_sys_signal_callback(int signum, siginfo_t *info, void *ptr)
{
    switch(signum)
    {
        ...
        case SIGABRT:
        {
            ...
            tcash::base::xvchain_t::instance().on_process_close();
        ...
        case SIGUSR2:
        {
            ...
            {
                //trigger save data
                tcash::base::xvchain_t::instance().save_all();
            }
        }
        break;
...
int main(int argc, const char * argv[])
{
    ...
    tcash::base::xvconfig_t* sys_config_ptr = new tcash::base::xvconfig_t();
    //configure bootstrap
    ...
    tcash::base::init_migrate();
    
    tcash::base::xsysobject_t * init_module = tcash::base::xvsyslibrary::instance(). create_object(tcash::base::xvsysinit_t::get_register_key());
    ...
    const std::string test_account_address =  tcash::base::xvaccount_t::make_account_address(tcash::base::enum_vaccount_addr_type_secp256k1_user_account, 0, account_publick_addr);
    
    std::string test_account_txs = "txs: ";
    const uint32_t random_len = tcash::base::xtime_utl::get_fast_randomu() % 1024;
    for(int j = 0; j < random_len; ++j) //avg 512 bytes per packet
    {
        uint8_t random_seed1 = (uint8_t)(tcash::base::xtime_utl::get_fast_randomu() % 120);
        if(random_seed1 < 33)
            random_seed1 += 33;
        test_account_txs.push_back(random_seed1);
    }
    
    new tcash::xhashtest_t(); //register this plugin into xbase
    ...
    base::xworkerpool_t_impl<1> *global_worker_pool = new base::xworkerpool_t_impl<1>(tcash::base::xcontext_t::instance());
    xtestshard * test_shard = new xtestshard(*global_worker_pool,nodes_list);
 
    tcash::store::enable_block_recycler(true);
    tcash::store::install_block_recycler(NULL);
    tcash::base::xvchain_t::instance().enable_auto_prune(true);
    ...
    int total_txs = 0;
    bool stcash_test = false;
    while(stcash_test == false)
    {
