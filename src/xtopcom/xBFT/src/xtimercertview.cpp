...
NS_BEG2(tcash, xconsensus)

const uint32_t CLOCK_TIME_INTERVAL = 20000;  // 20s
const uint32_t ELASTIC_RANGE = 2;  // 2*clock
const uint64_t tcash_CLOCK_BEGIN_TIME = base::tcash_BEGIN_GMTIME;
...
uint64_t xconspacemaker_t::get_gmt_clock_second() const {
    ...
    long long_gmt = gmt;
    uint64_t clock = long_gmt - tcash_CLOCK_BEGIN_TIME;
    return clock;
}
...
bool xconspacemaker_t::on_object_close() {
    base::xxtimer_t::on_object_close();  // stcash timmer and set close flag
...
bool xconspacemaker_t::on_certificate_finish(const base::xvevent_t & event, xcsobject_t * from_child, const int32_t cur_thread_id, const uint64_t timenow_ms) {
    ...
    return true;  // certificate_finish is internal event, so stcash throw up
}
...
// note: to return false may call child'push_event_down/up,or stcash further routing when return true
