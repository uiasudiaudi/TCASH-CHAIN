...
namespace tcash
{
    namespace chain_data
    {
        ...
        bool xtcash_chain_data_processor::check_state() {
            if (DATA_PROCESS_V != base::xvchain_t::instance().get_xdbstore()->get_value(DATA_PROCESS_K)) {
                return false;
            }
            xdbg("[xtcash_chain_data_processor::check_state] reset already");
            return true;
        }

        bool xtcash_chain_data_processor::set_state() {
            if (!base::xvchain_t::instance().get_xdbstore()->set_value(DATA_PROCESS_K, DATA_PROCESS_V)) {
                xwarn("[xtcash_chain_data_processor::set_state] write db failed");
                return false;
            }
            return true;
        }

        void xtcash_chain_data_processor::get_all_user_data(std::vector<data_processor_t> & data_vec) 
        {
            for (auto it = user_property_json_parse.begin(); it != user_property_json_parse.end(); it++) {
                ...
                data.address = it.key();
                data.tcash_balance = (it->count(data::XPROPERTY_BALANCE_AVAILABLE)) ? base::xstring_utl::touint64(static_cast<std::string>(it->at(data::XPROPERTY_BALANCE_AVAILABLE).get<std::string>())) : 0;
        ...
        std::map<common::xaccount_address_t, data_processor_t> xtcash_chain_data_processor::get_all_user_data() {
        ...
        void xtcash_chain_data_processor::get_user_data(common::xlegacy_account_address_t const &addr, data_processor_t & data) {
        ...
        void xtcash_chain_data_processor::get_all_contract_data(std::vector<data_processor_t> & data_vec) 
        {
        ...
        void xtcash_chain_data_processor::get_contract_data(common::xlegacy_account_address_t const & addr, data_processor_t & data) {
            std::string account = addr.to_string();
        ...
        void xtcash_chain_data_processor::get_stake_string_property(common::xlegacy_account_address_t const & addr, std::string const & property, std::string & value)
        {
        ...
        void xtcash_chain_data_processor::get_stake_map_property(common::xlegacy_account_address_t const & addr,
        ...
        void xtcash_chain_data_processor::release() {
            xdbg("[xtcash_chain_data_processor::release] db reset finish, clear data memory!");
