...
bool EcVHost::SyncMessageWhenStart(const vnetwork::xvnode_address_t & send_address,
    ...
    if (send_address.network_id() == common::xnetwork_id_t{tcash::config::to_chainid(XGET_CONFIG(chain_name))} && send_address.zone_id() == common::xfrozen_zone_id &&
        ...
        if (message_type == xmessage_id_sync_frozen_gossip || message_type == xmessage_id_sync_get_blocks || message_type == xmessage_id_sync_blocks ||
            ...
            tcash_DEBUG("found static xip for sync");
            return true;
        }
    }
    return false;
...
void EcVHost::send_to_through_root(common::xip2_t const & src, common::xnode_id_t const & dst_node_id, xbyte_buffer_t const & byte_message, std::error_code & ec) const {
    ...
    if (!kroot_rt || kroot_rt->nodes_size() == 0) {
        ...
        tcash_WARN("network not joined, send failed, try again ...");
        return;
    }
...
void EcVHost::send_to(common::xnode_id_t const & node_id, xbyte_buffer_t const & bytes_message) const {
    ...
    auto vnetwork_message = tcash::codec::msgpack_decode<vnetwork::xvnetwork_message_t>(bytes_message);
...
void EcVHost::spread_rumor(xbyte_buffer_t const & rumor) const {
    auto vnetwork_message = tcash::codec::msgpack_decode<vnetwork::xvnetwork_message_t>(rumor);
...
void EcVHost::forward_broadcast(const common::xsharding_info_t & shardInfo, common::xnode_type_t node_type, xbyte_buffer_t const & byte_msg) const {
    auto vnetwork_message = tcash::codec::msgpack_decode<vnetwork::xvnetwork_message_t>(byte_msg);
