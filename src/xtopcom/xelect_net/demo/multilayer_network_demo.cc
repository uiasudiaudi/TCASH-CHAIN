...
int MultilayerNetworkDemo::HandleParamsAndConfig(int argc, char ** argv, tcash::base::Config & edge_config) {
    tcash::ArgsParser args_parser;
    if (ParseParams(argc, argv, args_parser) != tcash::kadmlia::kKadSuccess) {
...
bool MultilayerNetworkDemo::ResetEdgeConfig(tcash::ArgsParser & args_parser, tcash::base::Config & edge_config) {
    std::string node_id;
    if (args_parser.GetParam("n", node_id) == tcash::kadmlia::kKadSuccess) {
    ...
    std::string db_path;
    if (args_parser.GetParam("d", db_path) == tcash::kadmlia::kKadSuccess) {
    ...
    int zone_id;
    if (args_parser.GetParam("z", zone_id) == tcash::kadmlia::kKadSuccess) {
    ...
    if (args_parser.GetParam("P", local_port) == tcash::kadmlia::kKadSuccess) {
    ...
    int show_cmd = 1;
    if (args_parser.GetParam("g", show_cmd) == tcash::kadmlia::kKadSuccess) {
    ...
    std::string log_path;
    if (args_parser.GetParam("L", log_path) == tcash::kadmlia::kKadSuccess) {
...
bool MultilayerNetworkDemo::GenerateXelectNetDemoNodes(const tcash::base::Config & edge_config) {
    int total_node = 100;
    ...
    tcash::base::enum_vaccount_addr_type account_address_type = static_cast<tcash::base::enum_vaccount_addr_type>('0');
    tcash::base::enum_xchain_zone_index zone_index{tcash::base::enum_chain_zone_consensus_index};
    uint16_t ledger_id = tcash::base::xvaccount_t::make_ledger_id(static_cast<tcash::base::enum_xchain_id>(network_id.value()), zone_index);
    for (int i = 1; i < total_node; ++i) {
        auto hash = tcash::utl::xsha2_512_t::digest(std::to_string(std::time(nullptr) + i));
        auto prefix_account = tcash::utl::xcrypto_util::make_address_by_assigned_key(hash.data(), account_address_type, ledger_id);
...
int MultilayerNetworkDemo::ParseParams(int argc, char ** argv, tcash::ArgsParser & args_parser) {
    ...
    args_parser.AddArgType('f', "first_node", tcash::kMaybeValue);
    args_parser.AddArgType('p', "public_endpoints", tcash::kMaybeValue);
    args_parser.AddArgType('n', "node_id", tcash::kMaybeValue);  // node_id, usually account id(not kroot id or kad_key)
    args_parser.AddArgType('v', "version", tcash::kNoValue);
    args_parser.AddArgType('x', "xelect_net_demo", tcash::kMaybeValue);

    args_parser.AddArgType(0, "test1", tcash::kMaybeValue);
    args_parser.AddArgType(0, "test2", tcash::kMaybeValue);
    ...
    std::string err_pos;
    if (args_parser.Parse(tmp_params, err_pos) != tcash::kadmlia::kKadSuccess) {
...
void MultilayerNetworkDemo::Stcash() {
    elect_manager_.reset();

    // MultilayerNetwork::Stcash();
}
