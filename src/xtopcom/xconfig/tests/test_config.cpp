...
class xconfig_register_listener_face_mock_t
    : public tcash::config::xconfig_register_listener_face_t {
public:
...
*/
    ...
    virtual int32_t string_get(const std::string& address, const std::string& key, std::string& value) {
        if(key == "update_action_type") {
            value = m_action_type;
            return tcash::store::xstore_success;
        }
        return -1;
    }
    ...
    void notify_update() {
        if (m_mbus) {
            tcash::mbus::xevent_store_ptr_t ptr = std::make_shared<tcash::mbus::xevent_store_t>(
                tcash::mbus::xevent_store_t::type_config_update,
...
TEST(xconfig_register, initial_onchain_load) {
    ...
    std::shared_ptr<tcash::xbase_io_context_wrapper_t> io_object = std::make_shared<tcash::xbase_io_context_wrapper_t>();
    std::shared_ptr<tcash::xbase_timer_driver_t> timer_driver = std::make_shared<tcash::xbase_timer_driver_t>(io_object);
