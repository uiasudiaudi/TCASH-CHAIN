...
using xquic_message_ready_callback = std::function<void(tcash::xbytes_t const &, std::string const &, std::size_t)>;
...
struct cli_user_stream_t {
    ...
    std::deque<tcash::xbyte_t> send_queue;

    tcash::xbytes_t send_buffer;
...
private:
    ...
    tcash::threading::xthreadsafe_queue<cli_user_conn_t *, std::vector<cli_user_conn_t *>> m_conn_queue{max_conn_queue_size};

    constexpr static std::size_t max_send_queue_size{100000};
    tcash::threading::xthreadsafe_queue<std::unique_ptr<client_send_buffer_t>, std::vector<std::unique_ptr<client_send_buffer_t>>> m_send_queue{max_send_queue_size};
...
public:
    ...
    bool send(cli_user_conn_t * cli_user_conn, tcash::xbytes_t send_data);
