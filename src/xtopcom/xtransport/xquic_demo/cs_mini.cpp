...
static tcash::xbytes_t xquic_client_generate_ping_packet_data(std::size_t server_inbound_port) {
    return tcash::to_bytes(std::string{PINGPACKETVERSION + std::to_string(server_inbound_port)});
}

static bool xquic_server_handle_ping_packet_data(tcash::xbytes_t const & ping_packet, std::size_t & peer_inbound_port) {
...
static tcash::xbytes_t size_to_bytes(std::size_t len) {
    // auto f = len;
    tcash::xbytes_t res(4, (tcash::xbyte_t)0);
    for (std::size_t index = 0; index < 4; index++) {
        res[3 - index] = (tcash::xbyte_t)(len % 256);
        len /= 256;
        if (len == 0)
            break;
    }
...
static std::size_t four_bytes_to_size(tcash::xbytes_t head) {
    assert(head.size() == 4);
    std::size_t res = 0;
    for (std::size_t index = 0; index < 4; index++) {
        res *= 256;
...
int xquic_server_stream_read_notify(xqc_stream_t * stream, void * user_data) {
    ...
    tcash::xbytes_t & result = srv_user_stream->recv_buffer;
    ...
    do {
        ...
        do {
            ...
            if (!has_recv_block_len && result.size() > 4) {
                no_more_cb = false;
                block_len = four_bytes_to_size(tcash::xbytes_t{result.begin(), result.begin() + 4});
            ...
            if (has_recv_block_len && result.size() >= block_len) {
                ...
                tcash::xbytes_t data_buffer = tcash::xbytes_t{result.begin(), result.begin() + block_len};
...
bool xquic_client_t::send(cli_user_conn_t * cli_user_conn, tcash::xbytes_t send_data) {
    std::unique_ptr<client_send_buffer_t> send_buffer_ptr = tcash::make_unique<client_send_buffer_t>();
