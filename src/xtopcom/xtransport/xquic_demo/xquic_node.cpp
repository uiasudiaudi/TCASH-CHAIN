...
void xquic_node_t::start() {
    ...
    tcash::threading::xbackend_thread::spawn([this, self] {
#if defined DEBUG
    ...
    tcash::threading::xbackend_thread::spawn([this, self] { m_client.init(m_server_port); });
...
bool xquic_node_t::send(std::string addr, uint32_t port, tcash::xbytes_t data) {
...
void xquic_node_t::on_quic_message_ready(tcash::xbytes_t const & bytes) {
    global_recv_cnt++;
...
int main(int argc, char * argv[]) {
    ...
    // tcash::xbytes_t send_data{data.begin(), data.end()};
    ...
    while ((ch = getcasht(argc, argv, "a:p:t:w:s:r:z:")) != -1) {
        switch (ch) {
        case 'a': {
