...
UdpProperty::~UdpProperty() {
    if (m_xudp_ != nullptr) {
        tcash_DBG_INFO("~UdpProperty:%p, xudp:%p, get_refcount:%d.call release_ref", this, m_xudp_, m_xudp_->get_refcount());
...
void UdpProperty::SetXudp(xp2pudp_t * xudp_in) {
    ...
    if (xudp_in != nullptr) {
        tcash_DBG_INFO("set xudp:%p, get_refcount:%d.this:%p. call add_ref", xudp_in, xudp_in->get_refcount(), this);
        xudp_in->add_ref();
        xudp_in->add_linkrefcount();
    }

    if (old != nullptr) {
        tcash_DBG_INFO("set xudp:%p, get_refcount:%d.this:%p. call release_ref.", old, old->get_refcount(), this);
...
xp2pudp_t::xp2pudp_t(xcontext_t & _context, xendpoint_t * parent, const int32_t target_thread_id, int64_t virtual_handle, xsocket_property & property, XudpSocket * listen_server)
  : xudp_t(_context, parent, target_thread_id, virtual_handle, property) {
    m_link_refcount = 0;
    m_status = tcash::transport::enum_xudp_status::enum_xudp_init;
    listen_server_ = listen_server;
    quic_node_ = listen_server->quic_node_;
}

int xp2pudp_t::add_linkrefcount() {
    tcash_DBG_INFO("add_linkrefcount:xudp:%p, m_link_refcount:%d, get_refcount:%d", this, m_link_refcount.load(), get_refcount());
    return ++m_link_refcount;
}
int xp2pudp_t::release_linkrefcount() {
    tcash_DBG_INFO("release_linkrefcount,xudp:%p, m_link_refcount:%d, get_refcount:%d", this, m_link_refcount.load(), get_refcount());
    return --m_link_refcount;
    ...
    //     } else {
    //         tcash_INFO("listen_server_ is null,xudp:%p", this);
    //     }
    // }
    // tcash_DBG_INFO("release_linkrefcount,xudp:%p, m_link_refcount:%d, get_refcount:%d", this, m_link_refcount.load(), get_refcount());
...
bool xp2pudp_t::on_endpoint_open(const int32_t error_code, const int32_t cur_thread_id, const uint64_t timenow_ms, xendpoint_t * from_child) {
    ...
    tcash_DBG_INFO(
        ...
        get_refcount(),
        m_status);
    if (m_status != tcash::transport::enum_xudp_status::enum_xudp_closed && m_status != tcash::transport::enum_xudp_status::enum_xudp_closed_released)
        m_status = tcash::transport::enum_xudp_status::enum_xudp_connected;
    else
        return result;
    ...
    if (listen_server_ != nullptr) {
        ...
        tcash_INFO("listen_server_ is null");
    }
    return result;
}
...
bool xp2pudp_t::on_endpoint_close(const int32_t error_code, const int32_t cur_thread_id, const uint64_t timenow_ms, xendpoint_t * from_child) {
    tcash_DBG_INFO(
        ...
        get_peer_logic_port_token(),
        get_refcount());
    m_status = tcash::transport::enum_xudp_status::enum_xudp_closed;
...
int32_t xp2pudp_t::connect_xudp(const std::string & target_ip, const uint16_t target_port, XudpSocket * udp_server) {
    ...
    if (m_status != tcash::transport::enum_xudp_status::enum_xudp_closed && m_status != tcash::transport::enum_xudp_status::enum_xudp_closed_released)
        m_status = tcash::transport::enum_xudp_status::enum_xudp_connecting;
    else
        return 0;
    tcash_DBG_INFO("connect,this:%p,ip:%s,port:%d,listen_ptr:%p, get_refcount:%d", this, target_ip.c_str(), target_port, udp_server, get_refcount());
...
int32_t xp2pudp_t::recv(uint64_t from_xip_addr_low,
    ...
    if (TransportFilter::Instance()->BlackIpCheck(packet.get_from_ip_addr())) {
        tcash_WARN("on_recv deny ddos black_list ip:%s", packet.get_from_ip_addr().c_str());
    ...
    if (packet.get_size() <= (int)enum_xbase_header_len) {
        tcash_ERROR("packet.size(%d) invalid", (int)packet.get_size());
        return 0;
    }

    /*
    tcash_DEBUG("Recv size: %d, local address[%s : %d]-[%d : %d],peer address[%s : %d]-[%d : %d]", packet.get_body().size(),
...
XudpSocket::XudpSocket(base::xcontext_t & _context, int32_t target_thread_id, xfd_handle_t native_handle, MultiThreadHandler * message_handler, quic::xquic_node_t * quic_node)
  : xudplisten_t(_context, NULL, target_thread_id, native_handle), multi_thread_message_handler_(message_handler), quic_node_{quic_node} {
    ...
    tcash_INFO("using mgr thread(%d) to manage all xudp_t objects", m_xudpsocket_mgr_thread_id);
}

XudpSocket::~XudpSocket() {
    Stcash();
    // dont stcash/close xudpsocket_mgr_thread that could be shared by other XudpSocket objects
}
xslsocket_t * XudpSocket::create_xslsocket(xendpoint_t * parent, xfd_handle_t handle, xsocket_property & property, int32_t cur_thread_id, uint64_t timenow_ms) {
    //    property._peer_account_id = tcash::global_node_id;
    xp2pudp_t * udp_ptr = new xp2pudp_t(*get_context(), parent, m_xudpsocket_mgr_thread_id, handle, property, this);
    tcash_DBG_INFO("create_xslsocket:%p,get_refcount:%d at thread(%d)", udp_ptr, udp_ptr->get_refcount(), m_xudpsocket_mgr_thread_id);
    return udp_ptr;
}
xslsocket_t * XudpSocket::on_xslsocket_accept(xfd_handle_t handle, xsocket_property & property, int32_t cur_thread_id, uint64_t timenow_ms) {
    ...
    if (property._peer_payload != XUDP_VERSION) {
        tcash_INFO("on_xslsocket_accept,account:%s,sign_size:%s,payload:%s.authentication fail.",
    ...
    tcash_INFO("on_xslsocket_accept,account:%s,sign_size:%s,payload:%s.authentication succ.",
             property._peer_account_id.c_str(),
             HexEncode(property._peer_signature).c_str(),
             property._peer_payload.c_str());
    ...
    if (!AddToConlimitMap(property._peer_ip_addr)) {
        tcash_WARN("on_xslsocket_accept deny connection for same ip_seg ip:%s", property._peer_ip_addr.c_str());
        return nullptr;
    }

    if (TransportFilter::Instance()->BlackIpCheck(property._peer_ip_addr)) {
        tcash_WARN("on_xslsocket_accept deny ddos black_list ip:%s", property._peer_ip_addr.c_str());
        return nullptr;
    }
#endif
...
int XudpSocket::SendToLocal(base::xpacket_t & packet) {
    if (packet.get_size() <= (int)enum_xbase_header_len) {
        tcash_ERROR("<blueshi> packet.size(%d) invalid", (int)packet.get_size());
        return kTransportFailed;
    }
    tcash_DBG_INFO("SendToLocal:%d", packet.get_size());
...
int XudpSocket::SendDataWithProp(base::xpacket_t & packet, UdpPropertyPtr & udp_property) {
    // tcash_FATAL("SendData(from(%s:%d), to(%s:%d), size=%d)", packet.get_from_ip_addr().c_str(), packet.get_from_ip_port(),
    ...
    if (udp_property != nullptr) {
        ...
        if (prop_xudp_ptr != nullptr) {
            if (prop_xudp_ptr->GetStatus() != tcash::transport::enum_xudp_status::enum_xudp_closed &&
                prop_xudp_ptr->GetStatus() != tcash::transport::enum_xudp_status::enum_xudp_closed_released && (false == prop_xudp_ptr->is_close())) {
                if (prop_xudp_ptr->send(packet) != enum_xcode_successful) {
                    tcash_WARN("send xpacket failed!packet size is :%d\n", packet.get_size());
                    return kTransportFailed;
                }

                tcash_DEBUG("send packet using node_ptr,to:%s:%d, size:%d, xudp:%p, prop:%p",
    ...
    if (packet.get_to_ip_port() == 0) {
        ...
        tcash_INFO("SendData error:%d,%s,%d,%s", pro_message.type(), pro_message.des_node_id().c_str(), pro_message.id(),packet.get_to_ip_addr().c_str());
        return kTransportFailed;
    }
    ...
    if (iter == xudp_client_map.end()) {
        if ((ret = CheckRatelimitMap(to_addr)) != enum_xcode_successful) {
            tcash_ERROR("reach xudp connection rate limit, drop this packet:%s,ret:%d", to_addr.c_str(), ret);
            return kTransportFailed;
        }
        tcash_DEBUG("not find:%s, size:%d", to_addr.c_str(), packet.get_body().size());
        ...
        if (GetSign(node_sign) != enum_xcode_successful) {
            tcash_ERROR("get sign failed.");
            return kTransportFailed;
        }
        tcash_DEBUG("xudp first connect %s:%u", packet.get_to_ip_addr().c_str(), packet.get_to_ip_port());
        ...
        tcash_DEBUG("conn %s:%p", to_addr.c_str(), peer_xudp_socket);
        AddToRatelimitMap(to_addr);
    } else {
        peer_xudp_socket = xudp_client_map[to_addr];
        tcash_DEBUG("find:%s, size:%d", to_addr.c_str(), packet.get_body().size());

        if (peer_xudp_socket->is_close() || (peer_xudp_socket->GetStatus() == tcash::transport::enum_xudp_status::enum_xudp_closed) ||
            (peer_xudp_socket->GetStatus() == tcash::transport::enum_xudp_status::enum_xudp_closed_released)) {
            tcash_INFO("wait reconnect... release_ref:%p, get_refcount:%d", peer_xudp_socket, peer_xudp_socket->get_refcount());

            if (m_offline_cb != nullptr) {
                m_offline_cb(peer_xudp_socket->get_peer_ip_address(), peer_xudp_socket->get_peer_real_port());
            }
            peer_xudp_socket->SetStatus(tcash::transport::enum_xudp_status::enum_xudp_closed_released);
            ...
            if ((ret = CheckRatelimitMap(to_addr)) != enum_xcode_successful) {
                tcash_ERROR("reach xudp connection rate limit2, drop this packet:%s, ret:%d", to_addr.c_str(), ret);
                return kTransportFailed;
            }
            std::string node_sign;
            if (GetSign(node_sign) != enum_xcode_successful) {
            ...
            tcash_DEBUG("xudp reconnect %s:%u", packet.get_to_ip_addr().c_str(), packet.get_to_ip_port());
            peer_xudp_socket = (xp2pudp_t *)xudplisten_t::create_xslsocket(global_node_id, node_sign, XUDP_VERSION, enum_socket_type_xudp);
            peer_xudp_socket->connect_xudp(packet.get_to_ip_addr(), packet.get_to_ip_port(), this);
            ...
            XMETRICS_GAUGE(metrics::xtransport_xudp_num, 1);
            AddToRatelimitMap(to_addr);
            tcash_INFO("reconn %s:%p", to_addr.c_str(), peer_xudp_socket);
        }
    }

    if (udp_property != nullptr) {
        udp_property->SetXudp(peer_xudp_socket);
        tcash_DEBUG("setxudp,udp_property:%p,xudp:%p", udp_property.get(), peer_xudp_socket);
    } else {
        tcash_DEBUG("udp_property null");
    }

    if (peer_xudp_socket->send(packet) != enum_xcode_successful) {
        tcash_ERROR("send xpacket failed!packet size is :%d\n", packet.get_size());
        return kTransportFailed;
    }
    tcash_DEBUG("send xpacket packet size:%d src %s:%d dest %s:%d\n",
              packet.get_size(),
...
int XudpSocket::GetSign(std::string & node_sign) {
    ...
    node_sign = tcash::safebox::xsafebox_proxy::get_instance().get_proxy_secp256_signature(global_node_pubkey, hash_value);

    tcash_INFO("node_id:%s,node_sign:%s", global_node_id.c_str(), HexEncode(node_sign).c_str());
    return enum_xcode_successful;
}
bool XudpSocket::CloseXudp(xp2pudp_t * xudpobj_ptr) {
    if (xudpobj_ptr == nullptr) {
        tcash_WARN("close xudp fail:null");
        return false;
    }
#ifdef ENABLE_XSECURITY
...

    {
        ...
        if (it == xudp_client_map.end() || it->second != xudpobj_ptr) {
            /*
            for (it= xudp_client_map.begin(); it != xudp_client_map.end(); ++it) {
                tcash_INFO("print xudp map:%s,%p",it->first.c_str(), it->second);
            }
            */
            tcash_WARN("close xudp: not find in map:%p.to_addr:%s", xudpobj_ptr, to_addr.c_str());
            return false;
        }
    ...
    tcash_DBG_INFO("close xudp: find in map:%p, get_refcount:%d.call release_ref. close ip:%s:%d",
                 xudpobj_ptr,
    ...
    xudpobj_ptr->SetStatus(tcash::transport::enum_xudp_status::enum_xudp_closed_released);
...
bool XudpSocket::GetSocketStatus() {
    if (this->get_socket_status() != tcash::base::enum_socket_status_connected) {
        return false;
    }
    return true;
}
...
int XudpSocket::AddXudp(const std::string & ip_port, xp2pudp_t * new_xudp) {
    ...
    if (new_xudp->GetStatus() == tcash::transport::enum_xudp_status::enum_xudp_closed) {
        tcash_INFO("new_xudp is closed");
        return 1;
    } else if (new_xudp->GetStatus() == tcash::transport::enum_xudp_status::enum_xudp_closed_released) {
        tcash_INFO("new_xudp is closed and released");
        return 1;
    }
    ...
    xp2pudp_t * existing_ptr = nullptr;
    {
        ...
        if (iter != xudp_client_map.end()) {
            ...
            XMETRICS_GAUGE(metrics::xtransport_xudp_num, 1);
            tcash_DBG_INFO("conn ok %s:%p.call add_ref:%d", ip_port.c_str(), new_xudp, new_xudp->get_refcount());
            return enum_xcode_successful;
        }

        if (existing_ptr == new_xudp) {
            tcash_INFO("need not add this xudp:%p", existing_ptr);
            return 0;
        }

        new_xudp->add_ref();
        xudp_client_map[ip_port] = new_xudp;
        tcash_DBG_INFO("add to map:to_addr:%s,xudp:%p.call add_ref:%d", ip_port.c_str(), new_xudp, new_xudp->get_refcount());
    }

    if (existing_ptr != NULL) {
        if (existing_ptr->GetStatus() == tcash::transport::enum_xudp_status::enum_xudp_closed_released) {
            tcash_DBG_INFO("this xudp already released,addr:%s,old xudp:%p,new xudp:%p, get_refcount:%d", ip_port.c_str(), existing_ptr, new_xudp, existing_ptr->get_refcount());
            return 0;
        }
        tcash_DBG_INFO("replace:%s, replaced:%p, new:%p, get_refcount:%d. call release_ref", ip_port.c_str(), existing_ptr, new_xudp, existing_ptr->get_refcount());

        //      if (m_offline_cb != nullptr) {
        //          m_offline_cb(existing_ptr->get_peer_ip_address(), existing_ptr->get_peer_real_port());
        //      }
        existing_ptr->SetStatus(tcash::transport::enum_xudp_status::enum_xudp_closed_released);
...
int XudpSocket::CheckRatelimitMap(const std::string & to_addr) {
    ...
    while (1) {
        conn_it = conns.begin();
        if (tcash::base::xtime_utl::gettimeofday() - *conn_it < XUDP_RATELIMIT_INTERVAL * 60) {  // recent connections
            break;
        }
        conns.pop_front();
        tcash_DBG_INFO("delete old connection:%s, remains:%d", to_addr.c_str(), conns.size());
        if (conns.empty())
            break;
    }
    ...
    for (conn_it = conns.begin(); conn_it != conns.end(); ++conn_it) {
        if (tcash::base::xtime_utl::gettimeofday() - *conn_it < 60)  // count connections within one minute
            count_one_minute++;
    }
...
uint32_t XudpSocket::IpToUInt(const std::string & ip) {
    // SUB_NETWORK_MASK
    if (ip.empty()) {
        tcash_WARN("iptouint failed, ip empty");
        return 0;
    }
    uint32_t a, b, c, d;
    uint32_t addr = 0;
    if (sscanf(ip.c_str(), "%u.%u.%u.%u", &a, &b, &c, &d) != 4) {
        tcash_WARN("iptouint failed, ip:%s invalid", ip.c_str());
        return 0;
    }
...
bool XudpSocket::AddToConlimitMap(const std::string & ip) {
    ...
    if (ip_addr == 0 || mask_addr == 0) {
        tcash_WARN("connect limit map ip:%s invalid", ip.c_str());
        return false;
    }
...

    {
        ...
        if (xudp_conlimit_map_[ip_segment] >= IP_SEG_MAX_ADDR) {
            tcash_WARN("ip:%s ip_seg:%u reach max_num:%u, deny connect", ip.c_str(), ip_segment, IP_SEG_MAX_ADDR);
            return false;
        }
...
void XudpSocket::RemoveConlimitMap(const std::string & ip) {
    ...
    tcash_DEBUG("remove connection limitmap for ip:%s", ip.c_str());
    return;
}
#endif
...
int XudpSocket::SendPing(base::xpacket_t & packet) {
    ...
    tcash_DEBUG("send_ping_packet, ret = %d", ret);
    return ret;
}
