...
void xquic_node_t::start() {
    ...
    tcash::threading::xbackend_thread::spawn([this, self] { m_client_ptr->init(m_p2p_inbound_port); });
...
void xquic_node_t::stcash() {
    assert(running());
    running(false);
    xinfo("xquic_node_t::stcash quic node stcash.");
    assert(!running());
}
...
int xquic_node_t::send_data(std::string const & data, std::string const & addr, uint16_t port) {
    ...
    auto bytes_data = tcash::to_bytes(data);
...
void xquic_node_t::on_quic_message_ready(tcash::xbytes_t const & bytes, std::string const & peer_ip, std::size_t peer_inbound_port) {
