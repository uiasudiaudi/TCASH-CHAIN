...
std::shared_ptr<xtcash_unit_state_sync> xtcash_unit_state_sync::new_state_sync(const common::xaccount_address_t & account,
    ...
    auto sync = std::make_shared<xtcash_unit_state_sync>();
    ...
    xinfo("xtcash_unit_state_sync::new_state_sync unit: %s, height: %lu, root: %s", account.to_string().c_str(), height, to_hex(block_hash).c_str());
    return sync;
}

void xtcash_unit_state_sync::run() {
    sync_unit(m_ec);
    if (m_ec) {
        xwarn("xtcash_unit_state_sync::run sync_unit error, unit: %s, index: %s, error: %s, %s",
...
void xtcash_unit_state_sync::wait() const {
    while (!m_done) {}
    return;
}

void xtcash_unit_state_sync::cancel() {
    m_cancel = true;
    wait();
}

std::error_code xtcash_unit_state_sync::error() const {
    return m_ec;
}

std::string xtcash_unit_state_sync::symbol() const {
    return m_symbol;
}

sync_result xtcash_unit_state_sync::result() const {
...
bool xtcash_unit_state_sync::is_done() const {
    return m_done;
}

void xtcash_unit_state_sync::deliver_req(const state_req & req) {
    {
...
void xtcash_unit_state_sync::sync_unit(std::error_code & ec) {
    // check exist
    auto unitstate = m_store->get_unit_state_by_accountindex(m_account, m_index);
    if (unitstate != nullptr) {
        xinfo("xtcash_unit_state_sync::sync_unit state already exist in db, {%s}", symbol().c_str());
        m_done = true;
        return;
    }
...
void xtcash_unit_state_sync::assign_unit_tasks(const sync_peers & peers) {
    ...
    xinfo("xtcash_unit_state_sync::assign_unit_tasks %s, id: %u, start sync", symbol().c_str(), m_req_sequence_id);
    state_req req;
...
void xtcash_unit_state_sync::process_unit(state_req & req, std::error_code & ec) {
    ...
    if (req.units_response.empty()) {
        xwarn("xtcash_state_sync::process_unit empty, %s", symbol().c_str());
        return;
    }
    // auto & data = req.units_response.at(0);
    // xinfo("xtcash_state_sync::process_unit value size: %zu, {%s}", data.size(), symbol().c_str());
    // {
    //     // check state
    ...
    //         xwarn("xtcash_unit_state_sync::process_unit state hash mismatch: %s, %s", to_hex(hash).c_str(), to_hex(m_index.get_latest_state_hash()).c_str());
    //         return;
    //     }
    // }
...
void xtcash_unit_state_sync::loop(std::function<bool()> condition,
    ...
    if (net.network == nullptr) {
        xwarn("xtcash_unit_state_sync::loop no network availble, exit, {%s}", symbol().c_str());
        ec = error::xerrc_t::state_network_invalid;
        m_cancel = true;
        return;
    }
    while (condition()) {
        xdbg("xtcash_unit_state_sync::loop unit finish: %d, {%s}", m_sync_unit_finish.load(), symbol().c_str());
        // step2: check peers
        if (net.peers.empty()) {
            xwarn("xtcash_unit_state_sync::loop peers empty, exit, {%s}", symbol().c_str());
            ec = error::xerrc_t::state_network_invalid;
            m_cancel = true;
            return;
        }
...
common::xnode_address_t xtcash_unit_state_sync::send_message(const sync_peers & peers, const xbytes_t & msg, common::xmessage_id_t id) {
