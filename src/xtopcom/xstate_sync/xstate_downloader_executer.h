...
class xtcash_download_executer {
#if !defined(NDEBUG)
    std::thread::id executor_thread_id_;
#endif

public:
    explicit xtcash_download_executer(observer_ptr<base::xiothread_t> thread, uint32_t overtime);
    ~xtcash_download_executer() = default;
...
using xdownload_executer_t = xtcash_download_executer;
