...
//<contract_address, map<tcashics, pair<speed_type, chainid_bits>>>
...
static cross_chain_contract_info get_cross_chain_config() {
    ...
    for (auto & str : str_vec) {
        ...
        cross_chain_config[addr][tcashic] =  std::make_pair(tx_seppd_type,chain_bit);
    }
...
static bool cross_tx_info_check_and_get(const evm_common::xevm_logs_t& logs, const cross_chain_contract_info& cross_chain_config,
    ...
    for (auto& log : logs) {
        ...
        std::string tcashic_hex = tcash::to_hex_prefixed(log.tcashics[0].asBytes());
        auto it2 = it->second.find(tcashic_hex);
...
std::vector<data::xcons_transaction_ptr_t> xtable_cross_plugin_t::make_contract_txs_after_execution(statectx::xstatectx_ptr_t const& statectx_ptr, uint64_t timestamp,
    std::vector<tcash::txexecutor::xatomictx_output_t> const& pack_outputs, std::error_code& ec)
{
    ...
    for (auto& txout : pack_outputs) {
        ...
        if (!cross_tx_info_check_and_get(evm_result.logs, cross_chain_config, speed_type, chain_bit)) {
            // xdbg("xtable_cross_plugin_t::make_contract_txs_after_execution  tcashic not match.tx:%s is not a cross chain tx", tcash::to_hex_prefixed(tcash::to_bytes(txout.m_tx.get_tx_hash())).c_str());
            continue;
        }
