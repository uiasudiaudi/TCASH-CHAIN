...
TEST(string_utl, base_split) {
    ...
    tcash::base::xstring_utl::split_string(orig_str, ',', sub_str);
...
TEST(xmerkle, sha_test) {
    ...
    xtest_digest_wrap_t<tcash::utl::xsha2_256_t, tcash::uint256_t> dw1("this");
    h.reset();
    h.update(string("this"));
    h.get_hash(v);
    ASSERT_TRUE(string((char*) v.data(), v.size()) == dw1.hash_list.back());

    xtest_digest_wrap_t<tcash::utl::xsha2_256_t, tcash::uint256_t> dw2("is");
    h.reset();
    h.update(string("is"));
    h.get_hash(v);
    ASSERT_TRUE(string((char*) v.data(), v.size()) == dw2.hash_list.back());

    xtest_digest_wrap_t<tcash::utl::xsha2_256_t, tcash::uint256_t> dw3("atest");
    h.reset();
    h.update(string("a"));
...
TEST(xmerkle_path_node_t, stream) {
    tcash::base::xstream_t s(tcash::base::xcontext_t::instance());
    tcash::xmerkle_path_node_t < tcash::uint256_t > n, n1;
...
TEST(xmerkle, calc_root) {
    tcash::xmerkle_t<tcash::utl::xsha2_256_t, tcash::uint256_t> m;
    ...
    for (int i = 0; i < 9; i++) {
        xtest_digest_wrap_t<tcash::utl::xsha2_256_t, tcash::uint256_t> dw(strs[i]);
...
TEST(xmerkle, calc_path) {
    std::vector<tcash::xmerkle_path_node_t < tcash::uint256_t >> hash_path;
    tcash::xmerkle_t<tcash::utl::xsha2_256_t, tcash::uint256_t> m;
    ...
    xtest_digest_wrap_t<tcash::utl::xsha2_256_t, tcash::uint256_t> dw2("this is");
    ...
    xtest_digest_wrap_t<tcash::utl::xsha2_256_t, tcash::uint256_t> dw3("this is a");
    int dw3_indexes[3][2]{
        {1, 4},
        {0, 4},
        {2, 3}
    ...
    xtest_digest_wrap_t<tcash::utl::xsha2_256_t, tcash::uint256_t> dw9("this is a test for merkle tree totally 9");
    int dw9_indexes[8][4]{
        {1, 10, 15, 18},
        {0, 10, 15, 18},
        {3, 9, 15, 18},
...
TEST(xmerkle, validate_path) {
    ...
    for(int i=1;i<=1025;i++) {
        std::shared_ptr<xtest_digest_wrap_t<tcash::utl::xsha2_256_t, tcash::uint256_t>> ptr =
                xtest_digest_wrap_t<tcash::utl::xsha2_256_t, tcash::uint256_t>::create_object(i);
        if (!(i % 10)) {
...
TEST(xpriority_queue, tests) {
    ...
    tcash::utl::xpriority_queue_t<xtest_priority_queue_ptr_t> q(3);
