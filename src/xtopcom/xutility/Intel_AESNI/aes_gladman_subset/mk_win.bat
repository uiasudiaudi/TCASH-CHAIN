...
if "%arch%" == "86" (
	...
	set src=aeskey aesaux aes_modes modetest process_options my_getcasht
	) else (
	set sz=32
	set asm=aes_x86_v1
	set defines= -D ASM_X86_V1C
	set options=
	set src=aesaux aes_modes modetest aeskey aestab process_options my_getcasht
	)
)
if "%arch%" == "64" (
	set sz=64
	...
	set src=aeskey aestab aesaux aes_modes modetest process_options my_getcasht
)
pushd .
cd ..\intel_aes_lib
