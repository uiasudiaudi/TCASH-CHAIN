...
public:
    xtcash_trie(xtcash_trie const &) = delete;
    xtcash_trie & operator=(xtcash_trie const &) = delete;
    xtcash_trie(xtcash_trie &&) = default;
    xtcash_trie & operator=(xtcash_trie &&) = default;
    ~xtcash_trie();

protected:
    explicit xtcash_trie(observer_ptr<xtrie_db_t> db, xh256_t const & root_hash);

public:
    observer_ptr<xtrie_db_t> trie_db() const noexcept;

    static std::unique_ptr<xtcash_trie> build_from(xh256_t const & hash, observer_ptr<xtrie_db_t> db, std::error_code & ec);
