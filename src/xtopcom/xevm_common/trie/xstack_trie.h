...
enum class xtcash_stack_trie_node_type : uint8_t {
    emptyNode = 0,
    branchNode = 1,
    extNode = 2,
    leafNode = 3,
...
using xstack_trie_node_type_t = xtcash_stack_trie_node_type;

// class xtcash_stack_trie;
...
private:
    ...
    xbytes_t m_val;
    xbytes_t m_key;
    std::array<std::shared_ptr<xtcash_stack_trie>, 16> m_children;
    xkv_db_face_ptr_t m_db;

public:
    xtcash_stack_trie(xkv_db_face_ptr_t _db) : m_db{_db} {
    }

public:
    static std::shared_ptr<xtcash_stack_trie> NewStackTrie(xkv_db_face_ptr_t db);
...
private:
    std::shared_ptr<xtcash_stack_trie> newLeaf(xbytes_t const & key, xbytes_t const & val);
    std::shared_ptr<xtcash_stack_trie> newExt(xbytes_t const & key, std::shared_ptr<xtcash_stack_trie> child);
};

using xstack_trie_t = xtcash_stack_trie;
