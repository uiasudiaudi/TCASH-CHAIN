...
class xtcash_secure_trie final : public xtrie_face_t {
public:
    xtcash_secure_trie(xtcash_secure_trie const &) = delete;
    xtcash_secure_trie & operator=(xtcash_secure_trie const &) = delete;
    xtcash_secure_trie(xtcash_secure_trie &&) = default;
    xtcash_secure_trie & operator=(xtcash_secure_trie &&) = default;
    ~xtcash_secure_trie() override = default;

private:
    std::unique_ptr<xtrie_t> trie_;
    // xbytes_t hashKeyBuf;
    // std::shared_ptr<std::map<std::string, xbytes_t>> secKeyCache;
    // xtcash_secure_trie * secKeyCacheOwner{nullptr};  // Pointer to self, replace the key cache on mismatch

protected:
    explicit xtcash_secure_trie(std::unique_ptr<xtrie_t> trie);
...
public:
    ...
    static std::unique_ptr<xtcash_secure_trie> build_from(xh256_t const & root, observer_ptr<xtrie_db_t> db, std::error_code & ec);
    ...
    //std::shared_ptr<xtcash_secure_trie> copy() {
    //    return std::make_shared<xtcash_secure_trie>(*this);
    //}
...
using xsecure_trie_t = xtcash_secure_trie;
