...
bool verify_eip1559_header(xeth_header_t const & parent_header, xeth_header_t const & header) {
    ...
    if (!heco::verify_gaslimit(parent_gas_limit, header.gas_limit)) {
        xwarn("[xtcash_evm_eth_bridge_contract::verifyEip1559Header] gaslimit mismatch, new: %s, old: %s", header.gas_limit.str().c_str(), parent_header.gas_limit.str().c_str());
        return false;
    }
    auto const expected_base_fee = calc_base_fee(parent_header);
    if (header.base_fee_per_gas.value() != expected_base_fee) {
        xwarn("[xtcash_evm_eth_bridge_contract::verifyEip1559Header] wrong basefee: %" PRIu64 ", should be: %" PRIu64, header.base_fee_per_gas.value(), expected_base_fee);
        return false;
    }
    return true;
}
