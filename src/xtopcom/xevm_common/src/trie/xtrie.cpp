...
xtcash_trie::~xtcash_trie() = default;

xtcash_trie::xtcash_trie(observer_ptr<xtrie_db_t> const db, xh256_t const & root_hash) : trie_db_{db}, original_root_hash_{root_hash} {
}

observer_ptr<xtrie_db_t> xtcash_trie::trie_db() const noexcept {
    return trie_db_;
}

std::unique_ptr<xtcash_trie> xtcash_trie::build_from(xh256_t const & hash, observer_ptr<xtrie_db_t> const db, std::error_code & ec) {
    assert(!ec);
    ...
    auto trie = std::unique_ptr<xtcash_trie>{new xtcash_trie{db, hash}};
...
xbytes_t xtcash_trie::get(xbytes_t const & key) const {
    std::error_code ec;
...
xbytes_t xtcash_trie::try_get(xbytes_t const & key, std::error_code & ec) const {
    xassert(!ec);
    xbytes_t value;
...
//std::pair<xbytes_t, std::size_t> xtcash_trie::try_get_node(xbytes_t const & path, std::error_code & ec) {
//    xassert(!ec);
//    xbytes_t item;
//    xtrie_node_face_ptr_t newroot;
//    std::size_t resolved;
...
// stored in the trie.
void xtcash_trie::update(xbytes_t const & key, xbytes_t const & value) {
    // printf("update: %s %s\n", tcash::to_hex(key).c_str(), tcash::to_hex(value).c_str());
    std::error_code ec;
...
//
// If a node was not found in the database, a MissingNodeError(trie_db_missing_node_error) is returned.
void xtcash_trie::try_update(xbytes_t const & key, xbytes_t const & value, std::error_code & ec) {
    xassert(!ec);
    unhashed_++;
...
void xtcash_trie::Delete(xbytes_t const & key) {
    std::error_code ec;
    try_delete(key, ec);
    if (ec) {
        xerror("trie error: %s %s", ec.category().name(), ec.message().c_str());
...
// If a node was not found in the database, a MissingNodeError(trie_db_missing_node_error) is returned.
void xtcash_trie::try_delete(xbytes_t const & key, std::error_code & ec) {
    xassert(!ec);
    unhashed_++;
    auto const k = key_bytes_to_hex(key);
    auto result = erase(trie_root_, {}, k, ec);
...
std::pair<xh256_t, int32_t> xtcash_trie::commit(std::error_code & ec) {
    assert(!ec);
    // todo leaf_callback
    if (trie_db_ == nullptr) {
...
bool xtcash_trie::prove(xbytes_t const & key, uint32_t from_level, xkv_db_face_ptr_t const & proof_db, std::error_code & ec) const {
    xassert(!ec);
    ...
    for (auto tn = trie_root_; !key_path.empty() && tn != nullptr;) {
        xdbg("key: %s", tcash::to_hex(key_path).c_str());
        ...
        case xtrie_node_type_t::shortnode: {
            ...
            nodes.push_back(n);
            xdbg("append shortnode %s", tcash::to_hex(key_path).c_str());
            break;
        }
        case xtrie_node_type_t::fullnode: {
            ...
            nodes.push_back(n);
            xdbg("append fullnode %s", tcash::to_hex(key_path).c_str());
            break;
        }
    ...
    for (std::size_t index = 0; index < nodes.size(); ++index) {
        ...
        if (hn->type() == xtrie_node_type_t::hashnode || index == 0) {
            ...
            xdbg("[Prove]: put into db <%s> %s", tcash::to_hex(hash->data()).c_str(), tcash::to_hex(enc).c_str());
...
std::tuple<xbytes_t, xtrie_node_face_ptr_t, bool> xtcash_trie::try_get(xtrie_node_face_ptr_t const & node, xbytes_t const & key, std::size_t const pos, std::error_code & ec) const {
    xdbg("tryGet key: %s ,pos: %zu", tcash::to_hex(key).c_str(), pos);
    if (node == nullptr) {
    ...
    case xtrie_node_type_t::hashnode: {
        ...
        if (ec) {
            xwarn("resolve hash error: at key: %s pos:%zu", tcash::to_hex(key).c_str(), pos);
...
std::tuple<xbytes_t, xtrie_node_face_ptr_t, std::size_t> xtcash_trie::try_get_node(xtrie_node_face_ptr_t const & orig_node,
                                                                                 xbytes_t const & path,
                                                                                 std::size_t const pos,
                                                                                 std::error_code & ec) const {
    xdbg("tryGetNode path: %s, pos: %zu", tcash::to_hex(path).c_str(), pos);
...
xtcash_trie::update_result::update_result(bool const updated, std::shared_ptr<xtrie_node_face_t> n) noexcept : node{std::move(n)}, dirty{updated} {
}

xtcash_trie::update_result xtcash_trie::insert(xtrie_node_face_ptr_t const & node, xbytes_t prefix, xbytes_t key, xtrie_node_face_ptr_t const & value, std::error_code & ec) {
    if (key.empty()) {
...
xtcash_trie::update_result xtcash_trie::erase(xtrie_node_face_ptr_t const & node, xbytes_t const & prefix, xbytes_t key, std::error_code & ec) {
    // printf("erase: prefix: %s key: %s\n", tcash::to_hex(prefix).c_str(), tcash::to_hex(key).c_str());
    if (node == nullptr) {
        return {false, nullptr};
    }
    assert(node != nullptr);
...
xtrie_node_face_ptr_t xtcash_trie::resolve(xtrie_node_face_ptr_t const & n, /*xbytes_t prefix,*/ std::error_code & ec) const {
...
xtrie_node_face_ptr_t xtcash_trie::resolve_hash(xtrie_hash_node_ptr_t const & n, /*xbytes_t prefix,*/ std::error_code & ec) const {
    assert(!ec);

    return resolve_hash(n->data(), ec);
}

xtrie_node_face_ptr_t xtcash_trie::resolve_hash(xh256_t const & hash, std::error_code & ec) const {
    assert(!ec);
...
xbytes_t xtcash_trie::resolve_blob(std::shared_ptr<xtrie_hash_node_t> const & n, std::error_code & ec) const {
    auto const & hash = n->data();
    return trie_db_->Node(hash, ec);
}

// hashRoot calculates the root hash of the given trie
std::pair<xtrie_node_face_ptr_t, xtrie_node_face_ptr_t> xtcash_trie::hash_root() {
    if (trie_root_ == nullptr) {
...
xnode_flag_t xtcash_trie::node_dirty() {
    return xnode_flag_t{true};
}

void xtcash_trie::prune(xh256_t const & old_trie_root_hash, std::unordered_set<xh256_t> & pruned_hashes, std::error_code & ec) {
    assert(!ec);

    if (pruner_ == nullptr) {
        pruner_ = tcash::make_unique<xtrie_pruner_t>();
...
void xtcash_trie::commit_pruned(std::unordered_set<xh256_t> const & pruned_hashes, std::error_code & ec) {
    assert(!ec);
    assert(pruner_);
...
void xtcash_trie::prune(std::error_code & ec) {
    assert(!ec);
...
void xtcash_trie::commit_pruned(std::vector<xh256_t> pruned_root_hashes, std::error_code & ec) {
    assert(!ec);
    assert(trie_db_);
...
void xtcash_trie::clear_pruned(xh256_t const & root_hash, std::error_code & ec) {
    assert(!ec);
    assert(trie_db_);
    assert(pending_to_be_pruned_.empty());
...
void xtcash_trie::clear_pruned(std::error_code & ec) {
    assert(!ec);
    assert(trie_db_);
    assert(pending_to_be_pruned_.empty());
...
std::string xtcash_trie::to_string() const {
    if (trie_root_ != nullptr) {
...
std::shared_ptr<xtrie_node_face_t> xtcash_trie::root() const noexcept {
    return trie_root_;
}

std::size_t xtcash_trie::pending_pruned_size() const noexcept {
    return pending_to_be_pruned_.size();
}

xh256_t const & xtcash_trie::original_root_hash() const noexcept {
