...
void xtcash_trie_pruner::init(std::shared_ptr<xtrie_node_face_t> const & trie_root, observer_ptr<xtrie_db_t> trie_db, std::error_code & ec) {
    assert(!ec);
    load_trie_node(trie_root, trie_db, ec);
}

std::shared_ptr<xtrie_node_face_t> xtcash_trie_pruner::load_trie_node(std::shared_ptr<xtrie_node_face_t> const & trie_node, observer_ptr<xtrie_db_t> const trie_db,
                                                                    std::error_code & ec) {
    assert(!ec);
...
std::shared_ptr<xtrie_short_node_t> xtcash_trie_pruner::load_short_node(std::shared_ptr<xtrie_short_node_t> const & short_node,
...
std::shared_ptr<xtrie_full_node_t> xtcash_trie_pruner::load_full_node(std::shared_ptr<xtrie_full_node_t> const & full_node,
                                                                    observer_ptr<xtrie_db_t> const trie_db,
                                                                    std::error_code & ec) {
    assert(!ec);
...
std::shared_ptr<xtrie_node_face_t> xtcash_trie_pruner::load_hash_node(std::shared_ptr<xtrie_hash_node_t> const & hash_node,
                                                                    observer_ptr<xtrie_db_t> const trie_db,
                                                                    std::error_code & ec) {
    assert(!ec);
...
void xtcash_trie_pruner::load_full_node_children(std::shared_ptr<xtrie_full_node_t> const & full_node, observer_ptr<xtrie_db_t> const trie_db, std::error_code & ec) {
    assert(!ec);
...
void xtcash_trie_pruner::try_prune_trie_node(std::shared_ptr<xtrie_node_face_t> const & trie_node, observer_ptr<xtrie_db_t> const trie_db, std::error_code & ec) {
    assert(!ec);
...
void xtcash_trie_pruner::try_prune_hash_node(std::shared_ptr<xtrie_hash_node_t> const & hash_node, observer_ptr<xtrie_db_t> const trie_db, std::error_code & ec) {
    assert(!ec);
...
void xtcash_trie_pruner::try_prune_short_node(std::shared_ptr<xtrie_short_node_t> const & short_node, observer_ptr<xtrie_db_t> const trie_db, std::error_code & ec) {
    assert(!ec);
...
void xtcash_trie_pruner::try_prune_full_node(std::shared_ptr<xtrie_full_node_t> const & full_node, observer_ptr<xtrie_db_t> const trie_db, std::error_code & ec) {
    assert(!ec);
    auto new_full_node = full_node;
...
void xtcash_trie_pruner::prune(xh256_t const & old_trie_root_hash, observer_ptr<xtrie_db_t> const trie_db, std::unordered_set<xh256_t> & pruned_hashes, std::error_code & ec) {
    assert(!ec);
...
void xtcash_trie_pruner::try_prune_trie_node(std::shared_ptr<xtrie_node_face_t> const & trie_node, observer_ptr<xtrie_db_t> const trie_db, std::unordered_set<xh256_t> & pruned_hashes, std::error_code & ec) {
    assert(!ec);
...
void xtcash_trie_pruner::try_prune_hash_node(std::shared_ptr<xtrie_hash_node_t> const & hash_node,
                                           observer_ptr<xtrie_db_t> const trie_db,
...
void xtcash_trie_pruner::try_prune_short_node(std::shared_ptr<xtrie_short_node_t> const & short_node,
                                            observer_ptr<xtrie_db_t> const trie_db,
                                            std::unordered_set<xh256_t> & pruned_hashes,
                                            std::error_code & ec) {
    assert(!ec);
...
void xtcash_trie_pruner::try_prune_full_node(std::shared_ptr<xtrie_full_node_t> const & full_node,
                                           observer_ptr<xtrie_db_t> const trie_db,
                                           std::unordered_set<xh256_t> & pruned_hashes,
                                           std::error_code & ec) {
    assert(!ec);
