...
std::shared_ptr<xtcash_stack_trie> xtcash_stack_trie::NewStackTrie(xkv_db_face_ptr_t db) {
    if (db == nullptr) {
        xerror("build stack trie from null db");
    }

    auto st_trie = xtcash_stack_trie{db};

    return std::make_shared<xtcash_stack_trie>(st_trie);
}

void xtcash_stack_trie::Update(xbytes_t const & key, xbytes_t const & value) {
    std::error_code ec;
...
void xtcash_stack_trie::TryUpdate(xbytes_t const & key, xbytes_t const & value, std::error_code & ec) {
...
void xtcash_stack_trie::insert(xbytes_t const & key, xbytes_t const & value) {
    switch (type()) {
    ...
    case xstack_trie_node_type_t::extNode: {
        ...
        // node directly.
        std::shared_ptr<xtcash_stack_trie> n;
        ...
        // Convert to hash
        n->hash();
        std::shared_ptr<xtcash_stack_trie> p;
        if (diffidx == 0) {
            // the break is on the first byte, so
            // the current node is converted into
            // a branch node.
            m_children[0] = nullptr;
            p = std::make_shared<xtcash_stack_trie>(*this);
            m_type = xstack_trie_node_type_t::branchNode;
        } else {
            // the common prefix is at least one byte
            // long, insert a new intermediate branch
            // node.
            m_children[0] = std::make_shared<xtcash_stack_trie>(m_db);
            p = m_children[0];
...
void xtcash_stack_trie::hash() {
    ...
    case xstack_trie_node_type_t::branchNode: {
        ...
        xdbg("xtcash_stack_trie hash branchNode buffer: %s", tcash::to_hex(hash_buffer).c_str());
    }
    case xstack_trie_node_type_t::extNode: {
        m_children[0]->hash();
        ...
        m_children[0] = nullptr;
        xdbg("xtcash_stack_trie hash extNode buffer: %s", tcash::to_hex(hash_buffer).c_str());
    }
    case xstack_trie_node_type_t::leafNode: {
        ...
        hash_buffer = RLP::encodeList(encoded);
        xdbg("xtcash_stack_trie hash leafNode buffer: %s", tcash::to_hex(hash_buffer).c_str());
    }
    case xstack_trie_node_type_t::emptyNode: {
...
xh256_t xtcash_stack_trie::Hash() {
    hash();
    if (m_val.size() != 32) {
        // If the node's RLP isn't 32 bytes long, the node will not
        // be hashed, and instead contain the  rlp-encoding of the
        // node. For the tcash level node, we need to force the hashing.
        xbytes_t res;
        utl::xkeccak256_t hasher;
...
xh256_t xtcash_stack_trie::Commit(std::error_code & ec) {
    if (m_db = nullptr) {
...
std::shared_ptr<xtcash_stack_trie> xtcash_stack_trie::newLeaf(xbytes_t const & key, xbytes_t const & val) {
    std::shared_ptr<xtcash_stack_trie> newnode = std::make_shared<xtcash_stack_trie>(m_db);
...
std::shared_ptr<xtcash_stack_trie> xtcash_stack_trie::newExt(xbytes_t const & key, std::shared_ptr<xtcash_stack_trie> child) {
    std::shared_ptr<xtcash_stack_trie> newnode = std::make_shared<xtcash_stack_trie>(m_db);
