...
std::pair<xtrie_node_face_ptr_t, xtrie_node_face_ptr_t> xtcash_trie_hasher::hash(xtrie_node_face_ptr_t const & node, bool const force) {
    {
...
xtrie_hash_node_ptr_t xtcash_trie_hasher::hashData(xbytes_t const & input) const {
    xdbg("hashData:(%zu) %s ", input.size(), tcash::to_hex(input).c_str());
    xbytes_t hashbuf;
    ...
    xdbg(" -> hashed data:(%zu) %s", hashbuf.size(), tcash::to_hex(hashbuf).c_str());
    return std::make_shared<xtrie_hash_node_t>(hashbuf);
}

std::pair<xtrie_node_face_ptr_t, xtrie_node_face_ptr_t> xtcash_trie_hasher::proofHash(xtrie_node_face_ptr_t node) {
    switch (node->type()) {
    case xtrie_node_type_t::shortnode: {
        auto const n = std::dynamic_pointer_cast<xtrie_short_node_t>(node);
        assert(n != nullptr);
...
std::pair<xtrie_short_node_ptr_t, xtrie_short_node_ptr_t> xtcash_trie_hasher::hashShortNodeChildren(xtrie_short_node_ptr_t const & node) {
...
std::pair<xtrie_full_node_ptr_t, xtrie_full_node_ptr_t> xtcash_trie_hasher::hashFullNodeChildren(xtrie_full_node_ptr_t const & node) {
    auto collapsed = node->clone();
    auto cached = node->clone();    // must be cloned?
...
xtrie_node_face_ptr_t xtcash_trie_hasher::shortnodeToHash(xtrie_short_node_ptr_t node, bool const force) {
    tmp.Reset();
    tmp.data().reserve(1024);

    std::error_code ec;
    node->EncodeRLP(tmp.data(), ec);
    xdbg("[shortnodeToHash] %s  -> %s ", tcash::to_hex(node->key).c_str(), tcash::to_hex(tmp.data()).c_str());
    xassert(!ec);
...
xtrie_node_face_ptr_t xtcash_trie_hasher::fullnodeToHash(xtrie_full_node_ptr_t node, bool const force) {
    tmp.Reset();

    std::error_code ec;
    node->EncodeRLP(tmp.data(), ec);
    xdbg("[fullnodeToHash] -> %s ", tcash::to_hex(tmp.data()).c_str());
    xassert(!ec);
