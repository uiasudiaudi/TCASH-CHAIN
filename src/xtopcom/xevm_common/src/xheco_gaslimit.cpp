...
bool verify_gaslimit(const u256 parent_gas_limit, const u256 header_gas_limit) {
    ...
    if (uint64_t(diff) >= limit) {
        xwarn("[xtcash_evm_eth_bridge_contract::verifyGaslimit] diff: %lu >= limit: %s", uint64_t(diff), limit.str().c_str());
        return false;
    }

    if (header_gas_limit < MinGasLimit) {
        xwarn("[xtcash_evm_eth_bridge_contract::verifyGaslimit] headerGasLimit: %s too small < %lu", header_gas_limit.str().c_str(), MinGasLimit);
        return false;
    }
    return true;
}
