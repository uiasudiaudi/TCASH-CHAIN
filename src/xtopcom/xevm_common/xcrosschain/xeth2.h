...
enum class xtcash_network_id : uint8_t { mainnet, kiln, goerli, sepolia };
using xnetwork_id_t = xtcash_network_id;

struct xtcash_proof_size {
    ...
    xtcash_proof_size() = default;
    xtcash_proof_size(size_t const beacon_block_body_tree_depth,
...
using xproof_size_t = xtcash_proof_size;

struct xtcash_fork_version {
    std::array<uint8_t, 4> fork_version{{0, 0, 0, 0}};

    xtcash_fork_version() = default;
    xtcash_fork_version(std::initializer_list<uint8_t> il) {
        assert(il.size() == 4);
        std::copy(il.begin(), il.end(), fork_version.begin());
    }
};
using xfork_version_t  = xtcash_fork_version;
...
struct xtcash_network_config {
    ...
    explicit xtcash_network_config(xnetwork_id_t const network_id) {
        switch (network_id) {
...
using xnetwork_config_t = xtcash_network_config;
