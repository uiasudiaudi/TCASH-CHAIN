...
class xtcash_group_element final : public xbasic_element_t
                               , public common::xbasic_rotation_aware_t<xtcash_group_element>
                               , public std::enable_shared_from_this<xtcash_group_element> {
    ...
    std::weak_ptr<xtcash_group_element> m_associated_parent_group{};
...
public:
    xtcash_group_element(xtcash_group_element const &)             = delete;
    xtcash_group_element & operator=(xtcash_group_element const &) = delete;
    xtcash_group_element(xtcash_group_element &&)                  = default;
    xtcash_group_element & operator=(xtcash_group_element &&)      = default;
    ~xtcash_group_element() override                             = default;

    xtcash_group_element(common::xelection_round_t const & election_round,
    ...
    void
    associate_parent_group(std::shared_ptr<xtcash_group_element> const & parent_group,
                           std::error_code & ec);

    void
    associate_parent_group(std::shared_ptr<xtcash_group_element> const & parent_group);

    std::shared_ptr<xtcash_group_element>
    associated_parent_group() const;

    std::shared_ptr<xtcash_group_element>
    associated_parent_group(std::error_code & ec) const;

    std::vector<std::shared_ptr<xtcash_group_element>>
    associated_child_groups(common::xlogic_time_t const logic_time, std::error_code & ec) const;

    std::vector<std::shared_ptr<xtcash_group_element>> associated_child_groups(std::error_code & ec) const;

private:
    void
    associate_child_group(std::shared_ptr<xtcash_group_element> const & child_group,
                          std::error_code & ec);

    bool
    exist_with_lock_hold_outside(common::xslot_id_t const & slot_id) const noexcept;
};
using xgroup_element_t = xtcash_group_element;
