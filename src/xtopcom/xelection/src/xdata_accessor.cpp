...
xtcash_data_accessor::xtcash_data_accessor(common::xnetwork_id_t const & network_id, observer_ptr<time::xchain_time_face_t> const & logic_timer)
  : m_network_element{std::make_shared<xnetwork_element_t>(network_id)}, m_logic_timer{logic_timer} {
    assert(m_logic_timer != nullptr);
}

common::xnetwork_id_t xtcash_data_accessor::network_id() const noexcept {
    assert(m_network_element);
    return m_network_element->network_id();
}

std::unordered_map<common::xgroup_address_t, xgroup_update_result_t> xtcash_data_accessor::update_zone(common::xzone_id_t const & zone_id,
...
std::map<common::xslot_id_t, data::xnode_info_t> xtcash_data_accessor::sharding_nodes(common::xgroup_address_t const & address,
    ...
    for (auto const & node_element_info : node_elements_info) {
        auto const & slot_id = tcash::get<common::xslot_id_t const>(node_element_info);
        auto const & node_element = tcash::get<std::shared_ptr<xnode_element_t>>(node_element_info);
...
std::map<common::xslot_id_t, data::xnode_info_t> xtcash_data_accessor::group_nodes(common::xgroup_address_t const & group_address,
...
common::xnode_address_t xtcash_data_accessor::parent_address(common::xgroup_address_t const & child_address, common::xelection_round_t const & child_version, std::error_code & ec) const
    noexcept {
    assert(!ec);
...
common::xnode_address_t xtcash_data_accessor::parent_address(common::xgroup_address_t const & child_address,
...
std::vector<common::xnode_address_t> xtcash_data_accessor::child_addresses(common::xgroup_address_t const & parent_group_address,
...
std::shared_ptr<xnode_element_t> xtcash_data_accessor::node_element(common::xgroup_address_t const & address,
...
common::xaccount_address_t xtcash_data_accessor::account_address_from(common::xip2_t const & xip2, std::error_code & ec) const {
    assert(!ec);
...
std::shared_ptr<xgroup_element_t> xtcash_data_accessor::group_element(common::xgroup_address_t const & group_address,
...
std::shared_ptr<xgroup_element_t> xtcash_data_accessor::group_element_by_height(common::xgroup_address_t const & group_address,
...
std::shared_ptr<xgroup_element_t> xtcash_data_accessor::group_element_by_logic_time(common::xgroup_address_t const & group_address,
...
std::shared_ptr<xgroup_element_t> xtcash_data_accessor::parent_group_element(common::xgroup_address_t const & child_sharding_address,
...
std::shared_ptr<xgroup_element_t> xtcash_data_accessor::parent_group_element(common::xgroup_address_t const & child_gropu_address,
                                                                           common::xlogic_epoch_t const & child_logic_epoch,
                                                                           std::error_code & ec) const {
    assert(!ec);
...
common::xelection_round_t xtcash_data_accessor::election_epoch_from(common::xip2_t const & xip2, std::error_code & ec) const {
    assert(!ec);
    auto group_element = this->group_element_by_height(xip2.network_id(), xip2.zone_id(), xip2.cluster_id(), xip2.group_id(), xip2.height(), ec);
    if (ec) {
        xwarn("%s %s", ec.category().name(), ec.message().c_str());
...
std::unordered_map<common::xgroup_address_t, xgroup_update_result_t> xtcash_data_accessor::update_zone(std::shared_ptr<xzone_element_t> const & zone_element,
                                                                                                     data::election::xelection_result_store_t const & election_result_store,
                                                                                                     std::uint64_t const associated_blk_height,
                                                                                                     std::error_code & ec) {
    assert(!ec);
    ...
    try {
        ...
        for (auto const & zone_data : zone_result.results()) {
            ...
            auto const & cluster_id = tcash::get<common::xcluster_id_t const>(zone_data);
            auto const & cluster_result = tcash::get<data::election::xelection_cluster_result_t>(zone_data);
...
std::unordered_map<common::xgroup_address_t, xgroup_update_result_t> xtcash_data_accessor::update_committee_zone(std::shared_ptr<xzone_element_t> const & zone_element,
                                                                                                                  data::election::xelection_result_store_t const & election_result_store,
                                                                                                                  std::uint64_t const associated_blk_height,
                                                                                                                  std::error_code & ec) {
    assert(!ec);
...
std::unordered_map<common::xgroup_address_t, xgroup_update_result_t> xtcash_data_accessor::update_consensus_zone(std::shared_ptr<xzone_element_t> const & zone_element,
                                                                                                               data::election::xelection_result_store_t const & election_result_store,
                                                                                                               std::uint64_t const associated_blk_height,
                                                                                                               std::error_code & ec) {
    assert(!ec);
...
std::unordered_map<common::xgroup_address_t, xgroup_update_result_t> xtcash_data_accessor::update_edge_zone(std::shared_ptr<xzone_element_t> const & zone_element,
                                                                                                          data::election::xelection_result_store_t const & election_result_store,
                                                                                                          std::uint64_t const associated_blk_height,
                                                                                                          std::error_code & ec) {
    assert(!ec);
...
std::unordered_map<common::xgroup_address_t, xgroup_update_result_t> xtcash_data_accessor::update_storage_zone(std::shared_ptr<xzone_element_t> const & zone_element,
                                                                                                             data::election::xelection_result_store_t const & election_result_store,
                                                                                                             std::uint64_t const associated_blk_height,
                                                                                                             std::error_code & ec) {
    assert(!ec);
...
std::unordered_map<common::xgroup_address_t, xgroup_update_result_t> xtcash_data_accessor::update_fullnode_zone(
...
std::unordered_map<common::xgroup_address_t, xgroup_update_result_t> xtcash_data_accessor::update_zec_zone(std::shared_ptr<xzone_element_t> const & zone_element,
                                                                                                            data::election::xelection_result_store_t const & election_result_store,
                                                                                                            std::uint64_t const associated_blk_height,
                                                                                                            std::error_code & ec) {
    assert(!ec);
...
std::unordered_map<common::xgroup_address_t, xgroup_update_result_t> xtcash_data_accessor::update_frozen_zone(std::shared_ptr<xzone_element_t> const & zone_element,
                                                                                                            data::election::xelection_result_store_t const & election_result_store,
                                                                                                            std::uint64_t const associated_blk_height,
                                                                                                            std::error_code & ec) {
    assert(!ec);
...
std::unordered_map<common::xgroup_address_t, xgroup_update_result_t> xtcash_data_accessor::update_eth_zone(std::shared_ptr<xzone_element_t> const & zone_element,
                                                                                                         data::election::xelection_result_store_t const & election_result_store,
                                                                                                         std::uint64_t const associated_blk_height,
                                                                                                         std::error_code & ec) {
    assert(!ec);
...
std::unordered_map<common::xgroup_address_t, xgroup_update_result_t> xtcash_data_accessor::update_relay_zone(std::shared_ptr<xzone_element_t> const & zone_element,
                                                                                                           data::election::xelection_result_store_t const & election_result_store,
                                                                                                           std::uint64_t const associated_blk_height,
                                                                                                           std::error_code & ec) {
    assert(!ec);
...
std::unordered_map<common::xgroup_address_t, xgroup_update_result_t> xtcash_data_accessor::update_cluster(std::shared_ptr<xzone_element_t> const & zone_element,
    ...
    for (auto const & group_data : cluster_result.results()) {
        ...
        auto const & group_id = tcash::get<common::xgroup_id_t const>(group_data);
        auto const & group_result = tcash::get<data::election::xelection_group_result_t>(group_data);
...
void xtcash_data_accessor::update_group(std::shared_ptr<xzone_element_t> const & /*zone_element*/,
                                      std::shared_ptr<xcluster_element_t> const & /*cluster_element*/,
