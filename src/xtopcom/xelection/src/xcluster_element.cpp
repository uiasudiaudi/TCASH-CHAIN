...
xtcash_cluster_element::xtcash_cluster_element(common::xcluster_id_t const & cluster_id, std::shared_ptr<xzone_element_t> const & zone_element)
  : xbase_t{zone_element->network_id(), zone_element->zone_id(), cluster_id}, m_zone_element{zone_element} {}

std::shared_ptr<xzone_element_t> xtcash_cluster_element::zone_element() const noexcept {
    return m_zone_element.lock();
}

bool xtcash_cluster_element::exist(common::xgroup_id_t const & group_id, common::xlogic_time_t const logic_time) const {
    XLOCK(m_group_elements_mutex);
    return exist_with_lock_hold_outside(group_id, logic_time);
}

xgroup_update_result_t xtcash_cluster_element::add_group_element(common::xgroup_id_t const & group_id,
    ...
    if (it != std::end(group_info_store)) {
        auto group_element = tcash::get<std::shared_ptr<xgroup_element_t>>(*it);
...
std::shared_ptr<xgroup_element_t> xtcash_cluster_element::group_element_by_height(common::xgroup_id_t const & group_id,
    ...
    auto const & group_info_store = tcash::get<xgroup_info_container_t>(*it);
    for (auto const & group_info : group_info_store) {
        assert(tcash::get<std::shared_ptr<xgroup_element_t>>(group_info));
        auto const & group_element = tcash::get<std::shared_ptr<xgroup_element_t>>(group_info);
...
std::shared_ptr<xgroup_element_t> xtcash_cluster_element::group_element_by_height(common::xgroup_id_t const & group_id, uint64_t const election_blk_height) const {
    std::error_code ec;
...
std::shared_ptr<xgroup_element_t> xtcash_cluster_element::group_element(common::xgroup_id_t const & group_id, common::xelection_round_t const & election_round, std::error_code & ec) const {
    assert(!ec);
    if (common::broadcast(group_id)) {
        ec = xdata_accessor_errc_t::group_id_empty;
...
std::shared_ptr<xgroup_element_t> xtcash_cluster_element::group_element(common::xgroup_id_t const & group_id,
...
std::shared_ptr<xgroup_element_t> xtcash_cluster_element::group_element(common::xgroup_id_t const & group_id, common::xelection_round_t const & election_round) const {
    std::error_code ec;
...
std::shared_ptr<xgroup_element_t> xtcash_cluster_element::group_element(common::xgroup_id_t const & group_id, common::xlogic_epoch_t const & logic_epoch) const {
    std::error_code ec;
...
std::shared_ptr<xgroup_element_t> xtcash_cluster_element::group_element_by_logic_time(common::xgroup_id_t const & group_id, common::xlogic_time_t const logic_time, std::error_code & ec) const {
    assert(!ec);
    if (common::broadcast(group_id)) {
        ec = xdata_accessor_errc_t::group_id_empty;
    ...
    auto const & group_info_store = tcash::get<xgroup_info_container_t>(*it);
    for (auto const & group_info : group_info_store) {
        if (tcash::get<common::xlogic_time_t const>(group_info) <= logic_time) {
            assert(tcash::get<std::shared_ptr<xgroup_element_t>>(group_info));
            return tcash::get<std::shared_ptr<xgroup_element_t>>(group_info);
        }
    }
...
std::shared_ptr<xgroup_element_t> xtcash_cluster_element::group_element_by_logic_time(common::xgroup_id_t const & group_id, common::xlogic_time_t const logic_time) const {
    std::error_code ec;
...
common::xelection_round_t xtcash_cluster_element::group_version(common::xgroup_id_t const & group_id, common::xlogic_time_t const logic_time, std::error_code & ec) const {
    assert(!ec);
...
common::xelection_round_t xtcash_cluster_element::group_version(common::xgroup_id_t const & group_id, common::xlogic_time_t const logic_time) const {
    std::error_code ec;
...
std::vector<std::shared_ptr<xgroup_element_t>> xtcash_cluster_element::children(common::xnode_type_t const  child_type,
    ...
    for (auto const & group_element_store : m_group_elements) {
        auto const & group_info_store = tcash::get<xgroup_info_container_t>(group_element_store);
        for (auto const & group_info : group_info_store) {
            if (tcash::get<common::xlogic_time_t const>(group_info) > logic_time) {
                continue;
            }
...
std::vector<std::shared_ptr<xgroup_element_t>> xtcash_cluster_element::children(common::xnode_type_t const child_type, common::xlogic_time_t const logic_time) const {
    std::error_code ec;
...
bool xtcash_cluster_element::exist_with_lock_hold_outside(common::xgroup_id_t const & group_id) const {
    return find_with_lock_hold_outside(group_id) != std::end(m_group_elements);
}

bool xtcash_cluster_element::exist_with_lock_hold_outside(common::xgroup_id_t const & group_id, common::xlogic_time_t const logic_time) const {
    auto const iterator = find_with_lock_hold_outside(group_id);
    if (iterator == std::end(m_group_elements)) {
        return false;
    }

    auto const & group_info_container = tcash::get<xgroup_info_container_t>(*iterator);
    return group_info_container.find(logic_time) != std::end(group_info_container);
}

xtcash_cluster_element::xgroup_info_container_t::const_iterator xtcash_cluster_element::find_with_lock_hold_outside(common::xgroup_id_t const & group_id,
                                                                                                                common::xlogic_time_t const logic_time) const {
    auto const iterator = find_with_lock_hold_outside(group_id);
    if (iterator == std::end(m_group_elements)) {
        return xtcash_cluster_element::xgroup_info_container_t::const_iterator{};
    }

    return tcash::get<xgroup_info_container_t>(*iterator).find(logic_time);
}

xtcash_cluster_element::xgroup_elements_container_t::const_iterator xtcash_cluster_element::find_with_lock_hold_outside(common::xgroup_id_t const & group_id) const {
    return m_group_elements.find(group_id);
}

xgroup_update_result_t xtcash_cluster_element::add_group_element_with_lock_hold_outside(common::xgroup_id_t const & group_id,
                                                                                      common::xelection_round_t const & election_round,
                                                                                      common::xlogic_time_t const timestamp,
                                                                                      common::xlogic_time_t const start_time,
                                                                                      std::uint16_t const sharding_size,
    ...
    result.added = tcash::get<std::shared_ptr<xgroup_element_t>>(*group_info_container.begin());
    auto const added_group_start_time = tcash::get<common::xlogic_time_t const>(*group_info_container.begin());
    ...
    if (group_info_container.size() > keepalive_strategy.faded_threshold_count) {
        ...
        while (faded_it != std::end(group_info_container)) {
            auto & faded = tcash::get<std::shared_ptr<xgroup_element_t>>(*faded_it);
    ...
    if (group_info_container.size() > keepalive_strategy.outdated_threshold_count) {
        ...
        while (outdated_it != std::end(group_info_container)) {
            auto outdated = tcash::get<std::shared_ptr<xgroup_element_t>>(*outdated_it);
