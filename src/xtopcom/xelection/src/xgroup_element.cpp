...
xtcash_group_element::xtcash_group_element(common::xelection_round_t const & election_round,
...
std::shared_ptr<xcluster_element_t> xtcash_group_element::cluster_element() const noexcept {
    return m_cluster_element.lock();
}

bool xtcash_group_element::exist(common::xslot_id_t const & slot_id) const {
    XLOCK(m_node_elements_mutex);
    return exist_with_lock_hold_outside(slot_id);
}

std::shared_ptr<xnode_element_t> xtcash_group_element::node_element(common::xslot_id_t const & slot_id, std::error_code & ec) const {
    assert(!ec);
    ...
    return tcash::get<std::shared_ptr<xnode_element_t>>(*it);
}

std::shared_ptr<xnode_element_t> xtcash_group_element::node_element(common::xslot_id_t const & slot_id) const {
    std::error_code ec;
...
std::shared_ptr<xnode_element_t> xtcash_group_element::node_element(common::xnode_id_t const & node_id, std::error_code & ec) const {
    assert(!ec);
    if (node_id.empty()) {
    ...
    XLOCK_GUARD(m_node_elements_mutex) {
        for (auto const & node_info : m_node_elements) {
            auto const & node_element = tcash::get<std::shared_ptr<xnode_element_t>>(node_info);
...
std::shared_ptr<xnode_element_t> xtcash_group_element::node_element(common::xnode_id_t const & node_id) const {
    std::error_code ec;
...
bool xtcash_group_element::contains(common::xnode_id_t const & node_id) const noexcept {
    try {
        std::error_code ec;
        return node_element(node_id, ec) != nullptr;
    } catch (error::xtcash_error_t const & eh) {
        xwarn("xtcash_group_element::contains %s", eh.what());
    } catch (std::exception const & eh) {
        xwarn("xtcash_group_element::contains %s", eh.what());
    } catch (...) {
        xerror("xtcash_group_element::contains unknown");
    }

    return false;
}

void xtcash_group_element::set_node_elements(std::map<common::xslot_id_t, data::election::xelection_info_bundle_t> const & election_data) {
    XLOCK(m_node_elements_mutex);
    ...
    for (auto const & node_data : election_data) {
        auto const & slot_id = tcash::get<common::xslot_id_t const>(node_data);
        auto const & election_info_bundle = tcash::get<data::election::xelection_info_bundle_t>(node_data);
...
void xtcash_group_element::associate_parent_group(std::shared_ptr<xtcash_group_element> const & parent_group, std::error_code & ec) {
    assert(!ec);
...
void xtcash_group_element::associate_parent_group(std::shared_ptr<xtcash_group_element> const & parent_group) {
    std::error_code ec;
    associate_parent_group(parent_group, ec);
    tcash::error::throw_error(ec);
}

std::shared_ptr<xtcash_group_element> xtcash_group_element::associated_parent_group() const {
    XLOCK(m_associated_parent_group_mutex);
    return m_associated_parent_group.lock();
}

std::shared_ptr<xtcash_group_element> xtcash_group_element::associated_parent_group(std::error_code & ec) const {
    assert(!ec);
    std::shared_ptr<xtcash_group_element> group_element;
...
void xtcash_group_element::associate_child_group(std::shared_ptr<xtcash_group_element> const & child_group, std::error_code & ec) {
    assert(!ec);
    ...
    for (auto & associated_child_store : m_associated_child_groups) {
        auto & associated_child_group_container = tcash::get<xassociated_group_container_t>(associated_child_store);
        for (auto it = std::begin(associated_child_group_container); it != std::end(associated_child_group_container);) {
            auto const & associated_child_group = tcash::get<std::weak_ptr<xgroup_element_t>>(*it).lock();
...
std::vector<std::shared_ptr<xtcash_group_element>> xtcash_group_element::associated_child_groups(common::xlogic_time_t const logic_time, std::error_code & ec) const {
    assert(!ec);
    std::vector<std::shared_ptr<xtcash_group_element>> ret;
    XLOCK_GUARD(m_associated_child_groups_mutex) {
        ...
        for (auto const & associated_child_group : m_associated_child_groups) {
            auto const & associated_children = tcash::get<xassociated_group_container_t>(associated_child_group);
            for (auto const & associated_child_info : associated_children) {
                auto candidate_child = tcash::get<std::weak_ptr<xtcash_group_element>>(associated_child_info).lock();
...
std::vector<std::shared_ptr<xtcash_group_element>> xtcash_group_element::associated_child_groups(std::error_code & ec) const {
    return associated_child_groups(common::xjudgement_day, ec);
}

bool xtcash_group_element::enabled(common::xlogic_time_t const logic_time) const noexcept {
    return start_time() <= logic_time;
}

std::map<common::xslot_id_t, std::shared_ptr<xnode_element_t>> xtcash_group_element::children() const {
...
std::map<common::xslot_id_t, std::shared_ptr<xnode_element_t>> xtcash_group_element::children(std::error_code & ec) const {
    assert(!ec);
...
bool xtcash_group_element::exist_with_lock_hold_outside(common::xslot_id_t const & slot_id) const noexcept {
