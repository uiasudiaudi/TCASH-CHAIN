...
xtcash_network_element::xtcash_network_element(common::xnetwork_id_t const & network_id)
    : xbase_t{ network_id } {
}

bool
xtcash_network_element::exist(common::xzone_id_t const & zone_id) const {
    XLOCK(m_zone_elements_mutex);
    return exist_with_lock_hold_outside(zone_id);
}

std::shared_ptr<xzone_element_t>
xtcash_network_element::zone_element(common::xzone_id_t const & zone_id,
    ...
    if (iterator == std::end(m_zone_elements)) {
        ...
        return tcash::get<std::shared_ptr<xzone_element_t>>(*iterator);
    }
}

std::shared_ptr<xzone_element_t>
xtcash_network_element::zone_element(common::xzone_id_t const & zone_id) const {
    std::error_code ec;
...
xtcash_network_element::children() const {
...
std::map<common::xzone_id_t, std::shared_ptr<xzone_element_t>> xtcash_network_element::children(std::error_code & ec) const {
    assert(!ec);
...
std::shared_ptr<xzone_element_t>
xtcash_network_element::add_zone_element(common::xzone_id_t const & zone_id,
                                       std::error_code & ec) {
    assert(!ec);
    XLOCK(m_zone_elements_mutex);
    auto const iterator = find_with_lock_hold_outside(zone_id);
...
bool
xtcash_network_element::exist_with_lock_hold_outside(common::xzone_id_t const & zone_id) const {
    return find_with_lock_hold_outside(zone_id) != std::end(m_zone_elements);
}

xtcash_network_element::xzone_elements_container_t::const_iterator
xtcash_network_element::find_with_lock_hold_outside(common::xzone_id_t const & zone_id) const {
