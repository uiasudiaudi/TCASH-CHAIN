...
xtcash_zone_element::xtcash_zone_element(common::xzone_id_t const & zone_id,
...
xtcash_zone_element::network_element() const noexcept {
    return m_network_element.lock();
}

bool
xtcash_zone_element::exist(common::xcluster_id_t const & cluster_id) const {
    XLOCK(m_cluster_elements_mutex);
    return exist_with_lock_hold_outside(cluster_id);
}

std::shared_ptr<xcluster_element_t>
xtcash_zone_element::cluster_element(common::xcluster_id_t const & cluster_id,
    ...
    return tcash::get<std::shared_ptr<xcluster_element_t>>(*iterator);
}

std::shared_ptr<xcluster_element_t>
xtcash_zone_element::cluster_element(common::xcluster_id_t const & cluster_id) const {
    std::error_code ec;
...
std::shared_ptr<xcluster_element_t>
xtcash_zone_element::add_cluster_element(common::xcluster_id_t const & cluster_id,
                                       std::error_code & ec) {
    assert(!ec);
    XLOCK(m_cluster_elements_mutex);
    auto const iterator = find_with_lock_hold_outside(cluster_id);
...
xtcash_zone_element::children() const {
...
std::map<common::xcluster_id_t, std::shared_ptr<xcluster_element_t>> xtcash_zone_element::children(std::error_code & ec) const {
    assert(!ec);
    XLOCK(m_cluster_elements_mutex);
...
bool
xtcash_zone_element::exist_with_lock_hold_outside(common::xcluster_id_t const & cluster_id) const {
    return find_with_lock_hold_outside(cluster_id) != std::end(m_cluster_elements);
}

xtcash_zone_element::xcluster_elements_container_t::const_iterator
xtcash_zone_element::find_with_lock_hold_outside(common::xcluster_id_t const & cluster_id) const {
