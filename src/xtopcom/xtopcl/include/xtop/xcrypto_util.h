...
public:
    ...
    static std::string digest_sign(const tcash::uint256_t & hash, const std::array<uint8_t, PRI_KEY_LEN> & private_key);
    static bool verify_sign(const tcash::uint256_t & hash, const std::string & signature);
    static bool verify_sign(const tcash::uint256_t & hash, const std::string & signature, const std::string & address);
};
