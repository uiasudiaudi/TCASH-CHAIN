...
#include "xtcashcl/include/task/task_info.h"
...
#endif 
private:
    bool hash_signature(tcash::data::xtransaction_t * trans_action, const std::array<uint8_t, PRI_KEY_LEN> & private_key);
...
public:
    ...
    std::string get_sign(const tcash::uint256_t & hash, std::array<uint8_t, PRI_KEY_LEN> & private_key);

    template <typename... Args>
    std::string stream_params(tcash::base::xstream_t & stream, Args...);

    template <typename T, typename... Args>
    std::string stream_params(tcash::base::xstream_t & stream, T, Args...);
...
template <typename... Args>
inline std::string api_method_imp::stream_params(tcash::base::xstream_t & stream, Args... args) {
    std::string param;
...
template <typename T, typename... Args>
inline std::string api_method_imp::stream_params(tcash::base::xstream_t & stream, T val, Args... args) {
    stream << val;
