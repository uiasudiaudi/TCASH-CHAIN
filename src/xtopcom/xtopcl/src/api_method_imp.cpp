...
#include "xtcashcl/include/task/task_dispatcher.h"
#include "xtcashcl/include/xtcash/tcashchain_type.h"
#include "xtcashcl/include/xtcash/xaction_param.h"
...
//    xcrypto_util::make_private_key(private_key);
//    address = xcrypto_util::make_child_address_by_assigned_key(parent_addr, private_key, tcash::base::enum_vaccount_addr_type_secp256k1_user_sub_account);
//    return !address.empty();
//}
...
bool api_method_imp::validate_key(const std::string & priv, const std::string & pub, const std::string & addr, const std::string & main_addr) {
    ...
    if (main_addr.empty()) {
        regenerate_addr = ecpub.to_address(tcash::base::enum_vaccount_addr_type_secp256k1_user_account,
                                           tcash::base::xvaccount_t::make_ledger_id(tcash::base::enum_main_chain_id, tcash::base::enum_chain_zone_consensus_index));
    }// else {
     //    regenerate_addr = ecpub.to_address(main_addr,
     //                                       tcash::base::enum_vaccount_addr_type_secp256k1_user_sub_account,
     //                                       tcash::base::xvaccount_t::make_ledger_id(tcash::base::enum_main_chain_id, tcash::base::enum_chain_zone_consensus_index));
     // }
...
bool api_method_imp::create_account(const user_info & uinfo, std::function<void(ResultBase *)> func) {
    ...
    tcash::base::xstream_t stream(tcash::base::xcontext_t::instance());
    std::string param = stream_params(stream, uinfo.account);
    auto tx_info = tcash::data::xtx_action_info(uinfo.account, "", "", uinfo.account, "", param);
...
bool api_method_imp::transfer(const user_info & uinfo,
    ...
    if (info->trans_action->get_tx_version() == xtransaction_version_2) {
        ...
        info->trans_action->source_address(tcash::common::xaccount_address_t::build_from(from));
        info->trans_action->target_address(tcash::common::xaccount_address_t::build_from(to));
    } else {
...
bool api_method_imp::stakeGas(const user_info & uinfo,
    ...
    xaction_asset_param asset_param(this, "", amount);
    std::string param = asset_param.create();
    auto tx_info = tcash::data::xtx_action_info(from, "", "", to, "", param);
...
bool api_method_imp::runContract(const user_info & uinfo,
    ...
    auto tx_info = tcash::data::xtx_action_info(uinfo.account, "", asset_param.create(), contract_account, contract_func, code_stream);
...
bool api_method_imp::hash_signature(tcash::data::xtransaction_t * trans_action, const std::array<std::uint8_t, PRI_KEY_LEN> & private_key) {
...
std::string api_method_imp::make_account_address(std::array<uint8_t, PRI_KEY_LEN> & private_key, const uint8_t addr_type, const std::string & parent_addr) {
    ...
    if (addr_type == tcash::base::enum_vaccount_addr_type_secp256k1_user_account) {
        address = pub_key_obj.to_address(addr_type, tcash::base::xvaccount_t::make_ledger_id(tcash::base::enum_main_chain_id, tcash::base::enum_chain_zone_consensus_index));
    }//  else if (addr_type == tcash::base::enum_vaccount_addr_type_secp256k1_user_sub_account || addr_type == tcash::base::enum_vaccount_addr_type_custom_contract) {
     //    if (!parent_addr.empty()) {
     //        address =
     //            pub_key_obj.to_address(parent_addr, addr_type, tcash::base::xvaccount_t::make_ledger_id(tcash::base::enum_main_chain_id, tcash::base::enum_chain_zone_consensus_index));
     //    }
     // }
     else {
        assert(0);
...
std::string api_method_imp::get_sign(const tcash::uint256_t & hash, std::array<uint8_t, PRI_KEY_LEN> & private_key) {
...
bool api_method_imp::getStandbys(const user_info & uinfo, const std::string & account, std::function<void(AccountInfoResult *)> func) {
    ...
    if (reader.parse(rpc_response, jv)) {
        auto data = jv["data"];
        if (data.isNull()) {
            std::cout << "The miner has not joined in node-standby-pool, please start node by command 'tcashio node startnode'." << std::endl;
            return false;
        } else {
...
bool api_method_imp::make_keys_base64(std::string & private_key, std::string & public_key, std::string & address) {
    ...
    address = pub_key_obj.to_address(tcash::base::enum_vaccount_addr_type_secp256k1_user_account,
                                     tcash::base::xvaccount_t::make_ledger_id(tcash::base::enum_main_chain_id, tcash::base::enum_chain_zone_consensus_index));
...
void api_method_imp::make_keys_base64(std::ostream & out) {
    ...
    auto main_address = main_pubkey.to_address(tcash::base::enum_vaccount_addr_type_secp256k1_user_account,
                                               tcash::base::xvaccount_t::make_ledger_id(tcash::base::enum_main_chain_id, tcash::base::enum_chain_zone_consensus_index));
...
bool api_method_imp::registerNode(const user_info & uinfo,
    ...
    tcash::base::xstream_t stream_t(tcash::base::xcontext_t::instance());
    ...
    //auto const & chain_fork_config = tcash::chain_fork::xchain_fork_config_center_t::chain_fork_config();
    //if (tcash::chain_fork::xchain_fork_config_center_t::is_forked(chain_fork_config.new_system_contract_runtime_fork_point,
    //                                                            tcash::config::gmttime_to_logic_time(tcash::base::xtime_utl::gmttime()))) {
    //    source_action_name = "mortgage";
    //}

    xaction_asset_param asset_param(this, "", mortgage);
    std::string param = asset_param.create();
    auto tx_info = tcash::data::xtx_action_info(uinfo.account, source_action_name, param, tcash::sys_contract_rec_registration_addr, target_action_name, param_t);
    info->trans_action->construct_tx(xtransaction_type_run_contract, 100, m_deposit, uinfo.nonce, "", tx_info);
...
bool api_method_imp::updateNodeType(const user_info & uinfo, const std::string & role, std::ostringstream & out_str, std::function<void(NodeRegResult *)> func) {
    ...
    auto tx_info = tcash::data::xtx_action_info(uinfo.account, source_action_name, param_s, tcash::sys_contract_rec_registration_addr, target_action_name, param_t);
    info->trans_action->construct_tx(xtransaction_type_run_contract, 100, m_deposit, uinfo.nonce, "", tx_info);
...
bool api_method_imp::stake_node_deposit(const user_info & uinfo, uint64_t deposit, std::ostringstream & out_str, std::function<void(NodeRegResult *)> func) {
    ...
    auto tx_info = tcash::data::xtx_action_info(uinfo.account, "", param, tcash::sys_contract_rec_registration_addr, target_action_name, "");
    info->trans_action->construct_tx(xtransaction_type_run_contract, 100, m_deposit, uinfo.nonce, "", tx_info);
...
bool api_method_imp::unstake_node_deposit(const user_info & uinfo, uint64_t unstake_deposit, std::ostringstream & out_str, std::function<void(NodeRegResult *)> func) {
    ...
    auto tx_info = tcash::data::xtx_action_info(uinfo.account, "", param_s, tcash::sys_contract_rec_registration_addr, target_action_name, param_t);
    info->trans_action->construct_tx(xtransaction_type_run_contract, 100, m_deposit, uinfo.nonce, "", tx_info);
...
bool api_method_imp::unRegisterNode(const user_info & uinfo, std::ostringstream & out_str, std::function<void(NodeRegResult *)> func) {
    ...
    auto tx_info = tcash::data::xtx_action_info(uinfo.account, "", param_s, tcash::sys_contract_rec_registration_addr, "unregisterNode", "");
    info->trans_action->construct_tx(xtransaction_type_run_contract, 100, m_deposit, uinfo.nonce, "", tx_info);
...
bool api_method_imp::redeemNodeDeposit(const user_info & uinfo, std::ostringstream & out_str, std::function<void(NodeRegResult *)> func) {
    ...
    auto tx_info = tcash::data::xtx_action_info(uinfo.account, "", param_s, tcash::sys_contract_rec_registration_addr, "redeemNodeDeposit", "");
    info->trans_action->construct_tx(xtransaction_type_run_contract, 100, m_deposit, uinfo.nonce, "", tx_info);
...
bool api_method_imp::setDividendRatio(const user_info & uinfo, uint32_t dividend_rate, std::ostringstream & out_str, std::function<void(NodeRegResult *)> func) {
    ...
    auto tx_info = tcash::data::xtx_action_info(uinfo.account, "", param_s, tcash::sys_contract_rec_registration_addr, "setDividendRatio", param_t);
    info->trans_action->construct_tx(xtransaction_type_run_contract, 100, m_deposit, uinfo.nonce, "", tx_info);
...
bool api_method_imp::voteNode(const user_info & uinfo, std::map<std::string, int64_t> & vote_infos, std::ostringstream & out_str, std::function<void(SetVoteResult *)> func) {
    ...
    auto tx_info = tcash::data::xtx_action_info(uinfo.account, "", "", tcash::sys_contract_sharding_vote_addr, "voteNode", param_t);
...
bool api_method_imp::unVoteNode(const user_info & uinfo, std::map<std::string, int64_t> & vote_infos, std::ostringstream & out_str, std::function<void(SetVoteResult *)> func) {
    ...
    auto tx_info = tcash::data::xtx_action_info(uinfo.account, "", "", tcash::sys_contract_sharding_vote_addr, "unvoteNode", param_t);
...
bool api_method_imp::claimNodeReward(const user_info & uinfo, std::ostringstream & out_str, std::function<void(ClaimRewardResult *)> func) {
    ...
    auto tx_info = tcash::data::xtx_action_info(uinfo.account, "", param_s, tcash::sys_contract_sharding_reward_claiming_addr, "claimNodeReward", "");
    info->trans_action->construct_tx(xtransaction_type_run_contract, 100, m_deposit, uinfo.nonce, "", tx_info);
...
bool api_method_imp::claimVoterDividend(const user_info & uinfo, std::ostringstream & out_str, std::function<void(ClaimRewardResult *)> func) {
    ...
    auto tx_info = tcash::data::xtx_action_info(uinfo.account, "", param_s, tcash::sys_contract_sharding_reward_claiming_addr, "claimVoterDividend", "");
    info->trans_action->construct_tx(xtransaction_type_run_contract, 100, m_deposit, uinfo.nonce, "", tx_info);
...
bool api_method_imp::submitProposal(const user_info & uinfo,
    ...
    auto tx_info = tcash::data::xtx_action_info(uinfo.account, source_action_name, param_s, tcash::sys_contract_rec_tcc_addr, "submitProposal", param_t);
    info->trans_action->construct_tx(xtransaction_type_run_contract, 100, m_deposit, uinfo.nonce, "", tx_info);
...
bool api_method_imp::withdrawProposal(const user_info & uinfo, const std::string & proposal_id, std::ostringstream & out_str, std::function<void(WithdrawProposalResult *)> func) {
    ...
    auto tx_info = tcash::data::xtx_action_info(uinfo.account, "", param_s, tcash::sys_contract_rec_tcc_addr, "withdrawProposal", param_t);
    info->trans_action->construct_tx(xtransaction_type_run_contract, 100, m_deposit, uinfo.nonce, "", tx_info);
...
bool api_method_imp::tccVote(const user_info & uinfo, const std::string & proposal_id, bool option, std::ostringstream & out_str, std::function<void(VoteProposalResult *)> func) {
    ...
    auto tx_info = tcash::data::xtx_action_info(uinfo.account, "", param_s, tcash::sys_contract_rec_tcc_addr, "tccVote", param_t);
    info->trans_action->construct_tx(xtransaction_type_run_contract, 100, m_deposit, uinfo.nonce, "", tx_info);
...
static void set_user_info(task_info_callback<T> * info,
    ...
    if (method == CMD_ACCOUNT_TRANSACTION || method == CMD_GET_BLOCK || method == CMD_GET_BLOCKS_BY_HEIGHT) {
        info->params["version"] = tcash::data::RPC_VERSION_V2;
    } else {
        info->params["version"] = tcash::data::RPC_VERSION_V1;
    }
    ...
    // info->params[tcash::xChainRPC::xrpc_signature::secretkey_key_] = uinfo.secret_key;
    // info->params[tcash::xChainRPC::xrpc_signature::method_key_] = uinfo.sign_method;
    // info->params[tcash::xChainRPC::xrpc_signature::version_key_] = uinfo.sign_version;
}
...
bool api_method_imp::nodeInfoReg(const user_info & uinfo, const std::string & account_name, uint64_t expiry_time, const std::string & account_cert,
    ...
    auto tx_info = tcash::data::xtx_action_info(uinfo.account, "", param_s, tcash::sys_contract_rec_node_manage_addr, target_action_name, param_t);
    info->trans_action->construct_tx(xtransaction_type_run_contract, 100, m_deposit, uinfo.nonce, "", tx_info);
