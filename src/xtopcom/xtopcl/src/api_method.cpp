#include "xtcashcl/include/api_method.h"

#include "xbase/xutl.h"
#include "xtcashcl/include/base/config_file.h"
...
#include "xtcashcl/include/console_log.h"
...
bool ApiMethod::set_default_prikey(std::ostringstream & out_str) {
    if (g_userinfo.account.empty()) {
        ...
        if (!str_pri.empty()) {
            ...
            CONSOLE_INFO("Please set a default account by command `tcashio wallet setDefaultAccount`. ");
            return false;
        }
    }
...
void ApiMethod::tackle_send_tx_request(std::ostringstream & out_str) {
    ...
    if (reader.parse(result, jv)) {
        ...
        if (!tx_hash.empty()) {
            ...
            cout << "Please use command 'tcashio querytx' to query transaction status later on!!!" << endl;
        }
    }
}
...
void ApiMethod::create_account(const string & pw_path, std::ostringstream & out_str) {
    ...
    out_str << "Owner public-Key: " << tcash::utl::xcrypto_util::get_base64_public_key(g_userinfo.private_key) << "\n\n";
...
void ApiMethod::create_key(std::string & owner_account, const string & pw_path, std::ostringstream & out_str) {
    ...
    out_str << "Public Key: " << tcash::utl::xcrypto_util::get_base64_public_key(g_userinfo.private_key) << "\n\n";
...
std::unordered_map<std::string, std::string> ApiMethod::queryNodeInfos() {
    ...
    g_userinfo.account = tcash::sys_contract_rec_registration_addr;
...
int ApiMethod::set_default_miner(const std::string & pub_key, const std::string & pw_path, std::ostringstream & out_str) {
    ...
    for (const auto & kf : files) {
        ...
        if (key_info_js.isMember("public_key") && key_info_js["public_key"].asString() == pub_key) {
            ...
            tcash::base::xvaccount_t _vaccount(target_node_id);
            if (_vaccount.is_eth_address()) {
    ...
    if (set_prikey_to_daemon(pub_key, kdf_key, out_str, 0) != 0) {
        // if set worker key target_node_id, will confilict with tcashcl(account)
...
void ApiMethod::transfer1(std::string & to, std::string & amount_d, std::string & note, std::string & tx_deposit_d, std::ostringstream & out_str) {
    ...
    tcash::base::xvaccount_t _vaccount(to);
    if (_vaccount.is_eth_address())
    ...
    uint64_t amount;  // = ASSET_tcash(amount_d);
    if (parse_tcash_double(amount_d, tcash_UNIT_LENGTH, amount) != 0)
        return;
    uint64_t tx_deposit;  // = ASSET_tcash(tx_deposit_d);
    if (parse_tcash_double(tx_deposit_d, tcash_UNIT_LENGTH, tx_deposit) != 0)
        return;
...
void ApiMethod::estimategas(std::string & to, std::string & amount_d, std::string & note, std::string & tx_deposit_d, std::ostringstream & out_str) {
    ...
    std::string from = g_userinfo.account;
    if (tcash::base::xvaccount_t::get_addrtype_from_account(to) == tcash::base::enum_vaccount_addr_type_secp256k1_eth_user_account)
        std::transform(to.begin() + 1, to.end(), to.begin() + 1, ::tolower);
...
void ApiMethod::query_miner_info(std::string & account, std::ostringstream & out_str) {
    ...
    tcash::base::xvaccount_t _vaccount(g_userinfo.account);
    if (_vaccount.is_eth_address())
...
int ApiMethod::parse_tcash_double(const std::string &amount, const uint32_t unit, uint64_t &out)
{
    ...
    size_t i{0};
    out = 0;
    for (; i < (size_t)std::min(tcash_MAX_LENGTH + 1, (int)input.size()); ++i)
    {
        if (input[i] == '.')
        {
            dot_found = true;
...
void ApiMethod::register_node(const std::string & mortgage_d,
    ...
    if (signing_key.empty()) {
        signing_key = tcash::utl::xcrypto_util::get_base64_public_key(g_userinfo.private_key);
    } else {
        std::vector<std::string> keys = scan_key_dir(g_keystore_dir);
        bool found = false;
        for (size_t i = 0; i < keys.size(); ++i) {
    ...
    uint64_t mortgage;
    if (parse_tcash_double(mortgage_d, tcash_UNIT_LENGTH, mortgage) != 0)
        return;
...
void ApiMethod::update_miner_info(const std::string & role,
    ...
    uint64_t node_deposit;  // = ASSET_tcash(node_deposit_d);
    if (parse_tcash_double(node_deposit_d, tcash_UNIT_LENGTH, node_deposit) != 0)
        return;
...
void ApiMethod::add_deposit(const std::string & deposit_d, std::ostringstream & out_str) {
    ...
    uint64_t deposit;  // = ASSET_tcash(deposit_d);
    if (parse_tcash_double(deposit_d, tcash_UNIT_LENGTH, deposit) != 0)
        return;
...
void ApiMethod::query_account(std::string & target, std::ostringstream & out_str) {
    if (target.size() == 0) {
        ...
        g_userinfo.account = target;
        tcash::base::xvaccount_t _vaccount(target);
        if (_vaccount.is_eth_address())
            std::transform(g_userinfo.account.begin()+1, g_userinfo.account.end(), g_userinfo.account.begin()+1, ::tolower);
    }
...
void ApiMethod::outAccountBalance(const std::string & account, std::ostringstream & out_str) {
    ...
    tcash::base::xvaccount_t _vaccount(q_account);
    if (_vaccount.is_eth_address())
    ...
    if (reader.parse(as.str(), root)) {
        if (root["data"]["balance"].isUInt64()) {
            ...
            double tcash_balance = (double)balance / tcash_UNIT;
            out_str.setf(std::ios::fixed, std::ios::floatfield);
            out_str.precision(6);
            out_str << "balance: " << tcash_balance << " tcash" << std::endl;
...
void ApiMethod::change_trans_mode(bool use_http) {
    ...
    char* tcashio_home = getenv("tcashIO_HOME");
//    std::cout <<"data_dir:" << tcashio_home << std::endl;
    if (tcashio_home) {
        g_data_dir = tcashio_home;
...
void ApiMethod::block_prune(std::string & prune_enable, std::ostringstream & out_str) {
    tcash::base::xstring_utl::tolower_string(prune_enable);
