...
#include "xtcashcl/include/protocol.h"
...
int protocol::encode_transaction_params(Json::Value & root, tcash::data::xtransaction_t * tx_ptr) {
    if (tx_ptr == nullptr)
        return -1;

    Json::Value & result_json = root["params"];
    if (tx_ptr->get_tx_version() == tcash::data::xtransaction_version_2) {
        tx_ptr->parse_to_json(result_json);
    } else {
        tx_ptr->parse_to_json(result_json, tcash::data::RPC_VERSION_V1);
    }
    return 0;
}

void protocol::set_transaction(tcash::data::xtransaction_ptr_t & val) {
    trans_action_ = val;
}
        ...
        if (reader.parse(params, root)) {
            ...
            if (!data.isNull()) {
                ...
                if (data["latest_tx_hash_xxhash64"].isString()) {
                    ...
                    result->_last_hash_xxhash64 = tcash::data::hex_to_uint64(xxhash_str);
                }
            }
        }
    } catch (...) {
