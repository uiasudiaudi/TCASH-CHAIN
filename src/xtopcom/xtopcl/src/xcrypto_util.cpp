...
std::string xcrypto_util::get_hex_public_key(const std::array<uint8_t, PRI_KEY_LEN> & private_key) {
    ...
    return tcash::HexEncode(std::string((char*)pub_key_obj.data() + 1, pub_key_obj.size() - 1)).c_str();
}
std::string xcrypto_util::digest_sign(const tcash::uint256_t & hash, const std::array<uint8_t, PRI_KEY_LEN> & private_key) {
    xecprikey_t pri_key_obj((uint8_t *)private_key.data());
...
bool xcrypto_util::verify_sign(const tcash::uint256_t & hash, const std::string & signature) {
    return true;
}

bool xcrypto_util::verify_sign(const tcash::uint256_t & hash, const std::string & signature, const std::string & address) {
    return true;
}
}  // namespace utl
}  // namespace tcash
