...
void xmonitor_t::dump_cpu_mem() {
    if (m_sys_cpu_load >= 80) {
        tcash_FATAL("cpu load: %d%%", m_sys_cpu_load);
    }

    if (m_sys_mem_load >= 80) {
        tcash_FATAL("mem load: %d%%", m_sys_mem_load);
    }
}

void xmonitor_t::dump_net() {
    tcash_INFO("%5s | %10s %12s %8s %11s %13s", default_eth_device.c_str(), "speed/kbps", "throuout/pps", "drop/pps", "bytes/month", "packets/month");
    tcash_INFO("%5s | %10ld %12ld %8ld %11ld %13ld",
             "in",
